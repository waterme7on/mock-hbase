...
import org.waterme7on.hbase.regionserver.NonThreadSafeMemStoreSizing;
import org.waterme7on.hbase.regionserver.Segment;

/**
 * An abstract class, which implements the behaviour shared by all concrete
 * memstore instances.
 */
...
public abstract class AbstractMemStore implements MemStore {
  ...
  // @formatter:on
    ...
    /**
     * Updates the wal with the lowest sequence id (oldest entry) that is still in
     * memory
     * 
     * @param onlyIfMoreRecent a flag that marks whether to update the sequence id
     *                         no matter what or
    ...
    /*
     * Inserts the specified Cell into MemStore and deletes any existing versions of
     * the same
     * row/family/qualifier as the specified Cell. <p> First, the specified Cell is
     * inserted into the
     * Memstore. <p> If there are any existing Cell in this MemStore with the same
     * row, family, and
    ...
    protected void doAdd(MutableSegment currentActive, Cell cell, MemStoreSizing memstoreSizing) {
        ...
        // HBASE-15180). By default MSLAB is ON and we might have copied cell to MSLAB
        // area. If
        // not we must do below deep copy. Or else we will keep referring to the bigger
        // chunk of
        ...
        // 2. When the size of the cell is bigger than the max size supported by MSLAB.
        // See
    ...
    private void doUpsert(MutableSegment currentActive, Cell cell, long readpoint,
        ...
        // here.
        // This cell data is backed by the same byte[] where we read request in RPC(See
        // HBASE-15180). We must do below deep copy. Or else we will keep referring to
        // the bigger
     ...
     * 
     * @return Next row or null if none found. If one found, will be a new KeyValue
     * -- can be
    ...
    /*
     * Internal version of add() that doesn't clone Cells with the allocator, and
     * doesn't take the
    ...
    /**
     * Returns The total size of cells in this memstore. We will not consider cells
     * in the snapshot
     */
    protected abstract long keySize();

    /**
     * @return The total heap size of cells in this memstore. We will not consider
     *         cells in the
     *         snapshot
     */
