...
import org.waterme7on.hbase.regionserver.InternalScanner;
import org.waterme7on.hbase.regionserver.KeyValueHeap;
import org.apache.hadoop.hbase.regionserver.ScanInfo;
import org.apache.hadoop.hbase.regionserver.ScanType;
import org.waterme7on.hbase.ScannerContext;
import org.waterme7on.hbase.ScannerContext.LimitScope;
import org.waterme7on.hbase.ScannerContext.NextState;
...
public class StoreScanner extends NonReversedNonLazyKeyValueScanner
        implements InternalScanner, ChangedReadersObserver {
    ...
    /**
     * The number of cells scanned in between timeout checks. Specifying a larger
     * value means that
    ...
    /**
     * If the read type is Scan.ReadType.DEFAULT, we will start with pread, and if
     * the kvs we scanned
     * reaches this limit, we will reopen the scanner with stream. The default value
     * is 4 times of
     * block size for this store. If configured with a value <0, for all scans with
     * ReadType DEFAULT,
    ...
    // it maybe changed if we use Scan.ReadType.DEFAULT and we have read lots of
    // data.
    ...
    // Since CompactingMemstore is now default, we get three memstore scanners from
    // a flush
    ...
    private StoreScanner(HStore store, Scan scan, ScanInfo scanInfo, int numColumns, long readPt,
        ...
        if (get) {
            ...
            // For compaction scanners never use Pread as already we have stream based
            // scanners on the
            ...
            // Always start with pread unless user specific stream. Will change to stream
            // later if
            // readType is default if the scan keeps running for a long time.
            this.scanUsePread = this.readType != Scan.ReadType.STREAM;
        }
        // Parallel seeking is on if the config allows and more there is more than one
        // store file.
    ...
    /**
     * Opens a scanner across memstore, snapshot, and all StoreFiles. Assumes we are
     * not in a
     * compaction.
     * 
    ...
    public StoreScanner(HStore store, ScanInfo scanInfo, Scan scan, NavigableSet<byte[]> columns,
        ...
        try {
            ...
            // remove us from the HStore#changedReaderObservers here or we'll have no chance
            // to
    ...
    // NavigableSet<byte[]> columns, long readPt, boolean isFileScanner) throws
    // IOException {
    ...
    // if (isFileScanner) {
    // List<org.apache.hadoop.hbase.regionserver.KeyValueScanner> scanners = null;
    // try {
    // // Pass columns to try to filter out unnecessary StoreFiles.
    // scanners = selectFileStoreScannersFrom(store,
    // store.getFileStoreSacnners(cacheBlocks, scanUsePread, false, matcher,
    // scan.getStartRow(),
    // scan.includeStartRow(), scan.getStopRow(), scan.includeStopRow(),
    // this.readPt));
    ...
    // // Always check bloom filter to optimize the top row seek for delete
    // // family marker.
    // seekFileStoreScanners(scanners, matcher.getStartKey(), explicitColumnQuery &&
    // lazySeekEnabledGlobally,
    // parallelSeekEnabled);
    ...
    // // set rowOffset
    // this.storeOffset = scan.getRowOffsetPerColumnFamily();
    // addCurrentFileStoreScanners(scanners);
    // // Combine all seeked scanners with a heap
    // resetFileStoreKVHeap(scanners, comparator);
    // } catch (IOException e) {
    // clearAndCloseOrigin;(scanners);
    // // remove us from the HStore#changedReaderObservers here or we'll have no
    // chance
    // // to
    // // and might cause memory leak
    ...
    private StoreScanner(HStore store, ScanInfo scanInfo, List<? extends KeyValueScanner> scanners,
        ...
        matcher = CompactionScanQueryMatcher.create(scanInfo, scanType, smallestReadPoint, earliestPutTs,
                oldestUnexpiredTS, now, dropDeletesFromRow, dropDeletesToRow, null);
    ...
    public boolean next(List<Cell> outResult, ScannerContext scannerContext) throws IOException {
        ...
        // if the heap was left null, then the scanners had previously run out anyways,
        // close and
        // return.
        if (this.heap == null) {
        ...
        // If no limits exists in the scope LimitScope.Between_Cells then we are sure we
        // are changing
        ...
        try {
            LOOP: do {
                // Update and check the time limit based on the configured value of
                // cellsPerTimeoutCheck
                // Or if the preadMaxBytes is reached and we may want to return so we can switch
                // to stream
                // in
                // the shipped method below.

                // todo: we should probably do this check in the shipped method as well, but
                if (kvsScanned % 10 == 0
                        || (scanUsePread && readType == Scan.ReadType.DEFAULT)) {
                ...
                if (scanUsePread && readType == Scan.ReadType.DEFAULT) {
                    // return immediately if we want to switch from pread to stream. We need this
                    // because we
                    // can
                    // only switch in the shipped method, if user use a filter to filter out
                    // everything and
                    // rpc
                    // timeout is very large then the shipped method will never be called until the
                    // whole scan
                    // is finished, but at that time we have already scan all the data...
                    // See HBASE-20457 for more details.
                    // And there is still a scenario that can not be handled. If we have a very
                    // large row,
                    // which
                    // have millions of qualifiers, and filter.filterRow is used, then even if we
                    // set the flag
                ...
                switch (qcode) {
                    ...
                    case INCLUDE_AND_SEEK_NEXT_COL:
                        ...
                        if (this.countPerRow > storeOffset) {
                            ...
                            /**
                             * Increment the metric if all the cells are from memstore. If not we will
                             * account it
                             * for mixed reads
                             */
    ...
    /**
     * If the top cell won't be flushed into disk, the new top cell may be changed
     * after
     * #reopenAfterFlush. Because the older top cell only exist in the memstore
     * scanner but the
     * memstore scanner is replaced by hfile scanner after #reopenAfterFlush. If the
     * row of top cell
     * is changed, we should return the current cells. Otherwise, we may return the
     * cells across
     * different rows.
     * 
    ...
    private void seekOrSkipToNextRow(Cell cell) throws IOException {
        // If it is a Get Scan, then we know that we are done with this row; there are
        // no more
     ...
     * or seek to an
     * arbitrary seek key. This method decides whether a seek is the most efficient
     * _actual_ way to
     * get us to the requested cell (SEEKs are more expensive than SKIP, SKIP, SKIP
     * inside the
     * current, loaded block). It does this by looking at the next indexed key of
     * the current HFile.
     * This key is then compared with the _SEEK_ key, where a SEEK key is an
     * artificial 'last possible
     * key on the row' (only in here, we avoid actually creating a SEEK key; in the
     * compare we work
     * with the current Cell but compare as though it were a seek key; see down in
     * matcher.compareKeyForNextRow, etc). If the compare gets us onto the next
     * block we *_SEEK,
     ...
     * after a few
     * SKIPs...</li>
     * <li>We want to SEEK when the chance is high that we'll be able to seek past
     * many Cells,
    ...
    private static void clearAndCloseOrigin(List<org.apache.hadoop.hbase.regionserver.KeyValueScanner> scanners) {
        if (scanners == null) {
            return;
        }
        for (org.apache.hadoop.hbase.regionserver.KeyValueScanner s : scanners) {
            s.close();
        }
        scanners.clear();
    }
    ...
    // // The reason for doing this is that when the current store scanner does not
    // // retrieve
    // // any new cells, then the scanner is considered to be done. The heap of this
    // // scanner
    // // is not closed till the shipped() call is completed. Hence in that case if
    // at
    // // all
    // // the partial close (close (false)) has been called before updateReaders(),
    // // there is no
    // // need for the updateReaders() to happen.
    // LOG.debug("StoreScanner already has the close lock. There is no need to
    // updateReaders");
    // // no lock acquired.
    ...
    // // SEE HBASE-19468 where the flushed files are getting compacted even before
    // a
    // // scanner
    // // calls next(). So its better we create scanners here rather than next()
    // call.
    // // Ensure
    // // these scanners are properly closed() whether or not the scan is completed
    // // successfully
    // // Eagerly creating scanners so that we have the ref counting ticking on the
    // // newly created
    // // store files. In case of stream scanners this eager creation does not
    // induce
    // // performance
    // // penalty because in scans (that uses stream scanners) the next() call is
    // bound
    // // to happen.
    // List<KeyValueScanner> scanners = store.getScanners(sfs, cacheBlocks, get,
    // usePread,
    ...
    protected final boolean reopenAfterFlush() throws IOException {
        // here we can make sure that we have a Store instance so no null check on
        // store.
        Cell lastTop = heap.peek();
        // When we have the scan object, should we not pass it to getScanners() to get a
        // limited set of
        ...
        try {
            List<KeyValueScanner> allScanners = new ArrayList<>(
                    flushedstoreFileScanners.size() + memStoreScannersAfterFlush.size());
            ...
            // Clear the current set of flushed store files scanners so that they don't get
            // added again
        ...
        // add the newly created scanners on the flushed files and the current active
        // memstore scanner
        addCurrentScanners(scanners);
        // Combine all seeked scanners with a heap
    ...
    protected final boolean checkFlushed() {
        ...
        // the heap but will continue with the referenced memstore's snapshot. For
        // compactions
        // any way we don't need the updateReaders at all to happen as we still continue
        // with
        // the older files
        if (flushed) {
            // If there is a flush and the current scan is notified on the flush ensure that
            // the
    ...
    /**
     * Returns The estimated number of KVs seen by this scanner (includes some
     * skipped KVs).
     */
    ...
    public void shipped() throws IOException {
        if (prevCell != null) {
            ...
            // This applies to blocks that are got from Bucket cache, L1 cache and the
            // blocks
            // fetched from HDFS. Copying this would ensure that we let go the references to
            // these
        ...
        if (this.heap != null) {
            this.heap.shipped();
            // When switching from pread to stream, we will open a new scanner for each
            // store file, but
            // the old scanner may still track the HFileBlocks we have scanned but not sent
            // back to client
            // yet. If we close the scanner immediately then the HFileBlocks may be messed
            // up by others
            // before we serialize and send it back to client. The HFileBlocks will be
            // released in shipped
            // method, so we here will also open new scanners and close old scanners in
            // shipped method.
    ...
    @Override
    public void updateReaders(List<HStoreFile> sfs,
            List<org.apache.hadoop.hbase.regionserver.KeyValueScanner> memStoreScanners) throws IOException {
        // TODO Auto-generated method stub
        throw new UnsupportedOperationException("Unimplemented method 'updateReaders'");
    }

    protected List<org.apache.hadoop.hbase.regionserver.KeyValueScanner> selectFileStoreScannersFrom(HStore store,
            List<? extends org.apache.hadoop.hbase.regionserver.KeyValueScanner> allScanners) {
        boolean memOnly;
        boolean filesOnly;
        if (scan instanceof org.apache.hadoop.hbase.regionserver.InternalScan) {
            org.apache.hadoop.hbase.regionserver.InternalScan iscan = (org.apache.hadoop.hbase.regionserver.InternalScan) scan;
            memOnly = iscan.isCheckOnlyMemStore();
            filesOnly = iscan.isCheckOnlyStoreFiles();
        } else {
            memOnly = false;
        ...
        List<org.apache.hadoop.hbase.regionserver.KeyValueScanner> scanners = new ArrayList<>(allScanners.size());
        ...
        // include only those scan files which pass all filters
        for (org.apache.hadoop.hbase.regionserver.KeyValueScanner kvs : allScanners) {
            boolean isFile = kvs.isFileScanner();
            if ((!isFile && filesOnly) || (isFile && memOnly)) {
                kvs.close();
                continue;
