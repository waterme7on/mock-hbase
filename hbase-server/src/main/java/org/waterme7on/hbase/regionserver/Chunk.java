...
public abstract class Chunk {
    ...
    /**
     * Offset for the next allocation, or the sentinel value -1 which implies that
     * the chunk is still
     * uninitialized.
     */
    ...
    /**
     * Create an uninitialized chunk. Note that memory is not allocated yet, so this
     * is cheap.
     * 
     * @param size in bytes
    ...
    /**
     * Actually claim the memory for this chunk. This should only be called from the
     * thread that
    ...
    /**
     * Try to allocate <code>size</code> bytes from the chunk. If a chunk is tried
     * to get allocated
     * before init() call, the thread doing the allocation will be in busy-wait
     * state as it will keep
