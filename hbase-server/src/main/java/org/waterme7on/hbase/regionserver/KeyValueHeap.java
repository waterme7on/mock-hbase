 ...
 * <p>
 * This class is used at the Region level to merge across Stores and at the
 * Store level to merge
 * across the memstore and StoreFiles.
 * <p>
 * In the Region case, we also need InternalScanner.next(List), so this class
 * also implements
 * InternalScanner. WARNING: As is, if you try to use this as an InternalScanner
 * at the Store level,
...
public class KeyValueHeap extends NonReversedNonLazyKeyValueScanner
    ...
    // closed
    // scans are collected and when the final scanner.close() happens will perform
    // the
    // actual close.
    protected List<KeyValueScanner> scannersForDelayedClose = null;

    /**
     * The current sub-scanner, i.e. the one that contains the next key/value to
     * return to the client.
     * This scanner is NOT included in {@link #heap} (but we frequently add it back
     * to the heap and
     * pull the new winner out). We maintain an invariant that the current
     * sub-scanner has already
     * done a real seek, and that current.peek() is always a real key/value (or
     * null) except for the
     * fake last-key-on-row-column supplied by the multi-column Bloom filter
     * optimization, which is OK
     * to propagate to StoreScanner. In order to ensure that, always use
     * {@link #pollRealKV()} to
     * update current.
     */
    ...
    public boolean next(List<Cell> result, ScannerContext scannerContext) throws IOException {
         ...
         * further rows to be
         * fetched. So, we can close a scanner if it returns false. All existing
         * implementations seem to
         * be fine with this. It is much more efficient to close scanners which are not
         * needed than keep
    ...
    protected static class KVScannerComparator implements Comparator<KeyValueScanner> {
        ...
        public int compare(KeyValueScanner left, KeyValueScanner right) {
            ...
            if (comparison != 0) {
                ...
                // Since both the keys are exactly the same, we break the tie in favor of higher
                // ordered
                // scanner since it'll have newer data. Since higher value should come first, we
                // reverse
                // sort here.
    ...
    public void close() {
        ...
        if (this.heap != null) {
            // Order of closing the scanners shouldn't matter here, so simply iterate and
            // close them.
    ...
    /**
     * Seeks all scanners at or below the specified seek key. If we earlied-out of a
     * row, we may end
     * up skipping values that were never reached yet. Rather than iterating down,
     * we want to give the
     ...
     * <p>
     * This function (and {@link #reseek(Cell)}) does not do multi-column Bloom
     * filter and lazy-seek
     * optimizations. To enable those, call
     ...
     *                 Enables Bloom
     *                 filter and most-recent-file-first optimizations for
     *                 multi-column get/scan
     *                 queries.
     ...
     * seek has been done on
     * it. Works by fetching the top sub-scanner, and if it has not done a real
     * seek, making it do so
     * (which will modify its top KV), putting it back, and repeating this until
     * success. Relies on
     * the fact that on a lazy seek we set the current key of a StoreFileScanner to
     * a KV that is not
     * greater than the real next KV to be read from that file, so the scanner that
     * bubbles up to the
     * top of the heap will have global next KV in this scanner heap if (1) it has
     * done a real seek
     * and (2) its KV is the top among all top KVs (some of which are fake) in the
     * scanner heap.
     */
