 ...
 * (2MB) chunks from
 * and then doles it out to threads that request slices into the array. These
 * chunks can get pooled
 * as well. See {@link ChunkCreator}.
 * <p>
 * The purpose of this is to combat heap fragmentation in the regionserver. By
 * ensuring that all
 * Cells in a given memstore refer only to large chunks of contiguous memory, we
 * ensure that large
 ...
 * collection occurs.
 * <p>
 * This manages the large sized chunks. When Cells are to be added to Memstore,
 * MemStoreLAB's
 * {@link #copyCellInto(Cell)} gets called. This allocates enough size in the
 * chunk to hold this
 * cell's data and copies into this area and then recreate a Cell over this
 * copied data.
 * <p>
 * 
 * @see ChunkCreator
...
public interface MemStoreLAB {
     ...
     * over the copied the data. When this MemStoreLAB can not copy this Cell, it
     * returns null. Since
     * the process of flattening to CellChunkMap assumes all cells are allocated on
     * MSLAB, and since
     * copyCellInto does not copy big cells (for whom size > maxAlloc) into MSLAB,
     * this method is
     * called while the process of flattening to CellChunkMap is running, for
