 ...
 * active cell set or its
 * snapshot. This abstraction facilitates the management of the compaction
 * pipeline and the shifts
 * of these segments from active set to snapshot set in the default
 * implementation.
 */
public abstract class Segment implements MemStoreSizing {

    public final static long FIXED_OVERHEAD = ClassSize.align(ClassSize.OBJECT + 6 * ClassSize.REFERENCE // cellSet,
                                                                                                         // comparator,
                                                                                                         // updatesLock,
                                                                                                         // memStoreLAB,
                                                                                                         // memStoreSizing,
    ...
    public void close() {
        ...
        // do not set MSLab to null as scanners may still be reading the data here and
        // need to decrease
        // the counter when they finish
    }

    /**
     * If the segment has a memory allocator the cell is being cloned to this space,
     * and returned;
     * otherwise the given cell is returned When a cell's size is too big (bigger
     * than maxAlloc), it
     * is not allocated on MSLAB. Since the process of flattening to CellChunkMap
     * assumes that all
     * cells are allocated on MSLAB, during this process, the input parameter
     * forceCloneOfBigCell is
     * set to 'true' and the cell is copied into MSLAB.
     * 
     * @return either the given cell or its clone
     */
    public Cell maybeCloneWithAllocator(org.apache.hadoop.hbase.Cell cell, boolean forceCloneOfBigCell) {
        if (this.memStoreLAB == null) {
            return (Cell) cell;
        }
        ...
        if (forceCloneOfBigCell) {
            ...
            cellFromMslab = (Cell) this.memStoreLAB.copyCellInto(cell);
        }
        return (cellFromMslab != null) ? cellFromMslab : (Cell) cell;
    }
    ...
    /**
     * Setting the CellSet of the segment - used only for flat immutable segment for
     * setting immutable
    ...
    public Iterator<org.apache.hadoop.hbase.Cell> iterator() {
        return getCellSet().iterator();
    }

    public SortedSet<org.apache.hadoop.hbase.Cell> headSet(org.apache.hadoop.hbase.Cell firstKeyOnRow) {
    ...
    protected void internalAdd(Cell cell, boolean mslabUsed, MemStoreSizing memstoreSizing,
        ...
        boolean succ = getCellSet().add((org.apache.hadoop.hbase.Cell) cell);
    ...
    protected void updateMetaInfo(Cell cellToAdd, boolean succ, boolean mslabUsed,
        ...
        // If there's already a same cell in the CellSet and we are using MSLAB, we must
        // count in the
        // MSLAB allocation size as well, or else there will be memory leak (occupied
        // heap size larger
     ...
     *         heap size itself and additional overhead because of addition on to
     *         CSLM.
     */
     ...
     */
    protected SortedSet<org.apache.hadoop.hbase.Cell> tailSet(Cell firstCell) {
