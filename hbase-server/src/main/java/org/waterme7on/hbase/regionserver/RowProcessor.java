...
/**
 * Defines the procedures to atomically perform multiple scans and mutations on
 * a HRegion. This is
 * invoked by {@link Region#processRowsWithLocks(RowProcessor)}. This class
 * performs scans and
 * generates mutations and WAL edits. The locks and MVCC will be handled by
 * HRegion. The
 * RowProcessor user code could have data that needs to be sent across for
 * proper initialization at
 * the server side. The generic type parameter S is the type of the request data
 * sent to the server.
...
public interface RowProcessor<S extends Message, T extends Message> {
    ...
    /**
     * Rows to lock while operation. They have to be sorted with
     * <code>RowProcessor</code> to avoid
     * deadlock.
     */
     ...
     * implement this, even if
     * the method is simply returning an empty Message.
     */
    T getResult();

    /**
     * Is this operation read only? If this is true, process() should not add any
     * mutations or it
     * throws IOException.
     * 
    ...
    /**
     * HRegion handles the locks and MVCC and invokes this method properly. You
     * should override this
     * to create your own RowProcessor. If you are doing read-modify-write here, you
     * should consider
     * using <code>IsolationLevel.READ_UNCOMMITTED</code> for scan because we
     * advance MVCC after
    ...
    /**
     * The hook to be executed after the process() but before applying the Mutations
     * to region. Also
     ...
     * region. The
     * difference of this one with {@link #postProcess(HRegion, WALEdit, boolean)}
     * is this hook will
    ...
    /**
     * This method should return any additional data that is needed on the server
     * side to construct
     * the RowProcessor. The server will pass this to the
     * {@link #initialize(Message msg)} method. If
    ...
    /**
     * This method should initialize any field(s) of the RowProcessor with a parsing
     * of the passed
