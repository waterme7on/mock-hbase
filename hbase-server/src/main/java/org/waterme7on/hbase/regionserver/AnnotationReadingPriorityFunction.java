...
import org.waterme7on.hbase.ipc.QosPriority;
...
/**
 * Reads special method annotations and table names to figure a priority for use
 * by QoS facility in
 * ipc; e.g: rpcs to hbase:meta get priority.
 */
// TODO: Remove. This is doing way too much work just to figure a priority. Do
// as Elliott
...
public class AnnotationReadingPriorityFunction implements PriorityFunction {
    private static final Logger LOG = LoggerFactory.getLogger(AnnotationReadingPriorityFunction.class.getName());
    ...
    private final Class<? extends Message>[] knownArgumentClasses = new Class[] { GetRegionInfoRequest.class,
            GetStoreFileRequest.class, CloseRegionRequest.class,
    ...
    /**
     * Calls {@link #AnnotationReadingPriorityFunction(RSRpcServices, Class)} using
     * the result of
    ...
    /**
     * Constructs the priority function given the RPC server implementation and the
     * annotations on the
     * methods in the provided {@code clz}. The RPC server implementation The
     * concrete RPC server
    ...
    public AnnotationReadingPriorityFunction(final RSRpcServices rpcServices,
        ...
        for (Method m : clz.getMethods()) {
            ...
            if (p != null) {
                // Since we protobuf'd, and then subsequently, when we went with pb style,
                // method names
                // are capitalized. This meant that this brittle compare of method names gotten
                // by
                // reflection no longer matched the method names coming in over pb. TODO: Get
                // rid of this
    ...
    /**
     * Returns a 'priority' based on the request type. Currently the returned
     * priority is used for
     * queue selection. See the SimpleRpcScheduler as example. It maintains a queue
     * per 'priory type'
     * HIGH_QOS (meta requests), REPLICATION_QOS (replication requests), NORMAL_QOS
     * (user requests).
     */
    @Override
     ...
     * 
     * @return Return the priority from the annotation. If there isn't an
     *         annotation, this returns
    ...
    /**
     * Get the priority for a given request from the header and the param This
     * doesn't consider which
    ...
    protected int getBasePriority(RequestHeader header, Message param) {
        ...
        try {
            // Check if the param has a region specifier; the pb methods are hasRegion and
            // getRegion if
            // hasRegion returns true. Not all listed methods have region specifier each
            // time. For
            // example, the ScanRequest has it on setup but thereafter relies on the
            // scannerid rather than
            ...
            // Not good throwing an exception out of here, a runtime anyways. Let the query
            // go into the
            // server and have it throw the exception if still an issue. Just mark it normal
            // priority.
            if (LOG.isTraceEnabled())
        ...
        if (param instanceof ScanRequest) { // scanner methods...
            ...
            // if (LOG.isTraceEnabled()) {
            // // Scanner requests are small in size so TextFormat version should not
            // overwhelm
            // // log.
            // LOG.trace("High priority scanner request " +
