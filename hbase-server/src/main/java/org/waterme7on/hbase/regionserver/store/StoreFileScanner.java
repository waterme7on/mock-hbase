...
// * {@link HFileScanner}
// *
// * @param useMVCC If true, scanner will filter out updates
// * with MVCC larger
// * than {@code readPt}.
// * @param readPt MVCC value to use to filter out the
// * updates newer than this
// * scanner.
// * @param hasMVCC Set to true if underlying store file
// * reader has MVCC info.
// * @param scannerOrder Order of the scanner relative to other
// * scanners. See
...
// PriorityQueue<HStoreFile> sortedFiles = new PriorityQueue<>(files.size(),
// StoreFileComparators.SEQ_ID);
// for (HStoreFile file : files) {
// // The sort function needs metadata so we need to open reader first before
// // sorting the list.
// file.initReader();
...
// canOptimizeForNonNullColumn);
// } else {
// scanner = sf.getStreamScanner(canUseDrop, cacheBlocks, isCompaction, readPt,
// i,
// canOptimizeForNonNullColumn);
// }
...
// public static boolean seekAtOrAfter(HFileScanner s, Cell k) throws
// IOException {
...
// * that we find
// * requested columns in more recent files and won't have to seek in older
// files.
// * Creates a fake
// * key/value with the given row/column and the highest (most recent) possible
// * timestamp we might
// * get from this file. When users of such "lazy scanner" need to know the next
// * KV precisely (e.g.
// * when this scanner is at the top of the heap), they run
// * {@link #enforceSeek()}.
// * <p>
// * Note that this function does guarantee that the current KV of this scanner
// * will be advanced to
// * at least the given KV. Because of this, it does have to do a real seek in
// * cases when the seek
// * timestamp is older than the highest timestamp of the file, e.g. when we are
// * trying to seek to
// * the next row/column and use OLDEST_TIMESTAMP in the seek key.
// */
// @Override
// public boolean requestSeek(Cell kv, boolean forward, boolean useBloom) throws
// IOException {
...
// setCurrentCell(PrivateCellUtil.createFirstOnRowColTS(kv,
// maxTimestampInFile));
// } else {
