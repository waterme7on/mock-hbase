...
import org.waterme7on.hbase.regionserver.Region.Operation;
...
class RegionScannerImpl implements RegionScanner, Shipper, RpcCallback {
    ...
    /**
     * Heap of key-values that are not essential for the provided filters and are
     * thus read on demand,
    ...
    RegionScannerImpl(Scan scan, List<KeyValueScanner> additionalScanners, HRegion region,
         ...
         * construct a default
         * scanner context that can be used to enforce the batch limit in the event that
         * a
    ...
    private void initializeScanners(Scan scan, List<KeyValueScanner> additionalScanners)
            throws IOException {
        // Here we separate all scanners into two lists - scanner that provide data
        // required
        // by the filter to operate (scanners list) and all others (joinedScanners
        // list).
    ...
    public boolean nextRaw(List<Cell> outResults, ScannerContext scannerContext) throws IOException {
        ...
        // If the size limit was reached it means a partial Result is being returned.
        // Returning a
        // partial Result means that we should not reset the filters; filters should
        // only be reset in
        // between rows
    ...
    /**
     * Fetches records with currentRow into results list, until next row, batchLimit
     * (if not -1) is
     * reached, or remainingResultSize (if not -1) is reaced
     * 
     * @param heap KeyValueHeap to fetch data from.It must be positioned on correct
     *             row before call.
    ...
    private boolean populateResult(List<Cell> results, KeyValueHeap heap,
        ...
        do {
            ...
            // We want to maintain any progress that is made towards the limits while
            // scanning across
            // different column families. To do this, we toggle the keep progress flag on
            // during calls
            // to the StoreScanner to ensure that any progress made thus far is not wiped
            // away.
            ...
            if (moreCellsInRow && scannerContext.checkBatchLimit(limitScope)) {
                ...
                ScannerContext.NextState state = moreCellsInRow ? NextState.SIZE_LIMIT_REACHED_MID_ROW
                        : NextState.SIZE_LIMIT_REACHED;
                return scannerContext.setScannerState(state).hasMoreValues();
            } else if (scannerContext.checkTimeLimit(limitScope)) {
                ScannerContext.NextState state = moreCellsInRow ? NextState.TIME_LIMIT_REACHED_MID_ROW
    ...
    /**
     * Based on the nextKv in the heap, and the current row, decide whether or not
     * there are more
     * cells to be read in the heap. If the row of the nextKv in the heap matches
     * the current row then
    ...
    private void resetProgress(ScannerContext scannerContext, int initialBatchProgress,
        ...
        // Starting to scan a new row. Reset the scanner progress according to whether
        // or not
        // progress should be kept.
        if (scannerContext.getKeepProgress()) {
            // Progress should be kept. Reset to initial values seen at start of method
            // invocation.
    ...
    private boolean nextInternal(List<Cell> results, ScannerContext scannerContext)
        ...
        // Save the initial progress from the Scanner context in these local variables.
        // The progress
        // may need to be reset a few times if rows are being filtered out so we save
        // the initial
        // progress.
        ...
        // that due to effects of filters or otherwise, we have an empty row in the
        // result.
        // Then we loop and try again. Otherwise, we must get out on the first iteration
        // via return,
        // "true" if there's more data to read, "false" if there isn't (storeHeap is at
        // a stop row,
        ...
        while (true) {
            ...
            // When has filter row is true it means that the all the cells for a particular
            // row must be
            // read before a filtering decision can be made. This means that filters where
            // hasFilterRow
            // run the risk of enLongAddering out of memory errors in the case that they are
            // applied to a
            // table that has very large rows.
            boolean hasFilterRow = this.filter != null && this.filter.hasFilterRow();

            // If filter#hasFilterRow is true, partial results are not allowed since
            // allowing them
            // would prevent the filters from being evaluated. Thus, if it is true, change
            // the
            ...
            if (joinedContinuationRow == null) {
                ...
                if (hasFilterRow) {
                    ...
                    // We don't know how the results have changed after being filtered. Must set
                    // progress
    ...
    private boolean joinedHeapMayHaveData(Cell currentRowCell) throws IOException {
        ...
        // If the next value in the joined heap does not match the current row, try to
        // seek to the
        // correct row
        if (!matchCurrentRow) {
     ...
     * HBASE-6429 combines both
     * filterRow & filterRow({@code List<KeyValue> kvs}) functions. While 0.94 code
     * or older, it may
     * not implement hasFilterRow as HBase-6429 expects because 0.94 hasFilterRow()
     * only returns true
     * when filterRow({@code List<KeyValue> kvs}) is overridden not the filterRow().
     * Therefore, the
    ...
    private static final List<Cell> MOCKED_LIST = new AbstractList<Cell>() {
        ...
        public boolean addAll(int index, Collection<? extends Cell> c) {
            return false; // this list is never changed as
    ...
    public void run() throws IOException {
        // This is the RPC callback method executed. We do the close in of the scanner
        // in this
        // callback
        this.close();
    }
