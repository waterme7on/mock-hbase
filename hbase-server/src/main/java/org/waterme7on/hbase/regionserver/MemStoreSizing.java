 ...
 * <ol>
 * <li></li>data size: the aggregated size of all key-value not including meta
 * data such as index,
 * time range etc.</li>
 * <li>heap size: the aggregated size of all data that is allocated on-heap
 * including all key-values
 * that reside on-heap and the metadata that resides on-heap</li>
 * <li></li>off-heap size: the aggregated size of all data that is allocated
 * off-heap including all
 ...
 * allocated on-heap (e.g., due to upserts) and 20MB of metadata from which 15MB
 * allocated off-heap
...
public interface MemStoreSizing {
    ...
    /**
     * @return Use this datastructure to return all three settings,
     *         {@link #getDataSize()},
