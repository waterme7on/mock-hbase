...
public class SegmentScanner implements KeyValueScanner {
    ...
    // Stop skipping KeyValues for MVCC if finish this row. Only used for reversed
    // scan
    ...
    public boolean reseek(Cell cell) throws IOException {
         ...
         * is executed concurrently with flush and puts, without locks. The ideal
         * implementation for
         * performance would use the sub skip list implicitly pointed by the iterator.
         * Unfortunately the
         * Java API does not offer a method to get it. So we remember the last keys we
         * iterated to and
     ...
     * 
     * @param key seek Cell
     * @return true if the scanner is at the valid KeyValue, false if such Cell does
     *         not exist
     */
    @Override
    ...
    public boolean seekToPreviousRow(Cell cell) throws IOException {
        ...
        do {
            ...
            SortedSet<org.apache.hadoop.hbase.Cell> cellHead = segment
                    .headSet((org.apache.hadoop.hbase.Cell) firstKeyOnRow);
            Cell lastCellBeforeRow = cellHead.isEmpty() ? null : (Cell) cellHead.last();
    ...
    public boolean seekToLastRow() throws IOException {
        ...
        Cell higherCell = segment.isEmpty() ? null : (Cell) segment.last();
        if (higherCell == null) {
            return false;
        }
     ...
     * MemStoreScanner, currently
     * returns true as default. Doesn't throw IllegalStateException in order not to
     * change the
     ...
     * store files,
     * MutableCellSetSegmentScanner always does the seek, therefore always returning
     * true.
     */
    @Override
    ...
    /**
     * This function should be never called on scanners that always do real seek
     * operations (i.e. most
     * of the scanners and also this one). The easiest way to achieve this is to
     * call
    ...
    /**
     * Private internal method for iterating over the segment, skipping the cells
     * with irrelevant MVCC
     */
    ...
    /**
     * Private internal method that returns the higher of the two key values, or
     * null if they are both
     * null
     */
