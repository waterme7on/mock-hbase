...
import static org.waterme7on.hbase.regionserver.HeapMemoryManager.BLOCK_CACHE_SIZE_MAX_RANGE_KEY;
import static org.waterme7on.hbase.regionserver.HeapMemoryManager.BLOCK_CACHE_SIZE_MIN_RANGE_KEY;
import static org.waterme7on.hbase.regionserver.HeapMemoryManager.MEMSTORE_SIZE_MAX_RANGE_KEY;
import static org.waterme7on.hbase.regionserver.HeapMemoryManager.MEMSTORE_SIZE_MIN_RANGE_KEY;
 ...
 * checks on number of
 * evictions, cache misses and flushes to decide whether there should be changes
 * in the heap size of
 * memstore/block cache. During each tuner operation tuner takes a step which
 * can either be
 * INCREASE_BLOCK_CACHE_SIZE (increase block cache size), INCREASE_MEMSTORE_SIZE
 * (increase memstore
 * size) and by default it is NEUTRAL (no change). We say block cache is
 * sufficient when there is no
 * block cache eviction at all or major amount of memory allocated to block
 * cache is empty,
 * similarly we say memory allocated for memstore is sufficient when there is no
 * memstore flushes
 * because of heap pressure or major amount of memory allocated to memstore is
 * empty. If both are
 * sufficient we do nothing, if exactly one of them is found to be sufficient we
 * decrease its size
 * by <i>step</i> and increase the other by same amount. If none of them is
 * sufficient we do
 * statistical analysis on number of cache misses and flushes to determine tuner
 * direction. Based on
 * these statistics we decide the tuner direction. If we are not confident about
 * which step
 * direction to take we do nothing and wait for next iteration. On expectation
 * we will be tuning for
 * at least 10% tuner calls. The number of past periods to consider for
 * statistics calculation can
 * be specified in config by
 * <i>hbase.regionserver.heapmemory.autotuner.lookup.periods</i>. Also
 * these many initial calls to tuner will be ignored (cache is warming up and we
 * leave the system to
 * reach steady state). After the tuner takes a step, in next call we insure
 * that last call was
 * indeed helpful and did not do us any harm. If not then we revert the previous
 * step. The step size
 * is dynamic and it changes based on current and past few tuning directions and
 * their step sizes.
 * We maintain a parameter <i>decayingAvgTunerStepSize</i> which is sum of past
 * tuner steps with
 * sign(positive for increase in memstore and negative for increase in block
 * cache). But rather than
 * simple sum it is calculated by giving more priority to the recent tuning
 * steps. When last few
 * tuner steps were NETURAL then we assume we are restarting the tuning process
 * and step size is
 * updated to maximum allowed size which can be specified in config by
 * <i>hbase.regionserver.heapmemory.autotuner.step.max</i>. If in a particular
 * tuning operation the
 * step direction is opposite to what indicated by
 * <i>decayingTunerStepSizeSum</i> we decrease the
 * step size by half. Step size does not change in other tuning operations. When
 * step size gets
 * below a certain threshold then the following tuner operations are considered
 * to be neutral. The
...
class DefaultHeapMemoryTuner implements HeapMemoryTuner {
    ...
    // If current block cache size or memstore size in use is below this level
    // relative to memory
    // provided to it then corresponding component will be considered to have
    // sufficient memory
    public static final float DEFAULT_SUFFICIENT_MEMORY_LEVEL_VALUE = 0.5f; // 50%
    // Number of tuner periods that will be considered while calculating mean and
    // deviation
    ...
    // If deviation of tuner step size gets below this value then it means past few
    // periods were
    ...
    // Set step size to max value for tuning, this step size will adjust dynamically
    // while tuning
    ...
    // It is not just arithmetic sum of past tuner periods. More priority is given
    // to recent
    // tuning steps.
    ...
    public TunerResult tune(TunerContext context) {
        ...
        // Adjusting step size for tuning to get to steady state or restart from steady
        // state.
        // Even if the step size was 4% and 32 GB memory size, we will be shifting 1 GB
        // back and forth
        // per tuner operation and it can affect the performance of cluster so we keep
        // on decreasing
        ...
        if (prevTuneDirection == StepDirection.NEUTRAL && newTuneDirection != StepDirection.NEUTRAL
            ...
            // The deviation cannot be that low if last period was neutral and some recent
            // periods were
            // not neutral.
            step = maximumStepSize;
        } else if ((newTuneDirection == StepDirection.INCREASE_MEMSTORE_SIZE && decayingTunerStepSizeSum < 0)
                || (newTuneDirection == StepDirection.INCREASE_BLOCK_CACHE_SIZE
                        && decayingTunerStepSizeSum > 0)) {
            // Current step is opposite of past tuner actions so decrease the step size to
            // reach steady
            // state.
        ...
        if (totalOnheapFlushCount == 0 && offheapMemstore
            ...
            // So don't do the memstore decrease equal to the step size. Instead do minimum
            // stepSize
            // decrease. But even if we have some flushes due to heap then it is better we
            // tune
            // the existing way.
            step = minimumStepSize;
        }
        // Increase / decrease the memstore / block cache sizes depending on new tuner
        // step.
        // We don't want to exert immediate pressure on memstore. So, we decrease its
        // size gracefully;
        // we set a minimum bar in the middle of the total memstore size and the lower
        // limit.
    ...
    private StepDirection getTuneDirection(TunerContext context) {
        ...
        if (earlyMemstoreSufficientCheck && earlyBlockCacheSufficientCheck) {
            // Both memstore and block cache memory seems to be sufficient. No operation
            // required.
            newTuneDirection = StepDirection.NEUTRAL;
            ...
            // Early checks for sufficient memory failed. Tuning memory based on past
            // statistics.
            // Boolean indicator to show if we need to revert previous step or not.
            boolean isReverting = false;
            switch (prevTuneDirection) {
                // Here we are using number of evictions rather than cache misses because it is
                // more
                ...
                case INCREASE_BLOCK_CACHE_SIZE:
                    if ((double) evictCount > rollingStatsForEvictions.getMean()
                        ...
                        // Tuning failed to decrease evictions or tuning resulted in large number of
                        // flushes.
                        newTuneDirection = StepDirection.INCREASE_MEMSTORE_SIZE;
                ...
                case INCREASE_MEMSTORE_SIZE:
                    if ((double) totalFlushCount > rollingStatsForFlushes.getMean()
                        ...
                        // Reverting previous step as it was not useful.
                        // Tuning failed to decrease flushes or tuning resulted in large number of
                        // evictions.
                        newTuneDirection = StepDirection.INCREASE_BLOCK_CACHE_SIZE;
                        tunerLog.append("We will revert previous tuning");
                        if ((double) totalFlushCount > rollingStatsForFlushes.getMean()) {
            ...
            // If we are not reverting. We try to tune memory sizes by looking at cache
            // misses / flushes.
            if (!isReverting) {
                // mean +- deviation*0.8 is considered to be normal
                // below it its consider low and above it is considered high.
                // We can safely assume that the number cache misses, flushes are normally
                // distributed over
                // past periods and hence on all the above mentioned classes (normal, high and
                // low)
                // are likely to occur with probability 56%, 22%, 22% respectively. Hence there
                // is at
                ...
                // dont find our steps helpful. Hence we want to do tuning only when we have
                // clear
                // indications because too many unnecessary tuning may affect the performance of
                // cluster.
        ...
        // Log NEUTRAL decisions at DEBUG, because they are the most frequent and not
        // that interesting.
        // Log other decisions at INFO because they are making meaningful operational
        // changes.
        switch (newTuneDirection) {
            case NEUTRAL:
