...
/**
 * Manages tuning of Heap memory using <code>HeapMemoryTuner</code>. Most part
...
public class HeapMemoryManager {
    ...
    private static final int CLUSTER_MINIMUM_MEMORY_THRESHOLD = (int) (CONVERT_TO_PERCENTAGE
    ...
    {
        // note that this initialization still isn't threadsafe, because updating a long
        // isn't atomic.
        long tempMaxHeap = -1L;
        try {
    ...
    /**
     * Returns heap occupancy percentage, 0 &lt;= n &lt;= 1. or -0.0 for error
     * asking JVM
     */
            ...
            if (result != null && result.needsTuning()) {
                ...
                if (CONVERT_TO_PERCENTAGE - (gml + bcul) < CLUSTER_MINIMUM_MEMORY_THRESHOLD) {
                    ...
                    for (HeapMemoryTuneObserver observer : tuneObservers) {
                        // Risky.. If this newMemstoreSize decreases we reduce the count in offheap
                        // chunk pool
        ...
        public void flushRequested(FlushType type, Region region) {
            switch (type) {
                ...
                case ABOVE_ONHEAP_LOWER_MARK:
                    ...
                    break;
                // Removed the counting of the offheap related flushes (after reviews). Will add
                // later if
                // needed
                default:
    ...
    /**
     * POJO to pass all the relevant information required to do the heap memory
     * tuning. It holds the
     * flush counts and block cache evictions happened within the interval. Also
     * holds the current
    ...
    /**
     * POJO which holds the result of memory tuning done by HeapMemoryTuner
     * implementation. It
    ...
    /**
     * Every class that wants to observe heap memory tune actions must implement
     * this interface.
     */
    public static interface HeapMemoryTuneObserver {

        /**
         * This method would be called by HeapMemoryManger when a heap memory tune
         * action took place.
         * 
