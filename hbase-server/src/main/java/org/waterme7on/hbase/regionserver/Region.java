...
import org.waterme7on.hbase.coprocessor.RegionCoprocessor;
 ...
 * {@link RegionCoprocessor
 * Coprocessors}. The operations include ability to do mutations, requesting
 * compaction, getting
 * different counters/sizes, locking rows and getting access to
 * {@linkplain Store}s.
 */
...
public interface Region extends ConfigurationObserver {
    ...
    /**
     * Check the region's underlying store files, open the files that have not been
     * opened yet, and
    ...
    /**
     * @return the max sequence id of flushed data on this region; no edit in memory
     *         will have a
     ...
     * major compacted.
     * 
     * @param majorCompactionOnly Only consider HFile that are the result of major
     *                            compaction
    ...
    /**
     * @return map of column family names to max sequence id that was read from
     *         storage when this
     *         region was opened
     */
    ...
    /**
     * @return The earliest time a store in the region was flushed. All other stores
     *         in the region
    ...
    /**
     * @return memstore size for this region, in bytes. It just accounts data size
     *         of cells added to
     *         the memstores of this Region. Means size in bytes for key, value and
     *         tags within Cells.
     *         It wont consider any java heap overhead for the cell objects or any
     *         other.
     */
    long getMemStoreDataSize();

    /**
     * @return memstore heap size for this region, in bytes. It accounts data size
     *         of cells added to
     *         the memstores of this Region, as well as java heap overhead for the
     *         cell objects or any
     *         other.
     */
    ...
    /**
     * @return memstore off-heap size for this region, in bytes. It accounts data
     *         size of cells added
     *         to the memstores of this Region, as well as overhead for the cell
    ...
    /**
     * Operation enum is used in {@link Region#startRegionOperation} and elsewhere
     * to provide context
     * for various checks.
     */
    enum Operation {
     ...
     * <p>
     * {@link #closeRegionOperation} MUST then always be called after the operation
     * has completed,
    ...
    /**
     * Row lock held by a given thread. One thread may acquire multiple locks on the
     * same row
     * simultaneously. The locks must be released by calling release() from the same
     * thread.
     */
    public interface RowLock {
        /**
         * Release the given lock. If there are no remaining locks held by the current
         * thread then
         * unlock the row and allow other threads to acquire the lock.
         * 
         * @throws IllegalArgumentException if called by a different thread than the
         *                                  lock owning thread
         */
        void release();
    }
    ...
    /**
     * Get a row lock for the specified row. All locks are reentrant. Before calling
     * this function
     ...
     */
    // TODO this needs to be exposed as we have RowProcessor now. If RowProcessor is
    // removed, we can
    // remove this too..
     ...
     * <p>
     * Please do not operate on a same column of a single row in a batch, we will
     * not consider the
     * previous operation in the same batch when performing the operations in the
     * batch.
     * 
     * @param mutations the list of mutations
     * @return an array of OperationStatus which internally contains the
     *         OperationStatusCode and the
     ...
     * and if it does, it
     * performs the mutation. If the passed value is null, the lack of column value
     * (ie:
     * non-existence) is used. See checkAndRowMutate to do many checkAndPuts at a
     * time on a single
     * row.
     * 
     * @param row        to check
    ...
    /**
     * Atomically checks if a row matches the filter and if it does, it performs the
     * mutation. See
    ...
    /**
     * Atomically checks if a row/family/qualifier value matches the expected values
     * and if it does,
     * it performs the row mutations. If the passed value is null, the lack of
     * column value (ie:
     * non-existence) is used. Use to do many mutations on a single row. Use
     * checkAndMutate to do one
    ...
    /**
     * Atomically checks if a row matches the conditions and if it does, it performs
     * the actions. Use
     * to do many mutations on a single row. Use checkAndMutate to do one
     * checkAndMutate at a time.
     * 
     ...
     * Return an iterator that scans over the HRegion, returning the indicated
     * columns and rows
     * specified by the {@link Scan}. The scanner will also include the additional
     * scanners passed
     * along with the scanners for the specified Scan instance. Should be careful
     * with the usage to
     ...
     * @throws IOException read exceptions
     */
    // RegionScanner getScanner(Scan scan, List<KeyValueScanner> additionalScanners)
    // throws IOException;
     ...
     * 
     * @param mutations  The list of mutations to perform. <code>mutations</code>
     *                   can contain
     *                   operations for multiple rows. Caller has to ensure that all
     *                   rows are
     ...
     *                   "more
     *                   idempotence") If multiple rows are locked care should be
     *                   taken that
     *                   <code>rowsToLock</code> is sorted in order to avoid
     *                   deadlocks.
     */
    // TODO Should not be exposing with params nonceGroup, nonce. Change when doing
    // the jira for
     ...
     * 
     * @param processor  The object defines the reads and writes to a row.
     * @param timeout    The timeout of the processor.process() execution Use a
     *                   negative number to
     ...
     * @return False when timeout elapsed but flushes are not over. True when
     *         flushes are over within
     *         max wait time period.
     */
    boolean waitForFlushes(long timeout);

    /**
     * @return a read only configuration of this region; throws
