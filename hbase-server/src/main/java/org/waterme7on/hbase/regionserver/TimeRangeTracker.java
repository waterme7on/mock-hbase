 ...
 * maximumTimestamp] in
 * interval notation. Use this class at write-time ONLY. Too much
 * synchronization to use at read
 * time Use {@link TimeRange} at read time instead of this. See toTimeRange() to
 * make TimeRange to
 * use. MemStores use this class to track minimum and maximum timestamps. The
 * TimeRangeTracker made
 * by the MemStore is passed to the StoreFile for it to write out as part a
 * flush in the the file
 * metadata. If no memstore involved -- i.e. a compaction -- then the StoreFile
 * will calculate its
 * own TimeRangeTracker as it appends. The StoreFile serialized TimeRangeTracker
 * is used at read
 * time via an instance of {@link TimeRange} to test if Cells fit the StoreFile
 * TimeRange.
 */
public abstract class TimeRangeTracker {
     ...
     * <code>cell</code>. If the Key
     * is of type DeleteColumn or DeleteFamily, it includes the entire time range
     * from 0 to timestamp
     * of the key.
     * 
    ...
    void includeTimestamp(final long timestamp) {
        ...
        if (timestamp < initialMinTimestamp) {
            ...
            // 1). timestamp >= curMinTimestamp, someone already sets the minimumTimestamp.
            // In this case,
            // it still needs to check if initialMinTimestamp == INITIAL_MIN_TIMESTAMP to
            // see
            ...
            // In this case,it still needs to check if initialMinTimestamp ==
            // INITIAL_MIN_TIMESTAMP
            // to see if it needs to set maximumTimestamp.
            if (initialMinTimestamp != INITIAL_MIN_TIMESTAMP) {
                // Someone already sets minimumTimestamp and timestamp is less than
                // minimumTimestamp.
    ...
    /**
     * This method used to serialize TimeRangeTracker (TRT) by protobuf while this
     * breaks the forward
     * compatibility on HFile.(See HBASE-21008) In previous hbase version ( < 2.0.0
     * ) we use
     * DataOutput to serialize TRT, these old versions don't have capability to
     * deserialize TRT which
     * is serialized by protobuf. So we need to revert the change of serializing
     * TimeRangeTracker back
    ...
    TimeRange toTimeRange() {
        long min = getMin();
        long max = getMax();
        // Initial TimeRangeTracker timestamps are the opposite of what you want for a
        // TimeRange. Fix!
    ...
    // In order to estimate the heap size, this inner class need to be accessible to
    // TestHeapSize.
