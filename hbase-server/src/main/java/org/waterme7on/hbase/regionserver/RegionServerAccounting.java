 ...
 * the Region Server.
 * Currently, it keeps record the global memstore size and global memstore
...
public class RegionServerAccounting {
    ...
    public RegionServerAccounting(Configuration conf) {
        ...
        this.globalMemStoreLimitLowMarkPercent = MemorySizeUtil.getGlobalMemStoreHeapLowerMark(conf,
                this.memType == MemoryType.HEAP);
        // When off heap memstore in use we configure the global off heap space for
        // memstore as bytes
        // not as % of max memory size. In such case, the lower water mark should be
        // specified using the
        // key "hbase.regionserver.global.memstore.size.lower.limit" which says % of the
        // global upper
        // bound and defaults to 95%. In on heap case also specifying this way is ideal.
        // But in the past
        // we used to take lower bound also as the % of xmx (38% as default). For
        // backward compatibility
        // for this deprecated config,we will fall back to read that config when new one
        // is missing.
        // Only for on heap case, do this fallback mechanism. For off heap it makes no
        // sense.
        ...
        this.globalOnHeapMemstoreLimitLowMark = (long) (this.globalOnHeapMemstoreLimit
    ...
    public FlushType isAboveLowWaterMark() {
        ...
        if (memType == MemoryType.HEAP) {
            ...
            if (getGlobalMemStoreOffHeapSize() >= globalMemStoreLimitLowMark) {
                // Indicates that the offheap memstore's size is greater than the global
                // memstore
                // lower limit
     ...
     *         or equal to 0.0, and any value greater than 1.0 means we enter the
