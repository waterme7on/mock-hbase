 ...
 * <p>
 * The MemStoreLAB is basically a bump-the-pointer allocator that allocates big
 * (2MB) byte[] chunks
 ...
 * regionserver. By ensuring that
 * all Cells in a given memstore refer only to large chunks of contiguous
 ...
 * would provide a
 * performance improvement - probably would speed up the
 * Bytes.toLong/Bytes.toInt calls in KeyValue,
 * but some of those are cached anyway. The chunks created by this MemStoreLAB
 * can get pooled at
 * {@link ChunkCreator}. When the Chunk comes from pool, it can be either an on
 * heap or an off heap
 * backed chunk. The chunks, which this MemStoreLAB creates on its own (when no
 * chunk available from
 * pool), those will be always on heap backed.
 * <p>
 * NOTE:if user requested to work with MSLABs (whether on- or off-heap), in
 * {@link CompactingMemStore} ctor, the {@link CompactingMemStore#indexType}
 * could only be
 * {@link IndexType#CHUNK_MAP},that is to say the immutable segments using
 * MSLABs are going to use
...
public class MemStoreLABImpl implements MemStoreLAB {
    ...
    /**
     * Its initial value is 1, so it is one bigger than the current count of open
     * scanners which
    ...
    /**
     * When a cell's size is too big (bigger than maxAlloc), copyCellInto does not
     * allocate it on
     * MSLAB. Since the process of flattening to CellChunkMap assumes that all cells
     * are allocated on
     * MSLAB, during this process, the big cells are copied into MSLAB using this
     * method.
     */
    @Override
    ...
    /**
     * Mostly a duplicate of {@link #copyCellInto(Cell, int)}} done for perf sake.
     * It presumes
     * ByteBufferExtendedCell instead of Cell so we deal with a specific type rather
     * than the super
     * generic Cell. Removes instanceof checks. Shrinkage is enough to make this
     * inline where before
    ...
    private Cell copyBBECellInto(ByteBufferExtendedCell cell, int maxAlloc) {
        ...
        while (true) {
            ...
            // and so the current thread has to try again to make its chunk or grab the
            // chunk
    ...
    /**
     * Clone the passed cell by copying its data into the passed buf and create a
     * cell with a chunkid
     * out of it
     * 
    ...
    private static Cell copyToChunkCell(Cell cell, ByteBuffer buf, int offset, int len) {
        ...
        if (cell instanceof ExtendedCell) {
            ...
            // Normally all Cell impls within Server will be of type ExtendedCell. Just
            // considering the
            // other case also. The data fragments within Cell is copied into buf as in
            // KeyValue
    ...
    private static Cell createChunkCell(ByteBuffer buf, int offset, int len, int tagsLen,
        ...
        if (tagsLen == 0) {
            // When tagsLen is 0, make a NoTagsByteBufferKeyValue version. This is an
            // optimized class
    ...
    /**
     * Try to retire the current chunk if it is still <code>c</code>. Postcondition
     * is that
     * curChunk.get() != c
     * 
    ...
    /*
     * Returning a new pool chunk, without replacing current chunk, meaning
     * MSLABImpl does not make
     * the returned chunk as CurChunk. The space on this chunk will be allocated
     * externally. The
    ...
    /*
     * Returning a new chunk, without replacing current chunk, meaning MSLABImpl
     * does not make the
     * returned chunk as CurChunk. The space on this chunk will be allocated
     * externally. The interface
     * is only for external callers. Chunks from pools are not allocated from here,
     * since they have
     * fixed sizes
     */
    @Override
