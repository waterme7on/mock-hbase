 ...
 * what files to return for scan, how to determine split point, and such). Does
 * NOT affect the
 * physical structure of files in HDFS. Example alternative structures - the
 * default list of files
 * by seqNum; levelDB one sorted by level and seqNum.
 * <p/>
 * Notice that, all the states are only in memory, we do not persist anything
 * here. The only place
 * where we throw an {@link IOException} is the {@link #getSplitPoint()} method,
 * where we need to
...
public interface StoreFileManager {
    ...
    /**
     * Clears all the compacted files and returns them. This method is expected to
     * be accessed single
     * threaded.
     * 
    ...
    /**
     * Gets the snapshot of the store files currently in use. Can be used for things
     * like metrics and
     * checks; should not assume anything about relations between store files in the
     * list.
     * 
     ...
     * because further
     * new reads will be using only the newly created files out of compaction. These
     * compacted files
     * will be deleted/cleared once all the existing readers on these compacted
     * files are done.
     * 
     ...
     * @return The files that may have the key less than or equal to targetKey, in
     *         reverse order of
    ...
    /**
     * Updates the candidate list for finding row key before. Based on the list of
     * candidates
     * remaining to check from getCandidateFilesForRowKeyBefore, targetKey and
     * current candidate, may
     * trim and reorder the list to remove the files where a better candidate cannot
     * be found.
     * 
     * @param candidateFiles The candidate files not yet checked for better
     *                       candidates - return value
     *                       from
     *                       {@link #getCandidateFilesForRowKeyBefore(KeyValue)},
     *                       with some files
     *                       already removed.
     ...
     * @return The files which don't have any necessary data according to TTL and
     *         other criteria.
     */
