...
import org.waterme7on.hbase.regionserver.StoreUtils;
...
public class DefaultStoreFileManager implements StoreFileManager {
    ...
    public void addCompactionResults(Collection<HStoreFile> newCompactedfiles,
        ...
        // Mark the files as compactedAway once the storefiles and compactedfiles list
        // is finalized
        // Let a background thread close the actual reader on these compacted files and
        // also
    ...
    public void removeCompactedFiles(Collection<HStoreFile> removedCompactedfiles) {
        this.compactedfiles = this.compactedfiles.stream().filter(sf -> !removedCompactedfiles.contains(sf))
    ...
    public Iterator<HStoreFile> updateCandidateFilesForRowKeyBefore(
        ...
        // Level store can trim the list by range, removing all the files which cannot
        // have
    ...
    public Collection<HStoreFile> getUnneededFiles(long maxTs, List<HStoreFile> filesCompacting) {
        ...
        // 2) Files that are not the latest can't become one due to (1), so the rest are
        // fair game.
