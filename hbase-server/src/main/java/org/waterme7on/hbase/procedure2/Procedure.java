...
/**
 * Base Procedure class responsible for Procedure Metadata; e.g. state,
 * submittedTime, lastUpdate,
 * stack-indexes, etc.
 * <p/>
 * Procedures are run by a {@link ProcedureExecutor} instance. They are
 * submitted and then the
 * ProcedureExecutor keeps calling {@link #execute(Object)} until the Procedure
 * is done. Execute may
 ...
 * <ul>
 * <li>{@link #isFailed()} A procedure has executed at least once and has
 * failed. The procedure may
 * or may not have rolled back yet. Any procedure in FAILED state will be
 * eventually moved to
 * ROLLEDBACK state.</li>
 * <li>{@link #isSuccess()} A procedure is completed successfully without
 * exception.</li>
 * <li>{@link #isFinished()} As a procedure in FAILED state will be tried
 ...
 * ({@link ProcedureState#WAITING},
 * {@link ProcedureState#WAITING_TIMEOUT}).</li>
 * </ul>
 ...
 * {@link #hasLock()}. The lock implementation is up to the implementor. If an
 * entity needs to be
 ...
 * <ul>
 * <li>Make {@link #hasLock()} method final, and add a {@link #locked} field in
 * Procedure to record
 * whether we have the lock. We will set it to {@code true} in
 * {@link #doAcquireLock(Object, ProcedureStore)} and to {@code false} in
 * {@link #doReleaseLock(Object, ProcedureStore)}. The sub classes do not need
 * to manage it any
 * more.</li>
 * <li>Also added a locked field in the proto message. When storing, the field
 * will be set according
 * to the return value of {@link #hasLock()}. And when loading, there is a new
 * field in Procedure
 * called {@link #lockedWhenLoading}. We will set it to {@code true} if the
 * locked field in proto
 * message is {@code true}.</li>
 * <li>The reason why we can not set the {@link #locked} field directly to
 * {@code true} by calling
 * {@link #doAcquireLock(Object, ProcedureStore)} is that, during
 * initialization, most procedures
 * need to wait until master is initialized. So the solution here is that, we
 * introduced a new
 * method called {@link #waitInitialized(Object)} in Procedure, and move the
 * wait master initialized
 * related code from {@link #acquireLock(Object)} to this method. And we added a
 * restoreLock method
 * to Procedure, if {@link #lockedWhenLoading} is {@code true}, we will call the
 * {@link #acquireLock(Object)} to get the lock, but do not set {@link #locked}
 * to true. And later
 * when we call {@link #doAcquireLock(Object, ProcedureStore)} and pass the
 * {@link #waitInitialized(Object)} check, we will test
 * {@link #lockedWhenLoading}, if it is
 * {@code true}, when we just set the {@link #locked} field to true and return,
 * without actually
 * calling the {@link #acquireLock(Object)} method since we have already called
 * it once.</li>
 * </ul>
 * <p/>
 ...
 * finish. See
 * {@link #updateMetricsOnSubmit(Object)} and
...
public abstract class Procedure<TEnvironment> implements Comparable<Procedure<TEnvironment>> {
    ...
    /**
     * Used for override complete of the procedure without actually doing any logic
     * in the procedure.
     * If bypass is set to true, when executing it will return null when
     * {@link #doExecute(Object)} is
     * called to finish the procedure and release any locks it may currently hold.
     * The bypass does
     * cleanup around the Procedure as far as the Procedure framework is concerned.
     * It does not clean
     * any internal state that the Procedure's themselves may have set. That is for
     * the Procedures to
     * do themselves when bypass is called. They should override bypass and do their
     * cleanup in the
     ...
     * a rollback. And
     * since the {@link #abort(Object)} method is overrideable Some procedures may
     * have chosen to
     * ignore the aborting.
     */
    ...
    /**
     * Indicate whether we need to persist the procedure to ProcedureStore after
     * execution. Default to
     * true, and the implementation can all {@link #skipPersistence()} to let the
     * framework skip the
     * persistence of the procedure.
     * <p/>
     * This is useful when the procedure is in error and you want to retry later.
     * The retry interval
     * and the number of retries are usually not critical so skip the persistence
     * can save some
     * resources, and also speed up the restart processing.
     * <p/>
     * Notice that this value will be reset to true every time before execution. And
     * when rolling back
     ...
     * {@link ProcedureExecutor#bypassProcedure(long, long, boolean, boolean)} for
     * now. DO NOT use
     * this method alone, since we can't just bypass one single procedure. We need
     * to bypass its
     * ancestor too. If your Procedure has set state, it needs to undo it in here.
     * 
     * @param env Current environment. May be null because of context; e.g.
     *            pretty-printing procedure
     *            WALs where there is no 'environment' (and where Procedures that
     *            require an
    ...
    /**
     * The main code of the procedure. It must be idempotent since execute() may be
     * called multiple
     * times in case of machine failure in the middle of the execution.
     * 
     * @param env the environment passed to the ProcedureExecutor
     * @return a set of sub-procedures to run or ourselves if there is more work to
     *         do or null if the
     *         procedure is done.
     ...
     *                                     queue and retried
     *                                     later.
     * @throws ProcedureSuspendedException Signal to the executor that Procedure has
     *                                     suspended itself
     *                                     and has set itself up waiting for an
     *                                     external event to wake
     *                                     it back up again.
     */
    ...
    /**
     * The code to undo what was done by the execute() code. It is called when the
     * procedure or one of
     * the sub-procedures failed or an abort was requested. It should cleanup all
     * the resources
     * created by the execute() call. The implementation must be idempotent since
     * rollback() may be
     * called multiple time in case of machine failure in the middle of the
     * execution.
     * 
     * @param env the environment passed to the ProcedureExecutor
     * @throws IOException          temporary failure, the rollback will retry later
     * @throws InterruptedException the procedure will be added back to the queue
     *                              and retried later
     */
     ...
     * with it, if they
     * want to be abortable. The simplest implementation is to have an AtomicBoolean
     * set in the
     * abort() method and then the execute() will check if the abort flag is set or
     * not. abort() may
     * be called multiple times from the client, so the implementation must be
     * idempotent.
     * <p>
     * NOTE: abort() is not like Thread.interrupt(). It is just a notification that
     * allows the
    ...
    /**
     * The {@link #doAcquireLock(Object, ProcedureStore)} will be split into two
     * steps, first, it will
     ...
     * for all the
     * procedures to not break the semantic if {@link #holdLock(Object)} is true.
     * But the
     * {@link ProcedureExecutor} will be started before the master finish
     * initialization(as it is part
     ...
     * will be dead lock.
     * 
     * @return true means we need to wait until the environment has been
     *         initialized, otherwise true.
     */
    ...
    /**
     * The user should override this method if they need a lock on an Entity. A lock
     * can be anything,
     * and it is up to the implementor. The Procedure Framework will call this
     * method just before it
     * invokes {@link #execute(Object)}. It calls {@link #releaseLock(Object)} after
     * the call to
     * execute.
     * <p/>
     * If you need to hold the lock for the life of the Procedure -- i.e. you do not
     * want any other
     * Procedure interfering while this Procedure is running, see
    ...
    /**
     * Used to keep the procedure lock even when the procedure is yielding or
     * suspended.
     * 
     * @return true if the procedure should hold on the lock until
     *         completionCleanup()
     */
    ...
    /**
     * This is used in conjunction with {@link #holdLock(Object)}. If
     * {@link #holdLock(Object)}
     * returns true, the procedure executor will call acquireLock() once and
     * thereafter not call
     * {@link #releaseLock(Object)} until the Procedure is done (Normally, it calls
     * release/acquire
    ...
    /**
     * Called when the procedure is loaded for replay. The procedure implementor may
     * use this method
     * to perform some quick operation before replay. e.g. failing the procedure if
     * the state on
    ...
    /**
     * Called when the procedure is ready to be added to the queue after the
     * loading/replay operation.
     */
    ...
    /**
     * Called when the procedure is marked as completed (success or rollback). The
     * procedure
     * implementor may use this method to cleanup in-memory states. This operation
     * will not be retried
     * on failure. If a procedure took a lock, it will have been released when this
     * method runs.
     */
    ...
    /**
     * By default, the procedure framework/executor will try to run procedures start
     * to finish. Return
     * true to make the executor yield between each execution step to give other
     * procedures a chance
     * to run.
     * 
     * @param env the environment passed to the ProcedureExecutor
     * @return Return true if the executor should yield on completion of an
     *         execution step. Defaults
     *         to return false.
     */
    ...
    /**
     * By default, the executor will keep the procedure result around util the
     * eviction TTL is
     * expired. The client can cut down the waiting time by requesting that the
     * result is removed from
     * the executor. In case of system started procedure, we can force the executor
     * to auto-ack.
     * 
     * @param env the environment passed to the ProcedureExecutor
    ...
    /**
     * Override this method to provide procedure specific counters for submitted
     * count, failed count
     * and time histogram.
     * 
     ...
     * implementation
     * updates submitted counter if {@link #getProcedureMetrics(Object)} returns
     * non-null
     ...
     * to update metrics at the end of the procedure. If
     * {@link #getProcedureMetrics(Object)} returns
     * non-null {@link ProcedureMetrics}, the default implementation adds runtime of
     * a procedure to a
    ...
    protected StringBuilder toStringSimpleSB() {
        ...
        /*
         * TODO Enable later when this is being used. Currently owner not used. if
         * (hasOwner()) {
    ...
    /**
     * Extend the toString() information with the procedure details e.g. className
     * and parameters
     * 
    ...
    /**
     * Called on store load to initialize the Procedure internals after the
    ...
    /**
     * Timeout of the next timeout. Called by the ProcedureExecutor if the procedure
     * has timeout set
    ...
    // ==========================================================================
    /**
     * Returns the time elapsed between the last update and the start time of the
     * procedure.
     */
    ...
    /**
     * Will only be called when loading procedures from procedure store, where we
     * need to record
     * whether the procedure has already held a lock. Later we will call
     * {@link #restoreLock(Object)}
    ...
    /**
     * Can only be called when restarting, before the procedure actually being
     * executed, as after we
     * actually call the {@link #doAcquireLock(Object, ProcedureStore)} method, we
     * will reset
    ...
    // just because the procedure can get scheduled on different executor threads on
    // each step.
    // ==============================================================================================
    ...
    /**
     * Returns true if the procedure is waiting for a child to finish or for an
     * external event.
     */
    ...
    protected void setAbortFailure(final String source, final String msg) {
        ...
        setFailure(source, new IOException(msg));
    }
     ...
     * {@link ProcedureSuspendedException} to halt the execution of the procedure,
     * and do not forget a
     * call {@link #setTimeout(int)} method to set the timeout. And you should also
     * override this
     ...
     * 
     * @return true to let the framework handle the timeout as abort, false in case
     *         the procedure
    ...
    protected synchronized boolean setTimeoutFailure(TEnvironment env) {
        if (state == ProcedureState.WAITING_TIMEOUT) {
            ...
            setFailure("ProcedureExecutor",
                    new TimeoutIOException("Operation timed out after " + timeDiff));
            return true;
        }
        return false;
    }
    ...
    /**
     * Called by the ProcedureExecutor to notify that one of the sub-procedures has
     * completed.
     */
    ...
    /**
     * Try to set this procedure into RUNNABLE state. Succeeds if all
     * subprocedures/children are done.
     * 
     * @return True if we were able to move procedure to RUNNABLE state.
     */
    synchronized boolean tryRunnable() {
        // Don't use isWaiting in the below; it returns true for WAITING and
        // WAITING_TIMEOUT
    ...
    /**
     * Called by the RootProcedureState on procedure execution. Each procedure store
     * its stack-index
     * positions.
     */
    ...
    /**
     * Internal method called by the ProcedureExecutor that starts the user-level
     * code execute().
     * 
     * @throws ProcedureSuspendedException This is used when procedure wants to halt
     *                                     processing and
     *                                     skip out without changing states or
     *                                     releasing any locks
     *                                     held.
     */
    ...
    final LockState doAcquireLock(TEnvironment env, ProcedureStore store) {
        ...
        if (lockedWhenLoading) {
            ...
            lockedWhenLoading = false;
            locked = true;
            // Here we return without persist the locked state, as lockedWhenLoading is true
            // means
            // that the locked field of the procedure stored in procedure store is true, so
            // we do not need
            // to store it again.
            return LockState.LOCK_ACQUIRED;
        }
        ...
        if (state == LockState.LOCK_ACQUIRED) {
            locked = true;
            // persist that we have held the lock. This must be done before we actually
            // execute the
            // procedure, otherwise when restarting, we may consider the procedure does not
            // have a lock,
            // but it may have already done some changes as we have already executed it, and
            // if another
            // procedure gets the lock, then the semantic will be broken if the holdLock is
            // true, as we do
    ...
    final void doReleaseLock(TEnvironment env, ProcedureStore store) {
        locked = false;
        // persist that we have released the lock. This must be done before we actually
        // release the
        // lock. Another procedure may take this lock immediately after we release the
        // lock, and if we
        // crash before persist the information that we have already released the lock,
        // then when
        // restarting there will be two procedures which both have the lock and cause
        // problems.
        if (getState() != ProcedureState.ROLLEDBACK) {
            // If the state is ROLLEDBACK, it means that we have already deleted the
            // procedure from
