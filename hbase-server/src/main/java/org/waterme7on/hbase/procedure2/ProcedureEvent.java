...
/**
 * Basic ProcedureEvent that contains an "object", which can be a description or
 * a reference to the
...
public class ProcedureEvent<T> {
    ...
    /**
     * Wakes up the suspended procedures by pushing them back into scheduler queues
     * and sets the event
     * as ready. See {@link #wakeInternal(AbstractProcedureScheduler)} for why this
     * is not
     * synchronized.
     */
    ...
    /**
     * Wakes up the suspended procedures only if the given {@code proc} is waiting
     * on this event.
     * <p/>
     * Mainly used by region assignment to reject stale
     * OpenRegionProcedure/CloseRegionProcedure. Use
     * with caution as it will cause performance issue if there are lots of
     * procedures waiting on the
     * event.
     */
    ...
    /**
     * Only to be used by ProcedureScheduler implementations. Reason: To wake up
     * multiple events,
     * locking sequence is schedLock --> synchronized (event) To wake up an event,
     * both schedLock()
     * and synchronized(event) are required. The order is schedLock() -->
     * synchronized(event) because
     * when waking up multiple events simultaneously, we keep the scheduler locked
     * until all
     * procedures suspended on these events have been added back to the queue (Maybe
     * it's not
     * required? Evaluate!) To avoid deadlocks, we want to keep the locking order
     * same even when
     * waking up single event. That's why, {@link #wake(AbstractProcedureScheduler)}
     * above uses the
     * same code path as used when waking up multiple events. Access should remain
     * package-private.
     */
    synchronized void wakeInternal(AbstractProcedureScheduler procedureScheduler) {
        ...
        // waitQueue' queue, so that the procedure which was added first goes in the
        // front for
        // the scheduler queue.
    ...
    /**
     * Access to suspendedProcedures is 'synchronized' on this object, but it's fine
     * to return it here
     * for tests.
     */
