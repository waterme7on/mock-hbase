 ...
 * Procedure". A "Root
 * Procedure" is a Procedure without parent, each subprocedure will be added to
 * the "Root Procedure"
 * stack (or rollback-stack). RootProcedureState is used and managed only by the
 * ProcedureExecutor.
...
class RootProcedureState<TEnvironment> {
    ...
    /**
     * Called by the ProcedureExecutor after the procedure step is completed, to add
     * the step to the
    ...
    /**
     * Called on store load by the ProcedureExecutor to load part of the stack. Each
     * procedure has its
     * own stack-positions. Which means we have to write to the store only the
     * Procedure we executed,
     * and nothing else. on load we recreate the full stack by aggregating each
     * procedure
     * stack-positions.
     */
    ...
    /**
     * Called on store load by the ProcedureExecutor to validate the procedure
     * stack.
     */
