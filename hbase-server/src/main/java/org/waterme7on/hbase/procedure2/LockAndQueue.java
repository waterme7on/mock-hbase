 ...
 * <li>Maintains a list of procedures waiting on this lock. {@link LockAndQueue}
 * extends
 * {@link ProcedureDeque} class. Blocked Procedures are added to our super
 * Deque. Using inheritance
...
public class LockAndQueue implements LockStatus {
    ...
    public boolean trySharedLock(Procedure<?> proc) {
        ...
        // If the parent proc or we have already held the xlock, then we return true
        // here as
    ...
    public boolean releaseSharedLock() {
        // hasExclusiveLock could be true, it usually means we acquire shared lock while
        // we or our
        // parent have held the xlock. And since there is still an exclusive lock, we do
        // not need to
        // wake any procedures.
    ...
    public boolean releaseExclusiveLock(Procedure<?> proc) {
        if (exclusiveLockOwnerProcedure == null
            ...
            // We are not the lock owner, it is probably inherited from the parent
            // procedures.
            return false;
        }
        exclusiveLockOwnerProcedure = null;
        // This maybe a bit strange so let me explain. We allow acquiring shared lock
        // while the parent
        // proc or we have already held the xlock, and also allow releasing the locks in
        // any order, so
        // it could happen that the xlock is released but there are still some procs
        // holding the shared
        // lock.
        // In HBase, this could happen when a proc which holdLock is false and schedules
        // sub procs which
        // acquire the shared lock on the same lock. This is because we will schedule
        // the sub proces
        // before releasing the lock, so the sub procs could call acquire lock before we
        // releasing the
        // xlock.
        return sharedLock == 0;
    }
    ...
    public int wakeWaitingProcedures(ProcedureScheduler scheduler) {
        ...
        // waitQueue' queue, so
        // that the procedure which was added first goes in the front for the scheduler
        // queue.
