 ...
 * important as for
 * executing remote procedures, we have some nonce checks at region server side
 * to prevent executing
 * non-idempotent operations more than once. If the procedure id could go back,
 * then we may
 * accidentally ignore some important operations such as region assign or
 * unassign.<br/>
 * This may lead to some garbages so we provide a {@link #cleanup()} method, the
 * framework will call
 * this method periodically and the store implementation could do some clean up
 * works in this
 * method.
 */
public interface ProcedureStore {
    ...
    public interface ProcedureStoreListener {
        ...
        /**
         * triggered when the store is not able to write out data. the main process
         * should abort.
         */
         ...
         * <p/>
         * For a WAL based ProcedureStore implementation, if all the procedures stored
         * in a WAL file
         * have been deleted, or updated later in another WAL file, then we can delete
         * the WAL file. If
         * there are old procedures in a WAL file which are never deleted or updated,
         * then we can not
         * delete the WAL file and this will cause we hold lots of WAL file and slow
         * down the master
         * restarts. So here we introduce this method to tell the upper layer that
         * please update the
    ...
    public interface ProcedureIterator {
        ...
        /**
         * Returns true if the iterator has more elements. (In other words, returns true
         * if next() would
        ...
        /**
         * Calling this method does not need to convert the protobuf message to the
         * Procedure class, so
         * if it returns true we can call {@link #skipNext()} to skip the procedure
         * without
         ...
         * 
         * @throws IOException if there was an error fetching/deserializing the
         *                     procedure
    ...
    /**
     * Interface passed to the ProcedureStore.load() method to handle the store-load
     * events.
     */
    public interface ProcedureLoader {
        /**
         * Called by ProcedureStore.load() to notify about the maximum proc-id in the
         * store.
         * 
        ...
        /**
         * Called by the ProcedureStore.load() every time a set of procedures are ready
         * to be executed.
         * The ProcedureIterator passed to the method, has the procedure sorted in
         * replay-order.
         * 
         * @param procIter iterator over the procedures ready to be added to the
         *                 executor.
         */
        void load(ProcedureIterator procIter) throws IOException;

        /**
         * Called by the ProcedureStore.load() in case we have procedures not-ready to
         * be added to the
         * executor, which probably means they are corrupted since some information/link
         * is missing.
         * 
         * @param procIter iterator over the procedures not ready to be added to the
         *                 executor, corrupted
         */
     ...
     * @return true if the listner was in the list and it was removed, otherwise
     *         false.
     */
    ...
    /**
     * Set the number of procedure running. This can be used, for example, by the
     * store to know how
     * long to wait before a sync.
     * 
     * @return how many procedures are running (may not be same as
     *         <code>count</code>).
     */
     ...
     *             {@link org.apache.hadoop.hbase.procedure2.store.wal.WALProcedureStore}.
     *             As now we
     *             will store the procedure data in a master local region, and
     *             master itself will deal
    ...
    /**
     * When a procedure is submitted to the executor insert(proc, null) will be
     * called. 'proc' has a
     * 'RUNNABLE' state and the initial information required to start up. When a
     * procedure is executed
     * and it returns children insert(proc, subprocs) will be called. 'proc' has a
     * 'WAITING' state and
     * an update state. 'subprocs' are the children in 'RUNNABLE' state with the
     * initial information.
     * 
    ...
    /**
     * Serialize a set of new procedures. These procedures are freshly submitted to
     * the executor and
     * each procedure has a 'RUNNABLE' state and the initial information required to
     * start up.
     * 
    ...
    /**
     * The specified procedure was executed, and the new state should be written to
     * the store.
     * 
     * @param proc the procedure to serialize and write to the store.
     */
    ...
    /**
     * The specified procId was removed from the executor, due to completion, abort
     * or failure. The
     * store implementor should remove all the information about the specified
     * procId.
     * 
    ...
    /**
     * The parent procedure completed. Update the state and mark all the child
     * deleted.
     * 
    ...
    /**
     * The specified procIds were removed from the executor, due to completion,
     * abort or failure. The
     * store implementor should remove all the information about the specified
     * procIds.
     * 
    ...
    /**
     * Will be called by the framework to give the store a chance to do some clean
     * up works.
     * <p/>
     * Notice that this is for periodical clean up work, not for the clean up after
     * close, if you want
