 ...
 * the underlying
 * collection so we can support RandomAccess. All mutations create a new copy of
 * the
 * <code>ArrayList</code> instance, so can be expensive. This class is only
 * intended for use on
 * small, very rarely written collections that expect highly concurrent reads.
 * <p>
 * Read operations are performed on a reference to the internal list at the time
 * of invocation, so
 * will not see any mutations to the collection during their operation.
 * Iterating over list elements
 * manually using the RandomAccess pattern involves multiple operations. For
 * this to be safe get a
...
public class SortedList<E> implements List<E>, RandomAccess {
    ...
    /**
     * Returns a reference to the unmodifiable list currently backing the
     * SortedList. Changes to the
     * SortedList will not be reflected in this list. Use this method to get a
