...
public class NettyEventLoopGroupConfig {
    ...
    private static boolean useEpoll(Configuration conf) {
        ...
        final boolean epollEnabled = conf.getBoolean(NETTY_NATIVETRANSPORT_KEY, DEFAULT_NETTY_NATIVETRANSPORT);
        // Use the faster native epoll transport mechanism on linux if enabled and the
        // hardware architecture is either amd64 or aarch64. Netty is known to have
        // native
    ...
    public NettyEventLoopGroupConfig(Configuration conf, String threadPoolName) {
        ...
        ThreadFactory eventLoopThreadFactory = new DefaultThreadFactory(threadPoolName, true, Thread.MAX_PRIORITY);
        // // commented out for now, as we don't have epoll support in aarch64
        // // environment / or haven't fix this problem
        // if (useEpoll) {
