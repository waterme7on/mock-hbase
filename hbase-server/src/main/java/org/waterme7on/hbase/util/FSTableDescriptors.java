...
import org.waterme7on.hbase.coprocessor.MultiRowMutationEndpoint;
 ...
 * expects descriptors to be in a file in the {@link #TABLEINFO_DIR} subdir of
 * the table's directory
 * in FS. Can be read-only -- i.e. does not modify the filesystem or can be read
 * and write.
 * <p>
 * Also has utility for keeping up the table descriptors tableinfo file. The
 * table schema file is
 * kept in the {@link #TABLEINFO_DIR} subdir of the table directory in the
 * filesystem. It has a
 * {@link #TABLEINFO_FILE_PREFIX} and then a suffix that is the edit sequenceid:
 * e.g.
 * <code>.tableinfo.0000000003</code>. This sequenceid is always increasing. It
 * starts at zero. The
 * table schema file with the highest sequenceid has the most recent schema
 * edit. Usually there is
 * one file only, the most recent but there may be short periods where there are
 * more than one file.
 * Old files are eventually cleaned. Presumption is that there will not be lots
 * of concurrent
 * clients making table schema edits. If so, the below needs a bit of a
 * reworking and perhaps some
...
public class FSTableDescriptors implements TableDescriptors {
     ...
     * conf and the
     * filesystem where that root dir lives. This instance can do write operations
     * (is not read only).
     */
    ...
    public static TableDescriptor tryUpdateAndGetMetaTableDescriptor(Configuration conf,
        ...
        // TableDescriptor td = StoreFileTrackerFactory.updateWithTrackerConfigs(conf,
        // builder.build());
    ...
    public static TableDescriptorBuilder createMetaTableDescriptorBuilder(final Configuration conf)
            throws IOException {
        // TODO We used to set CacheDataInL1 for META table. When we have BucketCache in
        // file mode, now
     ...
     * <p/>
     * Uses a local cache of the descriptor but still checks the filesystem on each
     * call if
     * {@link #fsvisited} is not {@code true}, i.e, we haven't done a full scan yet,
     * to see if a newer
    ...
    public void update(TableDescriptor td, boolean cacheOnly) throws IOException {
        // TODO: in fact this method will only be called at master side, so fsreadonly
        // and usecache will
        // always be true. In general, we'd better have a ReadOnlyFSTableDesciptors for
        // HRegionServer
        // but now, HMaster extends HRegionServer, so unless making use of generic, we
        // can not have
        // different implementations for HMaster and HRegionServer. Revisit this when we
        // make HMaster
    ...
    /**
     * Removes the table descriptor from the local cache and returns it. If not in
     * read only mode, it
     ...
     * @return Returns zero-prefixed decimal version of passed number (Does absolute
     *         in case number is
     *         negative).
     */
    ...
    /**
     * Attempts to write a new table descriptor to the given table's directory. It
     * begins at the
     * currentSequenceId + 1 and tries 10 times to find a new sequence number not
     * already in use.
     * <p/>
    ...
    private static Path writeTableDescriptor(final FileSystem fs, final TableDescriptor td,
        ...
        // Here we will write to the final directory directly to avoid renaming as on
        // OSS renaming is
        // not atomic and has performance issue. The reason why we could do this is
        // that, in the below
        // code we will not overwrite existing files, we will write a new file instead.
        // And when
        // loading, we will skip the half written file, please see the code in
        // getTableDescriptorFromFs
        Path tableInfoDir = new Path(tableDir, TABLEINFO_DIR);

        // In proc v2 we have table lock so typically, there will be no concurrent
        // writes. Keep the
        // retry logic here since we may still want to write the table descriptor from
        // for example,
        // HBCK2?
    ...
    /**
     * Create new TableDescriptor in HDFS. Happens when we are creating table. Used
     * by tests.
     * 
     ...
     * new table during cluster start or in Clone and Create Table Procedures.
     ...
     * @param forceCreation if <tt>true</tt>,then even if previous table descriptor
     *                      is present it will
     *                      be overwritten
     * @return <tt>true</tt> if the we successfully created the file, <tt>false</tt>
     *         if the file
     ...
     * Create a new TableDescriptor in HDFS in the specified table directory.
     * Happens when we create a
     * new table snapshoting. Does not enforce read-only. That is for caller to
     * determine.
     * 
