 ...
 * <p>
 * Event types are named by a convention: event type names specify the component
 * from which the
 * event originated and then where its destined -- e.g. RS_ZK_ prefix means the
 * event came from a
 * regionserver destined for zookeeper -- and then what the even is; e.g.
 * REGION_OPENING.
 * <p>
 * We give the enums indices so we can add types later and keep them grouped
 * together rather than
 * have to add them always to the end as we would have to if we used raw enum
 * ordinals.
 */
public enum EventType {
    ...
    // RS_ZK_REGION_CLOSING (1), // It is replaced by
    // M_ZK_REGION_CLOSING(HBASE-4739)
     ...
     * RS has requested to split a region. This is to notify master and check with
     * master if the
     ...
     * RS has requested to merge two regions. This is to notify master and check
     * with master if two
    ...
    /**
     * Master controlled events to be executed on the master M_SERVER_SHUTDOWN
     * Master is processing
     * shutdown of a RS
     */
