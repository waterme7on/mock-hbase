 ...
 * the
 * {@link #process()} and {@link #prepare()} methods. Subclasses should also do
 * all necessary checks
 * up in their prepare() if possible -- check table exists, is disabled, etc. --
 * so they fail fast
 * rather than later when process is running. Do it this way because process be
 * invoked directly but
 * event handlers are also run in an executor context -- i.e. asynchronously --
 * and in this case,
 ...
 * handler event types.
 * We need to keep a full list in one place -- and as enums is a good shorthand
 * for an
 * implemenations -- because event handlers can be passed to executors when they
 * are to be run
 * asynchronously. The hbase executor, see ExecutorService, has a switch for
 * passing event type to
 * executor.
 * <p>
 * 
 * @see ExecutorService
...
public abstract class EventHandler implements Runnable, Comparable<EventHandler> {
     ...
     * in the
     * constructor, or in process()) so that the caller, which is mostly executed in
     * the ipc context
     * can fail fast. Process is executed async from the client ipc, so this method
     * gives a quick
     * chance to do some basic checks. Should be called after constructing the
     * EventHandler, and
     * before process().
     * 
    ...
    /**
     * Get the priority level for this handler instance. This uses natural ordering
     * so lower numbers
     * are higher priority.
     * <p>
     ...
     * name about what
     * event they are handling. For example, event-specific information such as
     * which region or server
