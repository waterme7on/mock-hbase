...
import org.waterme7on.hbase.monitoring.ThreadMonitoring;
 ...
 * queue to which
 * {@link EventType}s can be submitted, and a <code>Runnable</code> that handles
 * the object that is
 * added to the queue.
 * <p>
 * In order to create a new service, create an instance of this class and then
 * do:
 * <code>instance.startExecutorService(executorConfig);</code>.
 * {@link ExecutorConfig} wraps the
...
public class ExecutorService {
    ...
    /**
     * Initialize the executor lazily, Note if an executor need to be initialized
     * lazily, then all
     * paths should use this method to get the executor, should not start executor
     * by using
    ...
    public class ExecutorConfig {
        ...
        /**
         * Allows timing out of core threads. Good to set this for non-critical thread
         * pools for release
         * of unused resources. Refer to
         * {@link ThreadPoolExecutor#allowCoreThreadTimeOut} for
         * additional details.
         */
        ...
        /**
         * Returns the executor name inferred from the type and the servername on which
         * this is running.
         */
    ...
    static class Executor {
        ...
        protected Executor(ExecutorConfig config) {
            ...
            this.threadPoolExecutor = new TrackingThreadPoolExecutor(
                    // setting maxPoolSize > corePoolSize has no effect since we use an unbounded
                    // task queue.
    ...
    static class TrackingThreadPoolExecutor extends ThreadPoolExecutor {
        ...
        /**
         * @return a map of the threads currently running tasks inside this executor.
         *         Each key is an
         *         active thread, and the value is the task that is currently running.
         *         Note that this is
    ...
    /**
     * A snapshot of the status of a particular executor. This includes the contents
     * of the executor's
     * pending queue, as well as the threads and events currently being processed.
     * This is a
    ...
    /**
     * The status of a particular event that is in the middle of being handled by an
     * executor.
     */
