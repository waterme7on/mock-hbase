...
import org.waterme7on.hbase.master.procedure.ProcedurePrepareLatch;
import org.apache.hadoop.hbase.util.NonceKey;
import org.apache.yetus.audience.InterfaceAudience;

/**
 * View and edit the current cluster schema. Use this API making any
 * modification to namespaces,
 * tables, etc.
 * <h2>Implementation Notes</h2> Nonces are for when operation is non-idempotent
 * to ensure once-only
 * semantic, even across process failures.
 */
// ClusterSchema is introduced to encapsulate schema modification. Currently the
// different aspects
// are spread about the code base. This effort is about cleanup, shutting down
// access, and
// coalescing common code. In particular, we'd contain filesystem modification.
// Other
// benefits are to make all schema modification work the same way (one way to do
// an operation only
// rather than the current approach where how an operation is done varies with
// context) and to make
// it so clusterschema modification can stand apart from Master to faciliate
// standalone
// testing. It is part of the filesystem refactor project that undoes the
// dependency on a
// layout in HDFS that mimics our model of tables have regions have column
// families have files.
// With this Interface in place, with all modifications going via this route
// where no filesystem
// particulars are exposed, redoing our internals will take less effort.
//
// Currently ClusterSchema Interface will include namespace and table
// manipulation. Ideally a
// form of this Interface will go all the ways down to the file manipulation
// level but currently
// TBD.
//
// ClusterSchema is private to the Master; only the Master knows current cluster
// state and has
// means of editing/altering it.
//
// TODO: Remove Server argument when MasterServices are passed.
// TODO: We return Future<ProcedureInfo> in the below from most methods. It may
// change to return
...
public interface ClusterSchema {
     ...
     */
    // TODO: Remove from here. Keep internal. This Interface is too high-level to
    // host this accessor.
     ...
     * @param nonceKey            A unique identifier for this operation from the
     *                            client or process.
     * @param latch               A latch to block on for precondition validation
     * @return procedure id
     * @throws IOException if service is not running see
     *                     {@link ServiceNotRunningException}
     */
     ...
     */
    // No Future here because presumption is that the request will go against cached
    // metadata so
