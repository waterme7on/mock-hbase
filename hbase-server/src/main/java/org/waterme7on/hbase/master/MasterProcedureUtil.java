...
import org.waterme7on.hbase.master.MasterServices;
import org.waterme7on.hbase.master.procedure.ServerProcedureInterface;
import org.waterme7on.hbase.procedure2.Procedure;
...
public final class MasterProcedureUtil {
     ...
     * NonceProcedureRunnable.run() will be called only if no one else registered
     * the nonce. any
     * Exception thrown by the run() method will be collected/handled and rethrown.
     * <code>
    ...
    public static long submitProcedure(final NonceProcedureRunnable runnable) throws IOException {
        ...
        // if (procId >= 0)
        //     return procId; // someone already registered the nonce
        // try {
        //     runnable.run();
        // } catch (IOException e) {
        //     throw e;
     ...
     * 
     * @deprecated Since 2.3.0, will be removed in 4.0.0. We do not use this style
     *             of procedure wal
     *             file name any more.
     */
    @Deprecated
    ...
    /**
     * Return the priority for the given table. Now meta table is 3, other system
     * tables are 2, and
     * user tables are 1.
     */
    ...
    /**
     * This is a version of unwrapRemoteIOException that can do DoNotRetryIOE. We
     * need to throw DNRIOE
     * to clients if a failed Procedure else they will keep trying. The default
     * proc.getException().unwrapRemoteException doesn't have access to DNRIOE from
     * the procedure2
     * module.
     */
    public static IOException unwrapRemoteIOException(Procedure proc) {
        ...
        return proc.getException().unwrapRemoteIOException();
    }
}
