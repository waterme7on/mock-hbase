...
import org.waterme7on.hbase.util.IdReadWriteLock;
...
public class TableStateManager {
    ...
    /**
     * Set this key to false in Configuration to disable migrating table state from
     * zookeeper so
     * hbase:meta table.
     */
    ...
    private void updateMetaState(TableName tableName, TableState.State newState) throws IOException {
        if (tableName.equals(TableName.META_TABLE_NAME)) {
            ...
            // Otherwise, just return; no need to set ENABLED on meta -- it is always
            // ENABLED.
            return;
        }
        boolean succ = false;
    ...
    private void fixTableStates(TableDescriptors tableDescriptors, Connection connection)
        ...
        for (TableDescriptor tableDesc : tableDescriptors.getAll().values()) {
            ...
            if (TableName.isMetaTableName(tableName)) {
                ...
                // Call through to fixTableState though in case a super class wants to do
                // something.
    ...
    /**
     * This code is for case where a hbase2 Master is starting for the first time.
     * ZooKeeper is where
     * we used to keep table state. On first startup, read zookeeper and update
     * hbase:meta with the
     * table states found in zookeeper. This is tricky as we'll do this check every
     * time we startup
     * until mirroring is disabled. See the {@link #MIGRATE_TABLE_STATE_FROM_ZK_KEY}
     * flag. Original
     * form of this migration came in with HBASE-13032. It deleted all znodes when
     * done. We can't do
    ...
    // ENABLING
    // // or
    // // DISABLING then we have a problem; we are starting up an hbase-2 on a
    // cluster
    // // with
    ...
    // "
    // + "ENABLED or DISABLED); NOT MIGRATING table state", entry.getKey(),
    // zkstate);
    // }
    // }
    // // What if the table states disagree? Defer to the hbase:meta setting rather
    // // than have the
