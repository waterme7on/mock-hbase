...
import org.waterme7on.hbase.ZKNamespaceManager;
...
/**
 * This is a helper class used internally to manage the namespace metadata that
 * is stored in
 * TableName.NAMESPACE_TABLE_NAME. It also mirrors updates to the ZK store by
 * forwarding updates to
 * {@link org.apache.hadoop.hbase.ZKNamespaceManager}. WARNING: Do not use. Go
 * via the higher-level
 * {@link ClusterSchema} API instead. This manager is likely to go aways
 * anyways.
 */
...
public class TableNamespaceManager implements Stoppable {
     ...
     * expires. Note,
     * by-passes notifying coprocessors and name checks. Use for system namespaces
     * only.
     */
    ...
    /**
     * This method checks if the namespace table is assigned and then tries to
     * create its Table
     * reference. If it was already created before, it also makes sure that the
     * connection isn't
     * closed.
     * 
    ...
    public synchronized boolean isTableAvailableAndInitialized() throws IOException {
        ...
        if (isTableAssigned() && isTableEnabled()) {
            try {
                ...
                if (!initGoodSofar) {
                    // some required namespace is created asynchronized. We should complete init
                    // later.
                    return false;
                }
