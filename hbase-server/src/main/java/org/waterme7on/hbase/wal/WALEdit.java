 ...
 * WALCellEncoder.
 * <p>
 * This class is LimitedPrivate for CPs to read-only. The {@link #add} methods
 * are classified as
 ...
 * events in the WAL
 * such as compaction, flush, or region open. These meta types do not traverse
 * hbase memstores. They
 * are edits made by the hbase system rather than edit data submitted by
 * clients. They only show in
 * the WAL. These 'Meta' types have not been formally specified (or made into an
 * explicit class
 * type). They evolved organically. HBASE-8457 suggests codifying a WALEdit
 * 'type' by adding a type
 * field to WALEdit that gets serialized into the WAL. TODO. Would have to work
 * on the
 * consumption-side. Reading WALs on replay we seem to consume a Cell-at-a-time
 * rather than by
 * WALEdit. We are already in the below going out of our way to figure
 * particular types -- e.g. if a
 * compaction, replay, or close meta Marker -- during normal processing so would
 * make sense to do
 * this. Current system is an awkward marking of Cell columnfamily as
 * {@link #METAFAMILY} and then
 * setting qualifier based off meta edit type. For replay-time where we read
 * Cell-at-a-time, there
 * are utility methods below for figuring meta type. See also
 * {@link #createBulkLoadEvent(RegionInfo, WALProtos.BulkLoadDescriptor)}, etc.,
 * for where we create
 * meta WALEdit instances.
 * </p>
 * <p>
 * WALEdit will accumulate a Set of all column family names referenced by the
 * Cells
 * {@link #add(Cell)}'d. This is an optimization. Usually when loading a
 * WALEdit, we have the column
 * family name to-hand.. just shove it into the WALEdit if available. Doing
 * this, we can save on a
 * parse of each Cell to figure column family down the line when we go to add
 * the WALEdit to the WAL
...
public class WALEdit implements HeapSize {
     ...
     * {@link #REGION_EVENT_PREFIX}
     * prefix ('HBASE::REGION_EVENT::'). After the prefix, we note the type of the
     * event which we get
     * from the RegionEventDescriptor protobuf instance type (A
     * RegionEventDescriptor protobuf
     * instance is written as the meta Marker Cell value). Adding a type suffix
     * means we do not have
     * to deserialize the protobuf to figure out what type of event this is.. .just
     * read the qualifier
     * suffix. For example, a close region event descriptor will have a qualifier of
     * HBASE::REGION_EVENT::REGION_CLOSE. See WAL.proto and the EventType in
     ...
     */
    private static final byte[] REGION_EVENT_CLOSE = createRegionEventDescriptorQualifier(
            RegionEventDescriptor.EventType.REGION_CLOSE);
    ...
    /**
     * All the Cell families in <code>cells</code>. Updated by {@link #add(Cell)}
     * and
     ...
     *             {@link #WALEdit(int, boolean)}
     *             instead.
     * @see #WALEdit(int, boolean)
     * @see <a href=
     *      "https://issues.apache.org/jira/browse/HBASE-20781">HBASE-20781</a>
     */
    @Deprecated
    ...
    /**
     * @return True when current WALEdit is created by log replay. Replication skips
     *         WALEdits from
     *         replay.
     */
    ...
    public WALEdit add(Cell cell) {
        // We clone Family each time we add a Cell. Expensive but safe. For CPU savings,
        // use
    ...
    /**
     * This is not thread safe. This will change the WALEdit and shouldn't be used
     * unless you are sure
    ...
    /**
     * @return Cell qualifier for the passed in RegionEventDescriptor Type; e.g.
     *         we'll return
     *         something like a byte array with HBASE::REGION_EVENT::REGION_OPEN in
     *         it.
     */
    ...
    /**
     * @return Returns a RegionEventDescriptor made by deserializing the content of
     *         the passed in
     *         <code>cell</code>, IFF the <code>cell</code> is a
     *         RegionEventDescriptor type WALEdit.
     */
    ...
    public static byte[] getRowForRegion(RegionInfo hri) {
        ...
        if (startKey.length == 0) {
            // empty row key is not allowed in mutations because it is both the start key
            // and the end key
            // we return the smallest byte[] that is bigger (in lex comparison) than
            // byte[0].
            return new byte[] { 0 };
        }
        return startKey;
     ...
     * 
     * @param hri                The RegionInfo for the region in which we are bulk
     *                           loading
    ...
    /**
     * Append the given map of family->edits to a WALEdit data structure. This does
     * not write to the
     * WAL itself. Note that as an optimization, we will stamp the Set of column
     * families into the
     * WALEdit to save on our having to calculate column families subsequently down
     * in the actual WAL
     * writing.
     * 
    ...
    public void add(Map<byte[], List<Cell>> familyMap) {
        for (Map.Entry<byte[], List<Cell>> e : familyMap.entrySet()) {
            // 'foreach' loop NOT used. See HBASE-12023 "...creates too many iterator
            // objects."
            int listSize = e.getValue().size();
            // Add all Cells first and then at end, add the family rather than call {@link
            // #add(Cell)}
