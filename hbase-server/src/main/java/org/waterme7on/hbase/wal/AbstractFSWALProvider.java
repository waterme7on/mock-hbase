 ...
 * writes to Hadoop FS. By
 * default, this implementation picks a directory in Hadoop FS based on a
 * combination of
 * <ul>
...
public abstract class AbstractFSWALProvider<T extends AbstractFSWAL<?>> implements WALProvider {
    ...
    public interface Reader extends org.apache.hadoop.hbase.wal.WAL.Reader {
        /**
         * @param fs   File system.
         * @param path Path.
         * @param c    Configuration.
         * @param s    Input stream that may have been pre-opened by the caller; may be
         *             null.
         */
    ...
    /**
     * We use walCreateLock to prevent wal recreation in different threads, and also
     * prevent getWALs
     ...
     * @param providerId differentiate between providers from one factory, used for
     *                   FS layout. may be
     *                   null
     */
    @Override
     ...
     * non-meta, count the
     * number of files (rolled and active). if either of them aren't, count 0 for
     * that provider.
     */
    @Override
     ...
     * iff the given WALFactory is using the DefaultWALProvider for meta and/or
     * non-meta, count the
     * size of files (only rolled). if either of them aren't, count 0 for that
     * provider.
     */
    @Override
     ...
     * {@link #validateWALFilename(String)} for
     * description.
     */
     ...
     * &lt;wal-name&gt;{@link #WAL_FILE_NAME_DELIMITER}
     * &lt;file-creation-timestamp&gt;[.meta]. provider-name is usually made up of a
     * server-name and a
     * provider-id
     * 
    ...
    /**
     * Construct the directory name for all WALs on a given server. Dir names
    ...
    /**
     * Construct the directory name for all old WALs on a given server. The default
     * old WALs dir looks
     * like: <code>hbase/oldWALs</code>. If you config
     * hbase.separate.oldlogdir.by.regionserver to
     * true, it looks like
     * <code>hbase//oldWALs/kalashnikov.att.net,61634,1486865297088</code>.
     * 
     * @param serverName Server name formatted as described in {@link ServerName}
    ...
    /**
     * Pulls a ServerName out of a Path generated according to our layout rules. In
     * the below layouts,
    ...
    /**
     * This function returns region server name from a log file name which is in one
     * of the following
     * formats:
     * <ul>
    ...
    /**
     * Comparator used to compare WAL files together based on their start time. Just
     * compares start
    ...
    public static class WALStartTimeComparator implements Comparator<Path> {
        ...
        /**
         * Split a path to get the start time For example:
         * 10.20.20.171%3A60020.1277499063250 Could also
         * be a meta WAL which adds a '.meta' suffix or a synchronous replication WAL
         * which adds a
     ...
     */
    public static org.apache.hadoop.hbase.wal.WAL.Reader openReader(Path path, Configuration conf) throws IOException {
        ...
        while (reader == null && attempt++ < maxAttempts) {
            try {
                ...
                // which throws a NPE if we open a file before any data node has the most recent
                // block
