...
/**
 * The Write Ahead Log (WAL) stores all durable edits to the HRegion. This
 * interface provides the
 * entry point for all WAL implementors.
 * <p>
 * See {@link FSHLogProvider} for an example implementation. A single
 * WALProvider will be used for
 * retrieving multiple WALs in a particular region server and must be
 * threadsafe.
 */
public interface WALProvider {
     ...
     * @param providerId differentiate between providers from one factory. may be
     *                   null
     */
    ...
    /**
     * persist outstanding WALs to storage and stop accepting new appends. This
     * method serves as
     * shorthand for sending a sync to every WAL provided by a given implementation.
     * Those WALs will
    ...
    /**
     * shutdown utstanding WALs and clean up any persisted state. Call this method
     * only when you will
     * not need to replay any of the edits to the WALs from this provider. After
     * this call completes,
     * the underlying resources should have been reclaimed.
     */
    void close() throws IOException;

    // Writers are used internally. Users outside of the WAL should be relying on
    // the
     ...
     * {@link #getWAL(RegionInfo)} as this method
     * will not effect the {@link WAL} which has already been created. And as long
     * as we can only it
