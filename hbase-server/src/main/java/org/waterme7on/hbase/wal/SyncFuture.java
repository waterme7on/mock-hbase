...
/**
 * A Future on a filesystem sync call. It given to a client or 'Handler' for it
 * to wait on till the
 * sync completes.
 * <p>
 ...
 * goes across a queue and is handled by a background thread; when it completes,
 * it finishes up the
 ...
 * single dfs sync call may complete and mark many SyncFutures as done; i.e. we
 * batch up sync calls
...
class SyncFuture {
    ...
    /**
     * The transaction id that was set in here when we were marked done. Should be
     * equal or > txnId.
    ...
    /**
     * Returns the thread that owned this sync future, use with caution as we return
     * the reference to
     ...
     * @param t    Can be null. Set if we are 'completing' on error (and this 't' is
     *             the error).
