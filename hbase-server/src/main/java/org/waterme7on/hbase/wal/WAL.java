...
/**
 * A Write Ahead Log (WAL) provides service for reading, writing waledits. This
 * interface provides
 * APIs for WAL users (such as RegionServer) to use the WAL (do append, sync,
 * etc). Note that some
 * internals, such as log rolling and performance evaluation tools, will use
 * WAL.equals to determine
...
public interface WAL extends Closeable, WALFileLengthProvider {
     ...
     * rollWriter running at any
     * given time.
     * 
     * @return If lots of logs, flush the stores of returned regions so next time
     *         through we can clean
     *         logs. Returns null if nothing to flush. Names are actual region names
     *         as returned by
     ...
     * The implementation is synchronized in order to make sure there's one
     * rollWriter running at any
     * given time. If true, force creation of a new writer even if no entries have
     * been written to the
     * current writer
     * 
     * @return If lots of logs, flush the stores of returned regions so next time
    ...
    /**
     * Stop accepting new writes. If we have unsynced writes still in buffer, sync
     * them. Extant edits
    ...
    /**
     * Caller no longer needs any edits from this WAL. Implementers are free to
     * reclaim underlying
     * resources after this call; i.e. filesystem based WALs can archive or delete
     * files.
     */
    @Override
    ...
    /**
     * Append a set of data edits to the WAL. 'Data' here means that the content in
     * the edits will
     ...
     *              edit/sequence id.
     * @param edits Edits to append. MAY CONTAIN NO EDITS for case where we want to
     *              get an edit
    ...
    /**
     * Append an operational 'meta' event marker edit to the WAL. A marker meta edit
     * could be a
     * FlushDescriptor, a compaction marker, or a region event marker; e.g. region
     * open or region
     * close. The difference between a 'marker' append and a 'data' append as in
     * {@link #appendData(RegionInfo, WALKeyImpl, WALEdit)}is that a marker will not
     * have transitioned
    ...
    /**
     * updates the seuence number of a specific store. depending on the flag:
    ...
    /**
     * WAL keeps track of the sequence numbers that are as yet not flushed im
     * memstores in order to be
     * able to do accounting to figure which WALs can be let go. This method tells
     * WAL that some
     * region is about to flush. The flush can be the whole region or for a column
     * family of the
     * region only.
     * <p>
     * Currently, it is expected that the update lock is held for the region; i.e.
     * no concurrent
     * appends while we set up cache flush.
     * 
     * @param families Families to flush. May be a subset of all families in the
     *                 region.
     * @return Returns {@link HConstants#NO_SEQNUM} if we are flushing the whole
     *         region OR if we are
     *         flushing a subset of all families but there are no edits in those
     *         families not being
     *         flushed; in other words, this is effectively same as a flush of all
     *         of the region
     *         though we were passed a subset of regions. Otherwise, it returns the
     *         sequence id of the
     ...
     * @param maxFlushedSeqId   The maxFlushedSeqId for this flush. There is no edit
     *                          in memory that is
    ...
    /**
     * Abort a cache flush. Call if the flush fails. Note that the only recovery for
     * an aborted flush
     * currently is a restart of the regionserver so the snapshot content dropped by
     * the failure gets
     ...
     * @deprecated Since version 1.2.0. Removing because not used and exposes subtle
     *             internal
     ...
     * to include information appropriate for debugging. Consumers are advised not
     * to rely on the
    ...
    /**
     * Split a WAL filename to get a start time. WALs usually have the time we start
     * writing to them
     * as part of their name, usually the suffix. Sometimes there will be an extra
     * suffix as when it
     * is a WAL for the meta table. For example, WALs might look like this
     * <code>10.20.20.171%3A60020.1277499063250</code> where
     * <code>1277499063250</code> is the
     * timestamp. Could also be a meta WAL which adds a '.meta' suffix or a
     * synchronous replication
     * WAL which adds a '.syncrep' suffix. Check for these. File also may have no
     * timestamp on it. For
     * example the recovered.edits files are WALs but are named in ascending order.
     * Here is an
