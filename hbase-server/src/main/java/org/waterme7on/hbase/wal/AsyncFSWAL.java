 ...
 * as txid.</li>
 * <li>Schedule the consumer task if needed. See
 * {@link #shouldScheduleConsumer()} for more details.
 * </li>
 * </ol>
 * </li>
 ...
 * <li>Poll the entry from {@link #toWriteAppends}, append it to the
 * AsyncWriter, and insert it into
 * {@link #unackedAppends}</li>
 * <li>If the buffered size reaches {@link #batchSize}, or there is a sync
 * request, then we call
 * sync on the AsyncWriter.</li>
 * <li>In the callback methods:
 * <ul>
 * <li>If succeeded, poll the entry from {@link #unackedAppends} and drop
 * it.</li>
 * <li>If failed, add all the entries in {@link #unackedAppends} back to
 * {@link #toWriteAppends} and
 ...
 * </li>
 * </ol>
 * For sync, the processing stages are almost same. And different from FSHLog,
 * we will open a new
 * writer and rewrite unacked entries to the new writer and sync again if we hit
 * a sync error.
 * <p>
 * Here we only describe the logic of doReplaceWriter. The main logic of
 * rollWriter is same with
 * FSHLog.<br>
 ...
 * {@link #readyForRolling} to false, and then wait on
 * {@link #readyForRolling}(see
 * {@link #waitForSafePoint()}).</li>
 * <li>In the consumer thread, we will stop polling entries from
 * {@link #waitingConsumePayloads} if
 * {@link #waitingRoll} is true, and also stop writing the entries in
 * {@link #toWriteAppends} out.
 * </li>
 * <li>If there are unflush data in the writer, sync them.</li>
 * <li>When all out-going sync request is finished, i.e, the
 * {@link #unackedAppends} is empty,
...
public class AsyncFSWAL extends AbstractFSWAL<AsyncWriter> {
    ...
    private static final Comparator<SyncFuture> SEQ_COMPARATOR = Comparator.comparingLong(SyncFuture::getTxid)
    ...
    // the lowest bit is waitingRoll, which means new writer is created and we are
    // waiting for old
    // writer to be closed.
    // the second lowest bit is writerBroken which means the current writer is
    // broken and rollWriter
    // is needed.
    // all other bits are the epoch number of the current writer, this is used to
    // detect whether the
    // writer is still the one when you issue the sync.
    // notice that, modification to this field is only allowed under the protection
    // of consumeLock.
    ...
    public AsyncFSWAL(FileSystem fs, Abortable abortable, Path rootDir, String logDir,
        ...
        int preallocatedEventCount = conf.getInt("hbase.regionserver.wal.disruptor.event.count", 1024 * 16);
        waitingConsumePayloads = RingBuffer.createMultiProducer(RingBufferTruck::new, preallocatedEventCount);
    ...
    private boolean trySetReadyForRolling() {
        ...
        // waitingRoll is volatile and unacedEntries is only accessed inside event loop
        // so it is safe to
    ...
    private void syncFailed(long epochWhenSync, Throwable error) {
        ...
        try {
            ...
            if (epoch(currentEpochAndState) != epochWhenSync || writerBroken(currentEpochAndState)) {
                // this is not the previous writer which means we have already rolled the
                // writer.
                // or this is still the current writer, but we have already marked it as broken
                // and request
                // a roll.
                return;
            }
            ...
            if (waitingRoll(currentEpochAndState)) {
                readyForRolling = true;
                readyForRollingCond.signalAll();
                // this means we have already in the middle of a rollWriter so just tell the
                // roller thread
    ...
    private void syncCompleted(long epochWhenSync, AsyncWriter writer, long processedTxid,
            long startTimeNs) {
        // Please see the last several comments on HBASE-22761, it is possible that we
        // get a
        // syncCompleted which acks a previous sync request after we received a
        // syncFailed on the same
        // writer. So here we will also check on the epoch and state, if the epoch has
        // already been
        // changed, i.e, we have already rolled the writer, or the writer is already
        // broken, we should
        // just skip here, to avoid mess up the state or accidentally release some WAL
        // entries and
        // cause data corruption.
        // The syncCompleted call is on the critical write path so we should try our
        // best to make it
        // fast. So here we do not hold consumeLock, for increasing performance. It is
        // safe because
        // there are only 3 possible situations:
        // 1. For normal case, the only place where we change epochAndState is when
        // rolling the writer.
        // Before rolling actually happen, we will only change the state to waitingRoll
        // which is another
        // bit than writerBroken, and when we actually change the epoch, we can make
        // sure that there is
        // no out going sync request. So we will always pass the check here and there is
        // no problem.
        // 2. The writer is broken, but we have not called syncFailed yet. In this case,
        // since
        // syncFailed and syncCompleted are executed in the same thread, we will just
        // face the same
        // situation with #1.
        // 3. The writer is broken, and syncFailed has been called. Then when we arrive
        // here, there are
        // only 2 possible situations:
        // a. we arrive before we actually roll the writer, then we will find out the
        // writer is broken
        // and give up.
        // b. we arrive after we actually roll the writer, then we will find out the
        // epoch is changed
        // and give up.
        // For both #a and #b, we do not need to hold the consumeLock as we will always
        // update the
        ...
        if (trySetReadyForRolling()) {
            // we have just finished a roll, then do not need to check for log rolling, the
            // writer will be
            // closed soon.
            return;
        }
    ...
    // find all the sync futures between these two txids to see if we need to issue
    // a hsync, if no
    ...
    private void sync(AsyncWriter writer) {
        ...
        boolean shouldUseHsync = isHsync(highestProcessedAppendTxidAtLastSync, currentHighestProcessedAppendTxid);
    ...
    private int finishSync() {
        if (unackedAppends.isEmpty()) {
            ...
            if (toWriteAppends.isEmpty()) {
                // Also no appends that wait to be written out, then just finished all pending
                // syncs.
                ...
                // There is no append between highestProcessedAppendTxid and
                // lowestUnprocessedAppendTxid, so
                // if highestSyncedTxid >= highestProcessedAppendTxid, then all syncs whose txid
                // are between
            ...
            // There are still unacked appends. So let's move the highestSyncedTxid to the
            // txid of the
    ...
    private void appendAndSync() {
        ...
        // maybe a sync request is not queued when we issue a sync, so check here to see
        // if we could
        // finish some.
        finishSync();
        long newHighestProcessedAppendTxid = -1L;
        // this is used to avoid calling peedLast every time on unackedAppends,
        // appendAndAsync is single
        ...
        for (Iterator<FSWALEntry> iter = toWriteAppends.iterator(); iter.hasNext();) {
            ...
            iter.remove();
            if (appended) {
                // This is possible, when we fail to sync, we will add the unackedAppends back
                // to
                ...
                // See HBASE-25905, here we need to make sure that, we will always write all the
                // entries in
                // unackedAppends out. As the code in the consume method will assume that, the
                // entries in
                // unackedAppends have all been sent out so if there is roll request and
                // unackedAppends is
                // not empty, we could just return as later there will be a syncCompleted call
                // to clear the
                // unackedAppends, or a syncFailed to lead us to another state.
                // There could be other ways to fix, such as changing the logic in the consume
                // method, but
                // it will break the assumption and then (may) lead to a big refactoring. So
                // here let's use
        ...
        if (writer.getLength() == fileLengthAtLastSync) {
            // we haven't written anything out, just advance the highestSyncedSequence since
            // we may only
        ...
        // reach here means that we have some unsynced data but haven't reached the
        // batch size yet
        // but we will not issue a sync directly here even if there are sync requests
        // because we may
        // have some new data in the ringbuffer, so let's just return here and delay the
        // decision of
    ...
    private void consume() {
        ...
        if (toWriteAppends.isEmpty()) {
            if (waitingConsumePayloadsGatingSequence.get() == waitingConsumePayloads.getCursor()) {
                ...
                // recheck here since in append and sync we do not hold the consumeLock. Thing
                // may
                // happen like
                // 1. we check cursor, no new entry
                // 2. someone publishes a new entry to ringbuffer and the consumerScheduled is
                // true and
                // give up scheduling the consumer task.
                // 3. we set consumerScheduled to false and also give up scheduling consumer
                // task.
                if (waitingConsumePayloadsGatingSequence.get() == waitingConsumePayloads.getCursor()) {
    ...
    protected boolean doCheckLogLowReplication() {
        // not like FSHLog, AsyncFSOutput will fail immediately if there are errors
        // writing to DNs, so
