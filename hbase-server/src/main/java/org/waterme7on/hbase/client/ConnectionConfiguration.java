...
/**
 * Configuration parameters for the connection. Configuration is a heavy weight
 * registry that does a
 * lot of string operations and regex matching. Method calls into Configuration
 * account for high CPU
 * usage and have huge performance impact. This class caches connection-related
 * configuration values
 * in the ConnectionConfiguration object so that expensive conf.getXXX() calls
 * are avoided every
...
public class ConnectionConfiguration {
    ...
    /**
     * Parameter name for client pause when server is overloaded, denoted by an
     * exception where
     * {@link org.apache.hadoop.hbase.HBaseServerException#isServerOverloaded(Throwable)}
     * is true.
     */
    public static final String HBASE_CLIENT_PAUSE_FOR_SERVER_OVERLOADED = "hbase.client.pause.server.overloaded";

    static {
        // This is added where the configs are referenced. It may be too late to happen
        // before
        // any user _sets_ the old cqtbe config onto a Configuration option. So we still
        // need
        // to handle checking both properties in parsing below. The benefit of calling
        // this is
        // that it should still cause Configuration to log a warning if we do end up
        // falling
    ...
    ConnectionConfiguration(Configuration conf) {
        ...
        this.replicaCallTimeoutMicroSecondScan = conf.getInt(PRIMARY_SCAN_TIMEOUT_MICROSECOND,
                PRIMARY_SCAN_TIMEOUT_MICROSECOND_DEFAULT);

        this.metaReplicaCallTimeoutMicroSecondScan = conf.getInt(HConstants.HBASE_CLIENT_META_REPLICA_SCAN_TIMEOUT,
    ...
    protected ConnectionConfiguration() {
        ...
        this.metaReplicaCallTimeoutMicroSecondScan = HConstants.HBASE_CLIENT_META_REPLICA_SCAN_TIMEOUT_DEFAULT;
