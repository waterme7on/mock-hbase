 ...
 * <p>
 * A ScannerContext instance should be updated periodically throughout execution
 * whenever progress
 * towards a limit has been made. Each limit can be checked via the appropriate
 * checkLimit method.
 * <p>
 * Once a limit has been reached, the scan will stop. The invoker of
 * {@link InternalScanner#next(java.util.List)} or
 * {@link RegionScanner#next(java.util.List)} can
 * use the appropriate check*Limit methods to see exactly which limits have been
 * reached.
 * Alternatively, {@link #checkAnyLimitReached(LimitScope)} is provided to see
 * if ANY limit was
 * reached
 * <p>
 * {@link NoLimitScannerContext#NO_LIMIT} is an immutable static definition that
 * can be used
 * whenever a {@link ScannerContext} is needed but limits do not need to be
 * enforced.
 * <p>
 * NOTE: It is important that this class only ever expose setter methods that
 * can be safely skipped
 * when limits should be NOT enforced. This is because of the necessary
 * immutability of the class
 * {@link NoLimitScannerContext}. If a setter cannot be safely skipped, the
 * immutable nature of
...
public class ScannerContext {
    ...
    LimitFields limits;
    /**
     * A different set of progress fields. Only include batch, dataSize and
     * heapSize. Compare to
     * LimitFields, ProgressFields doesn't contain time field. As we save a deadline
     * in LimitFields,
     * so use {@link EnvironmentEdgeManager#currentTime()} directly when check time
     * limit.
     */
     ...
     * {@link InternalScanner#next(java.util.List)} and
     * {@link RegionScanner#next(java.util.List)} that, if true, the progress
     * tracked within this
     * {@link ScannerContext} instance should be considered while evaluating the
     * limits. Useful for
     * enforcing a set of limits across multiple calls (i.e. the limit may not be
     * reached in a single
     * invocation, but any progress made should be considered in future invocations)
     * <p>
     * Defaulting this value to false means that, by default, any tracked progress
     * will be wiped clean
     * on invocations to {@link InternalScanner#next(java.util.List)} and
     * {@link RegionScanner#next(java.util.List)} and the call will be treated as
     * though no progress
     * has been made towards the limits so far.
     * <p>
     * This is an important mechanism. Users of Internal/Region scanners expect that
     * they can define
     * some limits and then repeatedly invoke {@link InternalScanner#next(List)} or
     * {@link RegionScanner#next(List)} where each invocation respects these limits
     * separately.
     * <p>
     * For example:
     *
     ...
     * limits for a
     * particular RPC request and have those limits respected across multiple
     * invocations. This means
     * that the progress made towards the limits in earlier calls will be saved and
     * considered in
     * future invocations
     */
    ...
    // immediately. For
    // example, when we want to switch from pread to stream, we can only do it after
    // the rpc call is
    // returned.
    ...
    /**
     * Get the metrics instance. Should only be called after a call to
     * {@link #isTrackingMetrics()}
     * has been made to confirm that metrics are indeed being tracked.
     * 
     * @return {@link ServerSideScanMetrics} instance that is tracking metrics for
     *         this scan
     */
    ...
    /**
     * @return true if the progress tracked so far in this instance will be
     *         considered during an
     *         invocation of {@link InternalScanner#next(java.util.List)} or
     *         {@link RegionScanner#next(java.util.List)}. false when the progress
     *         tracked so far
    ...
    /**
     * Clear away any progress that has been made so far. All progress fields are
     * reset to initial
     * values
     */
    void clearProgress() {
    ...
    /**
     * Note that this is not a typical setter. This setter returns the
     * {@link NextState} that was
     * passed in so that methods can be invoked against the new state. Furthermore,
     * this pattern
     * allows the {@link NoLimitScannerContext} to cleanly override this setter and
     * simply return the
    ...
    /**
     * @return true when we have more cells for the current row. This usually
    ...
    /**
     * @param checkerScope The scope that the limit is being checked from. The time
     *                     limit is always
     *                     checked against
     *                     {@link EnvironmentEdgeManager.currentTime}
     * @return true when the limit is enforceable from the checker's scope and it
     *         has been reached
     */
    ...
    public enum NextState {
        ...
        /**
         * Special case of size limit reached to indicate that the size limit was
        ...
        /**
         * Special case of time limit reached to indicate that the time limit was
         * reached in the middle
         * of a row and thus a partial results was formed
         */
        ...
        /**
         * @return true when the state indicates that more values may follow those that
         *         have been
         *         returned
         */
        ...
        /**
         * Returns true when the state indicates that a limit has been reached and scan
         * should stop
         */
    ...
    /**
     * The various scopes where a limit can be enforced. Used to differentiate when
     * a limit should be
     * enforced or not.
     */
    public enum LimitScope {
        /**
         * Enforcing a limit between rows means that the limit will not be considered
         * until all the
        ...
        /**
         * Enforcing a limit between cells means that the limit will be considered after
         * each full cell
         * has been retrieved
         */
        BETWEEN_CELLS(1);
        ...
        /**
         * When enforcing a limit, we must check that the scope is appropriate for
         * enforcement.
         * <p>
         * To communicate this concept, each scope has a depth. A limit will be enforced
         * if the depth of
         * the checker's scope is less than or equal to the limit's scope. This means
         * that when checking
         * limits, the checker must know their own scope (i.e. are they checking the
         * limits between
         ...
         * @return true when the checker is in a scope that indicates the limit can be
         *         enforced. Limits
         *         can be enforced from "higher or equal" scopes (i.e. the checker's
         *         scope is at a
    ...
    private static class LimitFields {
        /**
         * Default values of the limit fields. Defined such that if a field does NOT
         * change from its
        ...
        // The batch limit will always be enforced between cells, thus, there isn't a
        // field to hold the
        // batch scope
        int batch = DEFAULT_BATCH;

        LimitScope sizeScope = DEFAULT_SCOPE;
        // The sum of cell data sizes(key + value). The Cell data might be in on heap or
        // off heap area.
        long dataSize = DEFAULT_SIZE;
        // The sum of heap space occupied by all tracked cells. This includes Cell
        // POJO's overhead as
        ...
        /**
         * Returns {@link LimitScope} indicating scope in which the size limit is
         * enforced
         */
        ...
        /**
         * Returns {@link LimitScope} indicating scope in which the time limit is
         * enforced
         */
