...
/**
 * Interface of all necessary to carry out a RPC service invocation on the
 * server. This interface
 * focus on the information needed or obtained during the actual execution of
 * the service method.
 */
public interface RpcCallContext {
    /**
     * Check if the caller who made this IPC call has disconnected. If called from
     * outside the context
     * of IPC, this does nothing.
     * 
     * @return &lt; 0 if the caller is still connected. The time in ms since the
     *         disconnection
     *         otherwise
     */
    ...
    /**
     * If the client connected and specified a codec to use, then we will use this
     * codec making
     ...
     * serving slower). We need
     * to ask this question per call because a server could be hosting both clients
     * that support
    ...
    /**
     * Sets a callback which has to be executed at the end of this RPC call. Such a
     * callback is an
    ...
    /**
     * The size of response cells that have been accumulated so far. This along with
     * the corresponding
     * increment call is used to ensure that multi's or scans dont get too
     * excessively large
     */
    long getResponseCellSize();

    /**
     * Add on the given amount to the retained cell size. This is not thread safe
     * and not synchronized
     * at all. If this is used by more than one thread then everything will break.
     * Since this is
