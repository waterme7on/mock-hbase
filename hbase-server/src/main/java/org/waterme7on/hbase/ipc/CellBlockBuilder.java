...
import org.apache.hadoop.hbase.ipc.CellScannerButNoCodecException;
...
class CellBlockBuilder {
    ...
    public CellBlockBuilder(Configuration conf) {
        this.conf = conf;
        this.cellBlockDecompressionMultiplier = conf.getInt("hbase.ipc.cellblock.decompression.buffersize.multiplier",
                3);

        // Guess that 16k is a good size for rpc buffer. Could go bigger. See the TODO
        // below in
        // #buildCellBlock.
     ...
     *         been flipped and is ready for reading. Use limit to find total size.
     *         If
     *         <code>pool</code> was not null, then this returned ByteBuffer came
     *         from there and
    ...
    public CellScanner createCellScanner(final Codec codec, final CompressionCodec compressor,
        ...
        // Not making the Decoder over the ByteBuffer purposefully. The Decoder over the
        // BB will
        // make Cells directly over the passed BB. This method is called at client side
        // and we don't
        // want the Cells to share the same byte[] where the RPC response is being read.
        // Caching of any
        // of the Cells at user's app level will make it not possible to GC the response
        // byte[]
     ...
     * 
     * @param codec     to use for cellblock
     * @param cellBlock ByteBuffer containing the cells written by the Codec. The
     *                  buffer should be
     *                  position()'ed at the start of the cell block and limit()'ed
     *                  at the end.
     * @return CellScanner to work against the content of <code>cellBlock</code>.
     *         All cells created
    ...
    public CellScanner createCellScannerReusingBuffers(final Codec codec,
        ...
        // If compressed, decompress it first before passing it on else we will leak
        // compression
