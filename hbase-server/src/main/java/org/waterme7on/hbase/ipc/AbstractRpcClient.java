...
/**
 * Provides the basics for a RpcClient implementation like configuration and
 * Logging.
 * <p>
 * Locking schema of the current IPC implementation
 * <ul>
 * <li>There is a lock in {@link AbstractRpcClient} to protect the fetching or
 * creating
 * connection.</li>
 * <li>There is a lock in {@link Call} to make sure that we can only finish the
 * call once.</li>
 * <li>The same for {@link HBaseRpcController} as {@link Call}. And see the
 * comment of
 * {@link HBaseRpcController#notifyOnCancel(RpcCallback, HBaseRpcController.CancellationCallback)}
 * of how to deal with cancel.</li>
 * <li>For connection implementation, the construction of a connection should be
 * as fast as possible
 * because the creation is protected under a lock. Connect to remote side when
 * needed. There is no
 * forced locking schema for a connection implementation.</li>
 * <li>For the locking order, the {@link Call} and {@link HBaseRpcController}'s
 * lock should be held
 * at last. So the callbacks in {@link Call} and {@link HBaseRpcController}
 * should be execute
...
public abstract class AbstractRpcClient<T extends RpcConnection> implements RpcClient {
    ...
    private static final ScheduledExecutorService IDLE_CONN_SWEEPER = Executors.newScheduledThreadPool(1,
    ...
    private static final LoadingCache<Address, AtomicInteger> concurrentCounterCache = CacheBuilder.newBuilder()
            .expireAfterAccess(1, TimeUnit.HOURS).build(new CacheLoader<Address, AtomicInteger>() {
                @Override
    ...
    private void cleanupIdleConnections() {
        ...
        synchronized (connections) {
            for (T conn : connections.values()) {
                ...
                // maxIdleTime, and the
                // connection itself has already shutdown. The latter check is because we may
                // still
                // have some pending calls on connection so we should not shutdown the
                // connection outside.
                // The connection itself will disconnect if there is no pending call for
                // maxIdleTime.
    ...
    public static String getDefaultCodec(final Configuration c) {
        // If "hbase.client.default.rpc.codec" is empty string -- you can't set it to
        // null because
    ...
    /**
     * Encapsulate the ugly casting and RuntimeException conversion in private
     * method.
     * 
     * @return Codec to use on this client.
     */
    Codec getCodec() {
        // For NO CODEC, "hbase.client.rpc.codec" must be configured with empty string
        // AND
        // "hbase.client.default.rpc.codec" also -- because default is to do cell block
        // encoding.
     ...
     * {@link org.apache.hadoop.hbase.util.PoolMap.PoolType#ThreadLocal}, otherwise
     * default to the
     * former. For applications with many user threads, use a small round-robin
     * pool. For applications
     * with few user threads, you may want to try using a thread-local pool. In any
     * case, the number
     * of {@link org.apache.hadoop.hbase.ipc.RpcClient} instances should not exceed
     * the operating
    ...
    /**
     * Return the pool size specified in the configuration, which is applicable only
     * if the pool type
    ...
    /**
     * Make a blocking call. Throws exceptions if there are network problems or if
     * the remote code
     * threw an exception.
     * 
     * @param ticket Be careful which ticket you pass. A new user will mean a new
     *               Connection.
    ...
    private T getConnection(ConnectionId remoteId) throws IOException {
        ...
        synchronized (connections) {
            if (!running) {
                throw new IOException("StoppedRpcClientException");
            }
    ...
    /**
     * Interrupt the connections to the given ip:port server. This should be called
     * if the server is
     ...
     * startup. In any case, they're likely to get connection refused (if the
     * process died) or no
     * route to host: i.e. their next retries should be faster and with a safe
     * exception.
     */
    @Override
    ...
    private static class AbstractRpcChannel {
        ...
        protected HBaseRpcController configureRpcController(RpcController controller) {
            HBaseRpcController hrc;
            // TODO: Ideally we should not use an RpcController other than
            // HBaseRpcController at client
