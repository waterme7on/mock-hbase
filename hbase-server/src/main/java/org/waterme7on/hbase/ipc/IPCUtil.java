...
class IPCUtil {
    ...
    public static int write(final OutputStream dos, final Message header, final Message param,
        ...
        // Must calculate total size and write that first so other side can read it all
        // in in one
        // swoop. This is dictated by how the server is currently written. Server needs
        // to change
    ...
    /**
     * Returns Size on the wire when the two messages are written with
     * writeDelimitedTo
     */
    ...
    /**
     * Takes an Exception, the address, and if pertinent, the RegionInfo for the
     * Region we were trying
     * to connect to and returns an IOException with the input exception as the
     * cause. The new
     * exception provides the stack trace of the place where the exception is thrown
     * and some extra
     * diagnostics information.
     * <p/>
     * Notice that we will try our best to keep the original exception type when
     * creating a new
     * exception, especially for the 'connection' exceptions, as it is used to
     * determine whether this
     * is a network issue or the remote side tells us clearly what is wrong, which
     * is important
     * deciding whether to retry. If it is not possible to create a new exception
     * with the same type,
     * for example, the {@code error} is not an {@link IOException}, an
     * {@link IOException} will be
     * created.
     * 
    ...
    static IOException wrapException(Address addr, RegionInfo regionInfo, Throwable error) {
        if (error instanceof ConnectException) {
            ...
            // ClosedChannelException does not have a constructor which takes a String but
            // it is a
            // connection exception so we keep its original type
            return (IOException) error;
        } else if (error instanceof TimeoutException) {
            // TimeoutException is not an IOException, let's convert it to
            // TimeoutIOException.
            ...
            if (error instanceof IOException) {
                try {
                    return (IOException) error.getClass().asSubclass(IOException.class)
                            .getConstructor(String.class)
                            .newInstance(
                                    "Call to " + getCallTarget(addr, regionInfo) + " failed on local exception: "
                                            + error)
                            .initCause(error);
                } catch (InstantiationException | IllegalAccessException | IllegalArgumentException
                        | InvocationTargetException | NoSuchMethodException | SecurityException e) {
    ...
    static void execute(EventLoop eventLoop, Runnable action) {
        if (eventLoop.inEventLoop()) {
            // this is used to prevent stack overflow, you can see the same trick in netty's
            // LocalChannel
            // implementation.
