...
class NettyRpcDuplexHandler extends ChannelDuplexHandler {
    ...
    private void readResponse(ChannelHandlerContext ctx, ByteBuf buf) throws IOException {
        ...
        if (call == null) {
            // So we got a response for which we have no corresponding 'call' here on the
            // client-side.
            // We probably timed out waiting, cleaned up all references, and now the server
            // decides
            // to return a response. There is nothing we can do w/ the response at this
            // stage. Clean
            // out the wire of the response so its out of the way and we can get other
            // responses on
            // this connection.
