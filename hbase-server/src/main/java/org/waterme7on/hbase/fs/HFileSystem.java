package org.waterme7on.hbase.fs;
 ...
 * flexibility of using separate filesystem objects for reading and writing
 * hfiles and wals.
 */
...
public class HFileSystem extends FilterFileSystem {
    ...
    public HFileSystem(Configuration conf, boolean useHBaseChecksum) throws IOException {
        ...
        // We take pains to funnel all of our FileSystem instantiation through this call
        // to ensure
        // we never need to call FS.initialize ourself so that we do not have to track
        // any state to
        ...
        // This manifests itself in that incorrect data is read and HFileBlocks won't be
        // able to read
    ...
    /**
     * Wrap a FileSystem object within a HFileSystem. The noChecksumFs and writefs
     * are both set to be
     * the same specified fs. Do not verify hbase-checksums while reading data from
     * filesystem.
     * 
     ...
     * @param policyName The name of the storage policy: 'HOT', 'COLD', etc. See see
     *                   hadoop 2.6+
     *                   org.apache.hadoop.hdfs.protocol.HdfsConstants for possible
     *                   list e.g 'COLD',
     ...
     * 
     * @param path The source path (directory/file).
     * @return Storage policy name, or {@code null} if not using
     *         {@link DistributedFileSystem} or
    ...
    /**
     * Before Hadoop 2.8.0, there's no getStoragePolicy method for FileSystem
     * interface, and we need
    ...
    private String getStoragePolicyForOldHDFSVersion(Path path) {
        ...
        // // Get the unspecified id field through reflection to avoid compilation
        // error.
        // // In later version BlockStoragePolicySuite#ID_UNSPECIFIED is moved to
        // // HdfsConstants#BLOCK_STORAGE_POLICY_ID_UNSPECIFIED
        // Field idUnspecified =
        // BlockStoragePolicySuite.class.getField("ID_UNSPECIFIED");
        // unspecifiedStoragePolicyId =
        // idUnspecified.getByte(BlockStoragePolicySuite.class);
        // }
    ...
    /**
     * Returns a brand new instance of the FileSystem. It does not use the
     * FileSystem.Cache. In newer
     * versions of HDFS, we can directly invoke
     ...
     * hbase.fs.wrapper
     * property, if one is set in the configuration, returns unmodified FS instance
     * passed in as an
     * argument otherwise.
     * 
    ...
    // public static boolean addLocationsOrderInterceptor(Configuration conf) throws
    // IOException {
    // return addLocationsOrderInterceptor(conf, new ReorderWALBlocks());
    // }

    /**
     * Add an interceptor on the calls to the namenode#getBlockLocations from the
     * DFSClient linked to
     * this FileSystem. See HBASE-6435 for the background.
     * <p/>
     * There should be no reason, except testing, to create a specific
     * ReorderBlocks.
     * 
     * @return true if the interceptor was added, false otherwise.
     */
    // static boolean addLocationsOrderInterceptor(Configuration conf, final
    // ReorderBlocks lrb) {
    ...
    // if (dfsc == null) {
    // LOG.warn("The DistributedFileSystem does not contain a DFSClient. Can't add
    // the location "
    ...
    // if (namenode == null) {
    // LOG.warn("The DFSClient is not linked to a namenode. Can't add the location
    // block"
    ...
    // nf.set(dfsc, cp1);
    // LOG.info("Added intercepting call to namenode#getBlockLocations so can do
    // block reordering"
    ...
    // Proxy.newProxyInstance(cp.getClass().getClassLoader(),
    // new Class[] { ClientProtocol.class, Closeable.class }, new
    // InvocationHandler() {
    // @Override
    ...
    // && "getBlockLocations".equals(method.getName()) && res instanceof
    // LocatedBlocks
    ...
    // if (cause == null) {
    // throw new RuntimeException("Proxy invocation failed and getCause is null",
    // ite);
    // }
    ...
    // */
    // void reorderBlocks(Configuration conf, LocatedBlocks lbs, String src) throws
    // IOException;
    // }

    // /**
    // * We're putting at lowest priority the wal files blocks that are on the same
    // * datanode as the
    // * original regionserver which created these files. This because we fear that
    // * the datanode is
    ...
    // ServerName sn = AbstractFSWALProvider.getServerNameFromWALDirectoryName(conf,
    // src);
    // if (sn == null) {
    // // It's not an WAL
    // return;
    ...
    // if (LOG.isTraceEnabled()) {
    // LOG.trace(src + " is an WAL file, so reordering blocks, last hostname will
    // be:" + hostName);
    // }
    ...
    /**
     * Create a new HFileSystem object, similar to FileSystem.get(). This returns a
     * filesystem object
     * that avoids checksum verification in the filesystem for hfileblock-reads. For
     * these blocks,
