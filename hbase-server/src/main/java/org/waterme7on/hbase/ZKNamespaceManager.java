...
/**
 * Class servers two purposes: 1. Broadcast NamespaceDescriptor information via
 * ZK (Done by the
 * Master) 2. Consume broadcasted NamespaceDescriptor changes (Done by the
 * RegionServers)
 */
...
public class ZKNamespaceManager extends ZKListener {
        ...
        if (nsZNode.equals(ZKUtil.getParent(path))) {
            try {
                ...
                NamespaceDescriptor ns = ProtobufUtil
                        .toNamespaceDescriptor(HBaseProtos.NamespaceDescriptor.parseFrom(data));
                cache.put(ns.getName(), ns);
            } catch (KeeperException ke) {
    ...
    private void deleteNamespace(String name) throws IOException {
        ...
        try {
            ...
            if (e instanceof KeeperException.NoNodeException) {
                // If the node does not exist, it could be already deleted. Continue without
                // fail.
    ...
    private void refreshNodes(List<ZKUtil.NodeAndData> nodes) throws IOException {
        for (ZKUtil.NodeAndData n : nodes) {
            ...
            NamespaceDescriptor ns = ProtobufUtil
                    .toNamespaceDescriptor(HBaseProtos.NamespaceDescriptor.parseFrom(nodeData));
            cache.put(ns.getName(), ns);
        }
    }
}
