...
/**
 * static convenience methods for dealing with KeyValues and collections of
 * KeyValues
 */
...
public class KeyValueUtil {
    ...
    public static byte[] copyToNewByteArray(final Cell cell) {
        // Cell#getSerializedSize returns the serialized size of the Source cell, which
        // may
    ...
    /**
     * Creates a new KeyValue object positioned in the supplied ByteBuffer and sets
     * the ByteBuffer's
    ...
    /**
     * Decrement the timestamp. For tests (currently wasteful) Remember timestamps
     * are sorted reverse
     * chronologically.
     * 
     * @return previous key
     ...
     * larger than or
     * equal to all other possible KeyValues that have the same row, family,
     * qualifier. Used for
     * reseeking. Should NEVER be returned to a client. row key row offset row
     * length family name
     * family offset family length column qualifier qualifier offset qualifier
     * length
     * 
    ...
    /**
     * Create a KeyValue that is smaller than all other possible KeyValues for the
     * given row. That is
    ...
    /**
     * Creates a KeyValue that is last on the specified row id. That is, every other
     * possible KeyValue
    ...
    /**
     * Creates a KeyValue that is smaller than all other KeyValues that are older
     * than the passed
     * timestamp.
     * 
     * @param row - row key (arbitrary byte array)
     ...
     * Create a KeyValue for the specified row, family and qualifier that would be
     * smaller than all
     * other possible KeyValues that have the same row,family,qualifier. Used for
     * seeking.
     * 
     * @param row       - row key (arbitrary byte array)
     ...
     * @param qualifier column qualifier
     * @return First possible key on passed Row, Family, Qualifier.
     * @throws IllegalArgumentException The resulting <code>KeyValue</code> object
     *                                  would be larger
     ...
     *         we will return a
     *         new {@link KeyValue} instance made from <code>cell</code> Note: Even
     *         if the cell is an
     *         object of any of the subclass of {@link KeyValue}, we will create a
     *         new
     *         {@link KeyValue} object wrapping same buffer. This API is used only
     *         with MR based tools
     *         which expect the type to be exactly KeyValue. That is the reason for
     *         doing this way.
    ...
    static void checkKeyValueBytes(byte[] buf, int offset, int length, boolean withTags) {
        ...
        if (pos + Bytes.SIZEOF_INT > endOffset) {
            String msg = "Overflow when reading key length at position=" + pos + bytesToHex(buf, offset, length);
            LOG.warn(msg);
            throw new IllegalArgumentException(msg);
        }
        ...
        if (keyLen <= 0 || pos + keyLen > endOffset) {
            String msg = "Invalid key length in KeyValue. keyLength=" + keyLen + bytesToHex(buf, offset, length);
            LOG.warn(msg);
            throw new IllegalArgumentException(msg);
        }
        // check the value
        if (pos + Bytes.SIZEOF_INT > endOffset) {
            String msg = "Overflow when reading value length at position=" + pos + bytesToHex(buf, offset, length);
            LOG.warn(msg);
            throw new IllegalArgumentException(msg);
        }
        ...
        if (valLen < 0 || pos + valLen > endOffset) {
            String msg = "Invalid value length in KeyValue, valueLength=" + valLen + bytesToHex(buf, offset, length);
            LOG.warn(msg);
            throw new IllegalArgumentException(msg);
        }
        // check the row
        if (pos + Bytes.SIZEOF_SHORT > endOffset) {
            String msg = "Overflow when reading row length at position=" + pos + bytesToHex(buf, offset, length);
            LOG.warn(msg);
            throw new IllegalArgumentException(msg);
        }
        ...
        if (rowLen < 0 || pos + rowLen > endOffset) {
            String msg = "Invalid row length in KeyValue, rowLength=" + rowLen + bytesToHex(buf, offset, length);
            LOG.warn(msg);
            throw new IllegalArgumentException(msg);
        }
        pos += rowLen;
        ...
        if (pos + Bytes.SIZEOF_BYTE > endOffset) {
            String msg = "Overflow when reading family length at position=" + pos + bytesToHex(buf, offset, length);
            LOG.warn(msg);
            throw new IllegalArgumentException(msg);
        }
        ...
        if (pos + Bytes.SIZEOF_LONG > endOffset) {
            String msg = "Overflow when reading timestamp at position=" + pos + bytesToHex(buf, offset, length);
            LOG.warn(msg);
            throw new IllegalArgumentException(msg);
        }
        long timestamp = Bytes.toLong(buf, pos, Bytes.SIZEOF_LONG);
        if (timestamp < 0) {
            String msg = "Timestamp cannot be negative, ts=" + timestamp + bytesToHex(buf, offset, length);
            LOG.warn(msg);
            throw new IllegalArgumentException(msg);
        }
        ...
        // check the type
        if (pos + Bytes.SIZEOF_BYTE > endOffset) {
            String msg = "Overflow when reading type at position=" + pos + bytesToHex(buf, offset, length);
            LOG.warn(msg);
            throw new IllegalArgumentException(msg);
        }
        byte type = buf[pos];
        ...
        if (pos + valLen > endOffset) {
            String msg = "Overflow when reading value part at position=" + pos + bytesToHex(buf, offset, length);
            LOG.warn(msg);
            throw new IllegalArgumentException(msg);
        }
        pos += valLen;
    ...
    private static int checkKeyValueTagBytes(byte[] buf, int offset, int length, int pos,
            int endOffset) {
        if (pos + Bytes.SIZEOF_SHORT > endOffset) {
            String msg = "Overflow when reading tags length at position=" + pos + bytesToHex(buf, offset, length);
            LOG.warn(msg);
            throw new IllegalArgumentException(msg);
        }
        ...
        for (; pos < tagsEndOffset;) {
            if (pos + Tag.TAG_LENGTH_SIZE > endOffset) {
                String msg = "Overflow when reading tag length at position=" + pos + bytesToHex(buf, offset, length);
                LOG.warn(msg);
                throw new IllegalArgumentException(msg);
            }
     ...
     * <code>createKeyValueFromInputStream</code> so doesn't clash with
     * {@link #create(DataInput)}
     * 
    ...
    /**
     * Returns a KeyValue made of a byte array that holds the key-only part. Needed
     * to convert hfile
    ...
    /**
     * Return a KeyValue made of a byte buffer that holds the key-only part. Needed
     * to convert hfile
     * index members to KeyValues.
     */
    ...
    /**
     * Return a KeyValue made of a byte array that holds the key-only part. Needed
     * to convert hfile
     * index members to KeyValues.
     */
    ...
    /**
     * Where to read bytes from. Creates a byte array to hold the KeyValue backing
     * bytes copied from
     * the steam.
     * 
