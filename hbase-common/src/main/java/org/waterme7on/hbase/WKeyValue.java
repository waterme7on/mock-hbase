...
public class WKeyValue implements ExtendedCell, Cloneable {
    private static final Logger LOG = LoggerFactory.getLogger(WKeyValue.class);

    public static final int FIXED_OVERHEAD = ClassSize.OBJECT + // the WKeyValue object itself
     ...
     * WKeyValue only.
     * 
     * @deprecated Use {@link CellComparator#getInstance()} instead. Deprecated for
     *             hbase 2.0, remove
     *             for hbase 3.0.
     */
    @Deprecated
    ...
    // Size of the length ints in a WKeyValue datastructure.
    ...
    /**
     * Computes the number of bytes that a <code>WKeyValue</code> instance with the
     * provided
     ...
     * @return the <code>WKeyValue</code> data structure length
     */
    public static long getKeyValueDataStructureSize(int rlength, int flength, int qlength,
            int vlength) {
        return WKeyValue.KEYVALUE_INFRASTRUCTURE_SIZE
    ...
    public static long getKeyValueDataStructureSize(int rlength, int flength, int qlength,
        ...
        return WKeyValue.KEYVALUE_WITH_TAGS_INFRASTRUCTURE_SIZE
    ...
    public static long getKeyValueDataStructureSize(int klength, int vlength, int tagsLength) {
        if (tagsLength == 0) {
            return (long) WKeyValue.KEYVALUE_INFRASTRUCTURE_SIZE + klength + vlength;
        }
        return (long) WKeyValue.KEYVALUE_WITH_TAGS_INFRASTRUCTURE_SIZE + klength + vlength + tagsLength;
    }
    ...
    public static long getKeyDataStructureSize(int rlength, int flength, int qlength) {
        return (long) WKeyValue.KEY_INFRASTRUCTURE_SIZE + rlength + flength + qlength;
    }
    ...
    public static enum Type {
        ...
        /**
         * Cannot rely on enum ordinals . They change if item is removed or moved. Do
         * our own codes.
         * 
    ...
    /**
     * Lowest possible key. Makes a Key with highest possible Timestamp, empty row
     * and column. No key
     * can be equal or lower than this one in memstore or in store file.
     */
    public static final WKeyValue LOWESTKEY = new WKeyValue(HConstants.EMPTY_BYTE_ARRAY, HConstants.LATEST_TIMESTAMP);

    ////
    // WKeyValue core instance fields.
    ...
    // multi-version concurrency control version. default value is 0, aka do not
    // care.
    ...
    /**
     * Creates a WKeyValue from the start of the specified byte array. Presumes
     * <code>bytes</code>
     * content is formatted as a WKeyValue blob.
     * 
     * @param bytes byte array
     */
    public WKeyValue(final byte[] bytes) {
        this(bytes, 0);
    }

    /**
     * Creates a WKeyValue from the specified byte array and offset. Presumes
     * <code>bytes</code>
     * content starting at <code>offset</code> is formatted as a WKeyValue blob.
     * 
     * @param bytes  byte array
     * @param offset offset to start of WKeyValue
     */
    public WKeyValue(final byte[] bytes, final int offset) {
        this(bytes, offset, getLength(bytes, offset));
    }

    /**
     * Creates a WKeyValue from the specified byte array, starting at offset, and
     * for
     * length
     * <code>length</code>.
     * 
     * @param bytes  byte array
     * @param offset offset to start of the WKeyValue
     * @param length length of the WKeyValue
     */
    public WKeyValue(final byte[] bytes, final int offset, final int length) {
     ...
     * @param ts     timestamp
     */
    public WKeyValue(final byte[] bytes, final int offset, final int length, long ts) {
    ...
    /**
     * Constructs WKeyValue structure filled with null value. Sets type to
     * {@link WKeyValue.Type#Maximum}
     * 
     * @param row       - row key (arbitrary byte array)
     * @param timestamp version timestamp
     */
    public WKeyValue(final byte[] row, final long timestamp) {
        this(row, null, null, timestamp, Type.Maximum, null);
    }

    /**
     * Constructs WKeyValue structure filled with null value.
     * 
     * @param row       - row key (arbitrary byte array)
     * @param timestamp version timestamp
     */
    public WKeyValue(final byte[] row, final long timestamp, Type type) {
     ...
     */
    public WKeyValue(final byte[] row, final byte[] family, final byte[] qualifier) {
        this(row, family, qualifier, HConstants.LATEST_TIMESTAMP, Type.Maximum);
    }

    /**
     * Constructs WKeyValue structure as a put filled with specified values and
     * LATEST_TIMESTAMP.
     * 
     * @param row       - row key (arbitrary byte array)
     * @param family    family name
    ...
    /**
     * Constructs WKeyValue structure filled with specified values.
     * 
     * @param row       row key
     * @param family    family name
     * @param qualifier column qualifier
     ...
     * @throws IllegalArgumentException an illegal value was passed
     */
    public WKeyValue(byte[] row, byte[] family, byte[] qualifier, int qoffset, int qlength,
     ...
     * @throws IllegalArgumentException an illegal value was passed
     */
    public WKeyValue(final byte[] row, final int roffset, final int rlength, final byte[] family,
    ...
    /**
     * Constructs WKeyValue structure filled with specified values. Uses the
     * provided
     * buffer as the
     * data buffer.
     * <p>
     ...
     * @throws IllegalArgumentException an illegal value was passed or there is
     *                                  insufficient space
     *                                  remaining in the buffer
     */
    public WKeyValue(byte[] buffer, final int boffset, final byte[] row, final int roffset,
    ...
    /**
     * Constructs an empty WKeyValue structure, with specified sizes. This can be
     * used to partially
     * fill up KeyValues.
     * <p>
     * Column is split into two fields, family and qualifier.
     ...
     * @throws IllegalArgumentException an illegal value was passed
     */
    public WKeyValue(final int rlength, final int flength, final int qlength, final long timestamp,
    ...
    public WKeyValue(byte[] row, int roffset, int rlength, byte[] family, int foffset, int flength,
    ...
    /**
     * Create an empty byte[] representing a WKeyValue All lengths are preset and
     * can
     * be filled in
     * later.
     * 
    ...
    /**
     * Write WKeyValue format into the provided byte array.
     * 
     * @param buffer    the bytes buffer to use
     * @param boffset   buffer offset
     * @param row       row key
    ...
    /**
     * Write WKeyValue format into a byte array.
     * 
     * @param row       row key
     * @param roffset   row offset
     * @param rlength   row length
    ...
    public boolean equals(Object other) {
        ...
        return CellUtil.equals((Cell) this, (Cell) other);
    }

    /**
     * In line with {@link #equals(Object)}, only uses the key portion, not the
     * value.
     */
    @Override
    public int hashCode() {
    ...
    /**
     * Clones a WKeyValue. This creates a copy, re-allocating the buffer.
     * 
     * @return Fully copied clone of this WKeyValue
     * @throws CloneNotSupportedException if cloning of keyValue is not supported
     */
    @Override
    public WKeyValue clone() throws CloneNotSupportedException {
        WKeyValue ret = (WKeyValue) super.clone();
    ...
    /**
     * Creates a shallow copy of this WKeyValue, reusing the data byte buffer.
     * http://en.wikipedia.org/wiki/Object_copy
     * 
     * @return Shallow copy of this WKeyValue
     */
    public WKeyValue shallowCopy() {
        WKeyValue shallowCopy = new WKeyValue(this.bytes, this.offset, this.length);
     ...
     * and manipulation of
     * the data stored in an WALKey, for example, printing as JSON. Values are left
     * out due to their
     ...
     * Use for logging.
     * 
     * @param b Key portion of a WKeyValue.
    ...
    /**
     * To be used only in tests where the Cells are clearly assumed to be of type
     * WKeyValue and that we
     * need access to the backing array to do some test case related assertions.
     * 
     * @return The byte array backing this WKeyValue.
     */
    public byte[] getBuffer() {
        return this.bytes;
    }

    /** Returns Offset into {@link #getBuffer()} at which this WKeyValue starts. */
    public int getOffset() {
        return this.offset;
    }

    /** Returns Length of bytes this WKeyValue occupies in {@link #getBuffer()}. */
    ...
    /**
     * Determines the total length of the WKeyValue stored in the specified byte
     * array and offset.
     * Includes all headers.
     * 
     * @param bytes  byte array
     * @param offset offset to start of the WKeyValue
     * @return length of entire WKeyValue, in bytes
     */
    ...
    /**
     * Returns the backing array of the entire WKeyValue (all WKeyValue fields are
     * in
     * a single array)
     */
    @Override
    ...
    int getFamilyLengthPosition(int rowLength) {
        return this.offset + WKeyValue.ROW_KEY_OFFSET + rowLength;
    }
    ...
    /** Returns True if this WKeyValue has a LATEST_TIMESTAMP timestamp. */
     ...
     * Use
     * {@link #getRowArray()}, {@link #getFamilyArray()},
     * {@link #getQualifierArray()}, and
     * {@link #getValueArray()} if accessing a WKeyValue client-side.
     * 
    ...
    /** Returns WKeyValue.TYPE byte representation */
    @Override
    public byte getTypeByte() {
        return getTypeByte(getKeyLength());
    }

    /** Return the WKeyValue.TYPE byte representation */
    ...
    public int getTagsLength() {
        ...
        if (tagsLen > 0) {
            // There are some Tag bytes in the byte[]. So reduce 2 bytes which is added to
            // denote the tags
            // length
    ...
    /**
     * Creates a new WKeyValue that only contains the key portion (the value is set
     * to be null). TODO
     * only used by KeyOnlyFilter -- move there.
     * 
     * @param lenAsVal replace value with the actual value length (false=empty)
     */
    public WKeyValue createKeyOnly(boolean lenAsVal) {
        ...
        return new WKeyValue(newBuffer);
    }
     ...
     * @return Index of delimiter having started from start of <code>b</code> moving
     *         rightward.
     */
    ...
    public static class MetaComparator extends KVComparator {
        /**
         * Compare key portion of a {@link WKeyValue} for keys in
        ...
        /**
         * The HFileV2 file format's trailer contains this class name. We reinterpret
         * this and
        ...
        public String getLegacyKeyComparatorName() {
            return "org.apache.hadoop.hbase.WKeyValue$MetaKeyComparator";
        }
     ...
     * portion. This means two
     * KeyValues with same Key but different Values are considered the same as far
     * as this Comparator
     * is concerned.
     * 
    ...
    public static class KVComparator implements RawComparator<Cell>, SamePrefixComparator<byte[]> {
         ...
         */
        public String getLegacyKeyComparatorName() {
            return "org.apache.hadoop.hbase.WKeyValue$KeyComparator";
        }
        ...
        /**
         * Compares the only the user specified portion of a Key. This is overridden by
         * MetaComparator.
         * 
        ...
        /**
         * Compares left to right assuming that left,loffset,llength and
         * right,roffset,rlength are full
        ...
        public int compareOnlyKeyPortion(Cell left, Cell right) {
            return PrivateCellUtil.compareKeyIgnoresMvcc(CellComparatorImpl.COMPARATOR,
                    left, right);
        }

        /**
         * Compares the Key of a cell -- with fields being more significant in this
         * order: rowkey,
        ...
        /**
         * Compare columnFamily, qualifier, timestamp, and key type (everything except
         * the row). This
         * method is used both in the normal comparator and the "same-prefix"
         * comparator. Note that we
         * are assuming that row portions of both KVs have already been parsed and found
         * identical, and
         * we don't validate that assumption here. the length of the common prefix of
         * the two key-values
        ...
        private int compareWithoutRow(int commonPrefix, byte[] left, int loffset, int llength,
            ...
            /***
             * WKeyValue Format and commonLength:
        ...
        public boolean matchingRowColumn(final Cell left, final Cell right) {
            ...
            if ((left.getRowLength() + left.getFamilyLength() + left.getQualifierLength()) != (right.getRowLength()
        ...
        public byte[] getShortMidpointKey(final byte[] leftKey, final byte[] rightKey) {
            ...
            return new WKeyValue(newRowKey, null, null, HConstants.LATEST_TIMESTAMP, Type.Maximum)
                    .getKey();
        }
    ...
    /**
     * Create a WKeyValue reading from <code>in</code>
     * 
     * @param in Where to read bytes from. Creates a byte array to hold the
     *           WKeyValue
     *           backing bytes
     *           copied from the steam.
     * @return WKeyValue created by deserializing from <code>in</code> OR if we find
     *         a length of zero,
     *         we will return null which can be useful marking a stream as done.
     * @throws IOException if any IO error happen
     */
    public static WKeyValue create(final DataInput in) throws IOException {
        return create(in.readInt(), in);
    }

    /**
     * Create a WKeyValue reading <code>length</code> from <code>in</code>
     * 
     * @param length length of the Key
     * @param in     Input to read from
     * @return Created WKeyValue OR if we find a length of zero, we will return null
     *         which can be
     *         useful marking a stream as done.
     * @throws IOException if any IO error happen
     */
    public static WKeyValue create(int length, final DataInput in) throws IOException {
        ...
        in.readFully(bytes);
        return new WKeyValue(bytes, 0, length);
    }

    /**
     * Write out a WKeyValue in the manner in which we used to when WKeyValue was a
     * Writable.
     * 
     * @param kv  the WKeyValue on which write is being requested
     * @param out OutputStream to write keyValue to
     * @return Length written on stream
     * @throws IOException if any IO error happen
     * @see #create(DataInput) for the inverse function
     */
    public static long write(final WKeyValue kv, final DataOutput out) throws IOException {
        // This is how the old Writables write used to serialize KVs. Need to figure way
        // to make it
     ...
     * Write out a WKeyValue in the manner in which we used to when WKeyValue was a
     * Writable but do not
     * require a {@link DataOutput}, just take plain {@link OutputStream} Named
     * <code>oswrite</code>
     * so does not clash with {@link #write(WKeyValue, DataOutput)}
     * 
     * @param kv       the WKeyValue on which write is being requested
     * @param out      OutputStream to write keyValue to
     ...
     * @throws IOException if any IO error happen
     * @see #create(DataInput) for the inverse function
     * @see #write(WKeyValue, DataOutput)
     * @see KeyValueUtil#oswrite(Cell, OutputStream, boolean)
     * @deprecated As of release 2.0.0, this will be removed in HBase 3.0.0. Instead
     *             use
     *             {@link #write(OutputStream, boolean)}
     */
    @Deprecated
    public static long oswrite(final WKeyValue kv, final OutputStream out, final boolean withTags)
            throws IOException {
    ...
    /**
     * Avoids redundant comparisons for better performance. TODO get rid of this
     * wart
     */
     ...
     * <p/>
     * We do not count the bytes in the rowCache because it should be empty for a
     * WKeyValue in the
     * MemStore.
     */
    @Override
    public long heapSize() {
        // Deep object overhead for this KV consists of two parts. The first part is the
        // KV object
        // itself, while the second part is the backing byte[]. We will only count the
        // array overhead
    ...
    /**
     * A simple form of WKeyValue that creates a keyvalue with only the key part of
     * the byte[] Mainly
     * used in places where we need to compare two cells. Avoids copying of bytes In
     * places like block
     * index keys, we need to compare the key byte[] with a cell. Hence create a
     * Keyvalue(aka Cell)
     * that would help in comparing as two cells
     */
    public static class KeyOnlyKeyValue extends WKeyValue {
        ...
        /**
         * A setter that helps to avoid object creation every time and whenever there is
         * a need to
        ...
        @Override
        public int write(OutputStream out, boolean withTags) throws IOException {
            // This type of Cell is used only to maintain some internal states. We never
            // allow this type
    ...
    public ExtendedCell deepClone() {
        ...
        WKeyValue kv = new WKeyValue(copy, 0, copy.length);
