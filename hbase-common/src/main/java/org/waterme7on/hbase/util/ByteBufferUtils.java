...
import org.waterme7on.hbase.io.ByteBufferWriter;
...
public final class ByteBufferUtils {
    ...
    static class ComparerHolder {
        ...
        static Comparer getBestComparer() {
            try {
                Class<? extends Comparer> theClass = Class.forName(UNSAFE_COMPARER_NAME).asSubclass(Comparer.class);
    ...
    static class ConverterHolder {
        ...
        static Converter getBestConverter() {
            try {
                Class<? extends Converter> theClass = Class.forName(UNSAFE_CONVERTER_NAME).asSubclass(Converter.class);
    ...
    /**
     * Similar to {@link WritableUtils#writeVLong(java.io.DataOutput, long)}, but
     * writes to a
     * {@link ByteBuffer}.
     */
    ...
    /**
     * Similar to {@link WritableUtils#readVLong(java.io.DataInput)} but reads from
     * a
     * {@link ByteBuff}.
     */
    ...
    /**
     * Put in buffer integer using 7 bit encoding. For each written byte: 7 bits are
     * used to store
    ...
    /**
     * Copy data from a buffer to an output stream. Does not update the position in
     * the buffer.
     * 
     * @param out    the stream to write bytes to
     * @param in     the buffer to read bytes from
     * @param offset the offset in the buffer (from the buffer's array offset) to
     *               start copying bytes
     *               from
     * @param length the number of bytes to copy
     */
    ...
    /**
     * Copy from the InputStream to a new heap ByteBuffer until the InputStream is
     * exhausted.
     */
    ...
    /**
     * Copy one buffer's whole data to another. Write starts at the current position
     * of 'out' buffer.
     * Note : This will advance the position marker of {@code out} and also change
     * the position maker
     * for {@code in}.
     * 
    ...
    /**
     * Copy from one buffer to another from given offset. This will be absolute
     * positional copying and
    ...
    public static int findCommonPrefix(ByteBuffer buffer, int offsetLeft, int offsetRight,
            int limit) {
        int prefix = 0;
    ...
    public static int findCommonPrefix(ByteBuffer left, int leftOffset, int leftLength,
        ...
        while (result < length && ByteBufferUtils.toByte(left, leftOffset + result) == ByteBufferUtils.toByte(right,
                rightOffset + result)) {
            result++;
        }
    ...
    /**
     * Copy the bytes from position to limit into a new byte[] of the exact length
     * and sets the
    ...
    // The below two methods show up in lots of places. Versions of them in commons
    // util and in
    // Cassandra. In guava too? They are copied from ByteBufferUtils. They are here
    // as static
    // privates. Seems to make code smaller and make Hotspot happier (comes of
    // compares and study
    ...
    static int compareToUnsafe(Object obj1, long o1, int l1, Object obj2, long o2, int l2) {
        ...
        for (i = 0; i < strideLimit; i += stride) {
            ...
            if (lw != rw) {
                 ...
                 * This
                 * corresponds to the least significant nonzero byte in lw ^ rw, since lw and rw
                 * are
                 * little-endian. Long.numberOfTrailingZeros(diff) tells us the least
                 * significant nonzero
                 * bit, and zeroing out the first three bits of L.nTZ gives us the shift to get
                 * that least
                 * significant nonzero byte. This comparison logic is based on UnsignedBytes
                 * from guava v21
                 */
    ...
    /**
     * Reads an int value at the given buffer's current position. Also advances the
     * buffer's position
     */
     ...
     * @return the int value if there's not enough bytes left in the buffer after
     *         the given offset
     */
    ...
    /**
     * Put an int value out to the given ByteBuffer's current position in big-endian
     * format. This also
    ...
    /**
     * Put a short value out to the given ByteBuffer's current position in
     * big-endian format. This
    ...
    /**
     * Put a long value out to the given ByteBuffer's current position in big-endian
     * format. This also
     ...
     * to buffer's current position. This also advances the position in the 'out'
     * buffer by 'length'
     * 
    ...
    /**
     * Copies specified number of bytes from given offset of 'in' ByteBuffer to the
     * array. This
