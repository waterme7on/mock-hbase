 ...
 * actual bytes that an
 * object of this class will occupy in memory The core of this class is taken
 * from the Derby project
 */
...
public class ClassSize {
    ...
    private static class MemoryLayout {
        ...
        /**
         * Return the size of an "ordinary object pointer". Either 4 or 8, depending on
         * 32/64 bit, and
         * CompressedOops
         */
        int oopSize() {
    ...
    /**
     * UnsafeLayout uses Unsafe to guesstimate the object-layout related parameters
     * like object header
    ...
    private static class UnsafeLayout extends MemoryLayout {
        ...
        private static final int ARRAY_OBJECT_INDEX_SCALE = HBasePlatformDependent.arrayIndexScale(Object[].class);

        private static final int ARRAY_BYTE_INDEX_SCALE = HBasePlatformDependent.arrayIndexScale(byte[].class);
        ...
        @Override
        int oopSize() {
            // Unsafe.addressSize() returns 8, even with CompressedOops. This is how many
            // bytes each
    ...
    private static MemoryLayout getMemoryLayout() {
        // Have a safeguard in case Unsafe estimate is wrong. This is static context,
        // there is
    ...
    /**
     * Method for reading the arc settings and setting overheads according to 32-bit
     * or 64-bit
     * architecture.
     */
    static {
        ...
        // STRING is different size in jdk6 and jdk7. Just use what we estimate as size
        // rather than
        ...
        // different between
        // 23.6-b03 and 23.0-b21. Just use what we estimate as size rather than have a
        // conditional on
        // whether jdk7.
        ...
        // CellFlatMap object contains two integers, one boolean and one reference to
        // object, so
        ...
        // CELL_CHUNK_MAP is the size of an instance of CellChunkMap class, which
        // extends
        ...
        // The Cell Representation in the CellChunkMap, the Cell object size shouldn't
        // be counted
        // in KeyValue.heapSize()
        // each cell-representation requires three integers for chunkID (reference to
        // the ByteBuffer),
    ...
    /**
     * The estimate of the size of a class instance depends on whether the JVM uses
     * 32 or 64 bit
     * addresses, that is it depends on the size of an object reference. It is a
     * linear function of
     * the size of a reference, e.g. 24 + 5*r where r is the size of a reference
     * (usually 4 or 8
     * bytes). This method returns the coefficients of the linear function, e.g.
     * {24, 5} in the above
     * example.
     * 
     ...
     * the space taken up
     * by by references (the pointer) but not by the referenced object. So the
     * estimated size of an
     * array field does not depend on the size of the array. Similarly the size of
     * an object
    ...
    /**
     * Determines if we are running in a 32-bit JVM. Some unit tests need to know
     * this too.
     */
    ...
    /**
     * Calculate the memory consumption (in byte) of a byte array, including the
     * array header and the
     * whole backing byte array. If the whole byte array is occupied (not shared
     * with other objects),
     * please use this function. If not, please use {@link #sizeOfByteArray(int)}
     * instead.
     * 
     ...
     * including the array
     * header and the part of the backing byte array. This function is used when the
     * byte array backs
     * multiple objects. For example, in {@link org.apache.hadoop.hbase.KeyValue},
     * multiple KeyValue
     * objects share a same backing byte array
     * ({@link org.apache.hadoop.hbase.KeyValue#bytes}). Also
