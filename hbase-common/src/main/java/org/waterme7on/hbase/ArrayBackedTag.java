...
public class ArrayBackedTag implements Tag {
    ...
    /**
     * The special tag will write the length of each tag and that will be followed
     * by the type and
     * then the actual tag. So every time the length part is parsed we need to add +
     * 1 byte to it to
    ...
    /**
     * Format for a tag : {@code <length of tag - 2 bytes><type code - 1 byte><tag>}
     * tag length is
     * serialized using 2 bytes only but as this will be unsigned, we can have max
     * tag length of
     * (Short.MAX_SIZE * 2) +1. It includes 1 byte type length and actual tag bytes
     * length.
     */
     ...
     * <code>bytes</code> content
     * starting at <code>offset</code> is formatted as a Tag blob. The bytes to
     * include the tag type,
     * tag length and actual tag bytes.
     * 
