...
/**
 * This class is a server side extension to the {@link Cell} interface. It is
 * used when the Cell is
 * backed by a {@link ByteBuffer}: i.e.
 * <code>cell instanceof ByteBufferedCell</code>.
 * <p>
 ...
 * these field bytes
 * begin. These are needed because a single ByteBuffer may back one or many Cell
 * instances -- it
 ...
 * these position
 * methods with the getXXXOffset methods from the super Interface, {@link Cell};
 * dependent up on
 ...
 * getXXXArray() will result in a
 * temporary byte array creation and a bytes copy. Avoid these allocations by
 * using the appropriate
 * Cell access server-side: i.e. ByteBufferedCell when backed by a ByteBuffer
 * and Cell when it is
 * not.
 */
/*
 * Even though all the methods are abstract, ByteBufferExtendedCell is not made
 * to be an interface
 * with intent. In CellComparator compare method, we have instance of check to
 * decide whether to use
 * getXXXArray() or getXXXByteBuffer(). This is a very hot method in read and
 * write paths. if (left
 * instanceof ByteBufferExtendedCell && right instanceof ByteBufferExtendedCell)
 * { .... } if (left
 * instanceof ByteBufferExtendedCell) { .... } if (right instanceof
 * ByteBufferExtendedCell) { .... }
 * return Bytes.compareTo(left.getRowArray(), left.getRowOffset(),
 * left.getRowLength(),
 * right.getRowArray(), right.getRowOffset(), right.getRowLength()); We did JMH
 * micro benchmark
 * tests with both left and right cells as ByteBufferExtendedCell, one only
 * ByteBufferExtendedCell
 * and both as Cells. This is compared against JMH results on compare logic with
 * out any instance of
 * checks. We noticed that if ByteBufferExtendedCell is an interface, the
 * benchmark result seems to
 * be very bad for case of both right and left are Cell only (Not
 * ByteBufferExtendedCell). When
 * ByteBufferExtendedCell is an abstract class all 4 possible cases giving
 * almost similar
