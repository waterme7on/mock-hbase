 ...
 */
package org.waterme7on.hbase.io.encoding;
...
import org.waterme7on.hbase.io.util.LRUDictionary;
...
abstract class BufferedDataBlockEncoder extends AbstractDataBlockEncoder {
  /**
   * TODO: This datablockencoder is dealing in internals of hfileblocks. Purge
   * reference to HFBs
   */
  ...
  public ByteBuffer decodeKeyValues(DataInputStream source,
    ...
    if (decodingCtx.getHFileContext().isIncludesTags()
      ...
      if (decodingCtx.getTagCompressionContext() != null) {
        // It will be overhead to create the TagCompressionContext again and again for
        // every block
        // decoding.
  ...
  protected static class SeekerState {
     ...
     * placeholder). Used to
     * save the previous state when we are advancing the seeker to the next
     * key/value.
     */
    ...
    public Cell toCell() {
      ...
      if (this.includeTags && this.tagCompressionContext == null) {
        // Include the tags part also. This will be the tags bytes + 2 bytes of for
        // storing tags
        // length
   ...
   * seeker state
   * members for taking a clone. Note that the value byte[] part is still pointing
   * to the
  ...
  protected static class OnheapDecodedCell implements ExtendedCell {
    ...
    public int write(OutputStream out, boolean withTags) throws IOException {
      ...
      if (withTags && this.tagsLength > 0) {
        ...
        // tags length is serialized with 2 bytes only(short way) even if the type is
        // int.
  ...
  protected static class OffheapDecodedExtendedCell extends ByteBufferExtendedCell {
    private static final long FIXED_OVERHEAD = (long) ClassSize
        .align(ClassSize.OBJECT + (3 * ClassSize.REFERENCE) + (2 * Bytes.SIZEOF_LONG)
            + (7 * Bytes.SIZEOF_INT) + Bytes.SIZEOF_SHORT)
        + (2 * Bytes.SIZEOF_BYTE)
  ...
  protected abstract static class BufferedEncodedSeeker<STATE extends SeekerState>
    ...
    protected void decodeTags() {
      ...
      if (tagCompressionContext != null) {
        ...
        // When tag compress is not used, let us not do copying of tags bytes into
        // tagsBuffer.
    ...
    private void moveToPrevious() {
      ...
      // Already decoded the tag bytes. We cache this tags into current state and also
      // the total
      // compressed length of the tags bytes. For the next time decodeNext() we don't
      // need to decode
      // the tags again. This might pollute the Data Dictionary what we use for the
      // compression.
      // When current.uncompressTags is false, we will just reuse the
      // current.tagsBuffer and skip
  ...
  protected final int afterEncodingKeyValue(Cell cell, DataOutputStream out,
    ...
    if (encodingCtx.getHFileContext().isIncludesTags()) {
      ...
      if (tagsLength > 0) {
        ...
        // When tag compression is enabled, tagCompressionContext will have a not null
        // value. Write
        // the tags using Dictionary compression in such a case
        if (tagCompressionContext != null) {
          // Not passing tagsLength considering that parsing of the tagsLength is not
          // costly
    ...
    if (encodingCtx.getHFileContext().isIncludesMvcc()) {
      ...
      // TODO use a writeVLong which returns the #bytes written so that 2 time parsing
      // can be
      // avoided.
  ...
  protected final void afterDecodingKeyValue(DataInputStream source, ByteBuffer dest,
    ...
    if (decodingCtx.getHFileContext().isIncludesTags()) {
      ...
      if (tagsLength > 0) {
        ...
        // When tag compression is been used in this file, tagCompressionContext will
        // have a not
        // null value passed.
        if (tagCompressionContext != null) {
  ...
  /**
   * Asserts that there is at least the given amount of unfilled space remaining
   * in the given
   * buffer.
   * 
