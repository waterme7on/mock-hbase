...
public interface HFileBlockEncodingContext {
  ...
  /**
   * Do any action that needs to be performed after the encoding. Compression is
   * also included if a
   * non-null compression algorithm is used
   */
   ...
   * @return Bytes with header which are ready to write out to disk. This is
   *         compressed and
   *         encrypted bytes applying the set compression algorithm and
   *         encryption. The bytes may be
   *         changed. If need a Bytes reference for later use, clone the bytes and
   *         use that. Null if
   *         the data doesn't need to be compressed and encrypted.
   */
