...
import org.waterme7on.hbase.io.encoding.AbstractDataBlockEncoder.AbstractEncodedSeeker;
...
public class RowIndexSeekerV1 extends AbstractEncodedSeeker {
  ...
  public int seekToKeyInBlock(Cell seekCell, boolean seekBefore) {
    ...
    do {
      int comp = PrivateCellUtil.compareKeyIgnoresMvcc(this.cellComparator, seekCell, current.currentKey);
  ...
  private class SeekerState {
    /**
     * The size of a (key length, value length) tuple that prefixes each entry in a
     * data block.
     */
