...
/**
 * Dictionary interface Dictionary indexes should be either bytes or shorts,
 * only positive. (The
...
public interface Dictionary {
   ...
   * @param length Length beyond <code>offset</code> that comprises entry; must be
   *               &gt; 0.
  ...
  /**
   * Adds an entry to the dictionary. Be careful using this method. It will add an
   * entry to the
   * dictionary even if it already has an entry for the same data. Call
   * {{@link #findEntry(byte[], int, int)}} to add without duplicating dictionary
   * entries.
   * 
