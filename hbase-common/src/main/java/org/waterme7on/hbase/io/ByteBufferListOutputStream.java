...
/**
 * An OutputStream which writes data into ByteBuffers. It will try to get
 * ByteBuffer, as and when
 * needed, from the passed pool. When pool is not giving a ByteBuffer it will
 * create one on heap.
 * Make sure to call {@link #releaseResources()} method once the Stream usage is
 * over and data is
...
public class ByteBufferListOutputStream extends ByteBufferOutputStream {
  ...
  // Keep track of the BBs where bytes written to. We will first try to get a BB
  // from the pool. If
  // it is not available will make a new one our own and keep writing to that. We
  // keep track of all
  // the BBs that we got from pool, separately so that on closeAndPutbackBuffers,
  // we can make sure
  ...
  public int size() {
    ...
    // On the last BB, it might not be flipped yet if getByteBuffers is not yet
    // called
  ...
  public void writeTo(OutputStream out) throws IOException {
    // No usage of this API in code. Just making it as an Unsupported operation as
    // of now
   ...
   * Call this only
   * when all the data is fully used. And it must be called at the end of usage
   * else we will leak
