...
import org.waterme7on.hbase.io.compress.Compression.Algorithm;
...
public class EncodedDataBlock {
  ...
  // The is for one situation that there are some cells includes tags and others
  // are not.
  // isTagsLenZero stores if cell tags length is zero before doing encoding since
  // we need
  // to check cell tags length is zero or not after decoding.
  // Encoders ROW_INDEX_V1 would abandon tags segment if tags is 0 after decode
  // cells to
  // byte array, other encoders won't do that. So we have to find a way to add
  // tagsLen zero
    ...
    return new Iterator<Cell>() {
      ...
      public Cell next() {
        ...
        if (meta.isIncludesTags()) {
          ...
          // ROW_INDEX_V1 will not put tagsLen back in cell if it is zero, there is no
          // need
          // to read short here.
        ...
        KeyValue kv = new KeyValue(decompressedData.array(), decompressedData.arrayOffset() + offset,
  ...
  public byte[] encodeData() {
    ...
    try {
      ...
      // Below depends on BAOS internal behavior. toByteArray makes a copy of bytes so
      // far.
      baos.flush();
      baosBytes = baos.toByteArray();
      this.dataBlockEncoder.endBlockEncoding(encodingCtx, out, baosBytes);
      // In endBlockEncoding(encodingCtx, out, baosBytes), Encoder ROW_INDEX_V1 write
      // integer in
      // out while the others write integer in baosBytes(byte array). We need to add
      // baos.toByteArray() after endBlockEncoding again to make sure the integer
      // writes in
      // outputstream with Encoder ROW_INDEX_V1 dump to byte array (baosBytes).
      // The if branch is necessary because Encoders excepts ROW_INDEX_V1 write
      // integer in
      // baosBytes directly, without if branch and do toByteArray() again, baosBytes
      // won't
