...
/**
 * Compress key by storing size of common prefix with previous KeyValue and
 * storing raw size of
 * rest. Format: 1-5 bytes: compressed key length minus prefix (7-bit encoding)
 * 1-5 bytes:
 * compressed value length (7-bit encoding) 1-3 bytes: compressed length of
 * common key prefix ...
 * bytes: rest of key (including timestamp) ... bytes: value In a worst case
 * compressed KeyValue
...
public class PrefixKeyDeltaEncoder extends BufferedDataBlockEncoder {
  ...
  private void writeKeyExcludingCommon(Cell cell, int commonPrefix, DataOutputStream out)
    ...
    if (commonPrefix < rLen + KeyValue.ROW_LENGTH_SIZE) {
      // Previous and current rows are different. Need to write the differing part
      // followed by
      // cf,q,ts and type
      ...
      // The full row key part is common. CF part will be common for sure as we deal
      // with Cells in
