 ...
 */
package org.waterme7on.hbase.io.crypto;
...
import org.waterme7on.hbase.io.crypto.aes.AES;
...
public final class Encryption {
  ...
  /**
   * Default value for globally enable / disable column family encryption (set to
   * "true" for
   ...
   * encrypted HFiles. This
   * is a MessageDigest algorithm identifier string, like "MD5", "SHA-256" or
   * "SHA-384". (default:
  ...
  /**
   * Default hash algorithm used for generating key hash in encrypted HFiles. (we
   * use "MD5" for
   ...
   * the one used for generating key hash in encrypted HFiles currently being
   * read. - "false"
   * (default): we won't fail but use the hash algorithm stored in the HFile -
   * "true": we throw an
   * exception (this can be useful if regulations are enforcing the usage of
   * certain algorithms,
   ...
   * the HFile. (this is the more fail-safe approach, allowing us to read
   * encrypted HFiles written
   ...
   */
  @InterfaceAudience.Public
  public static class Context extends org.waterme7on.hbase.io.crypto.Context {
  ...
  /**
   * Returns the hash of the supplied argument, using the hash algorithm specified
   * in the given
   * config.
   */
  ...
  /**
   * Return a 128 bit key derived from the concatenation of the supplied arguments
   * using
   ...
   * PBKDF2WithHmacSHA384 key derivation algorithm at 10,000 iterations. The
   * length of the returned
   * key is determined based on the need of the cypher algorithm. E.g. for the
   * default "AES" we will
   * need a 128 bit long key, while if the user is using a custom cipher, we might
   * generate keys
   * with other length. This key generation method is used currently e.g. in the
   * HBase Shell
   * (admin.rb) to generate a column family data encryption key, if the user
   * provided an
   ...
   * @param keyLengthBytes the length of the key to be derived (in bytes, not in
   *                       bits)
   * @param password       char array to use as password for the key generation
   *                       algorithm
  ...
  /**
   * Decrypt a block of ciphertext read in from a stream with the given cipher and
   * context
   * <p>
  ...
  /**
   * Encrypts a block of plaintext with the symmetric key resolved for the given
   * subject
   * 
   * @param out    ciphertext
   * @param in     plaintext
  ...
  /**
   * Decrypts a block of ciphertext with the symmetric key resolved for the given
   * subject
   * 
   * @param out     plaintext
   * @param in      ciphertext
  ...
  public static void decryptWithSubjectKey(OutputStream out, InputStream in, int outLen,
    ...
    try {
      ...
      // If the current cipher algorithm fails to unwrap, try the alternate cipher
      // algorithm, if one
      // is configured
