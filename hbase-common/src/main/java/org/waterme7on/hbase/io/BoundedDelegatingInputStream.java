...
public class BoundedDelegatingInputStream extends DelegatingInputStream {
  ...
  /**
   * @return the remaining bytes within the bound if the current position is less
   *         than the limit, or
   *         0 otherwise.
   */
  @Override
  public int available() throws IOException {
    ...
    // Do not call the delegate's available() method. Data in a bounded input stream
    // is assumed
    // available up to the limit and that is the contract we have with our callers.
    // Regardless
    // of what we do here, read() and skip() will behave as expected when EOF is
    // encountered if
    // the underlying stream is closed early or otherwise could not provide enough
    // bytes.
    // Note: This class is used to supply buffers to compression codecs during WAL
    // tailing and
