 ...
 */
package org.waterme7on.hbase.io.util;
...
import org.waterme7on.hbase.io.hfile.trace.HFileContextAttributesBuilderConsumer;
...
public final class BlockIOUtils {
  ...
  private static void initByteBufferPositionReadableMethod() {
    try {
      ...
      byteBufferPositionedReadMethod = FSDataInputStream.class.getMethod("read", long.class, ByteBuffer.class);
   ...
   * @param dis    the HDFS input stream which implement the ByteBufferReadable
   *               interface.
  ...
  public static void readFully(ByteBuff buf, FSDataInputStream dis, int length) throws IOException {
    ...
    if (!isByteBufferReadable(dis)) {
      // If InputStream does not support the ByteBuffer read, just read to heap and
      // copy bytes to
  ...
  /**
   * Copying bytes from InputStream to {@link ByteBuff} by using an temporary heap
   * byte[] (default
   * size is 1024 now).
   * 
  ...
  /**
   * Read bytes into ByteBuffers directly, those buffers either contains the
   * extraLen bytes or only
   * contains necessaryLen bytes, which depends on how much bytes do the last time
   * we read.
   * 
   * @param buf          the destination {@link ByteBuff}.
   ...
   * @return if the returned flag is true, then we've finished to read the
   *         extraLen into our
   ...
   * read. If the input stream supports ByteBufferPositionedReadable, it reads to
   * the byte buffer
   ...
   * @param necessaryLen the number of bytes that are absolutely necessary to read
   * @param extraLen     the number of extra bytes that would be nice to read
   * @return true if and only if extraLen is > 0 and reading those extra bytes was
   *         successful
   * @throws IOException if failed to read the necessary bytes
   */
  ...
  /**
   * Construct a fresh {@link AttributesBuilder} from the provided
   * {@link Context}, populated with
   * relevant attributes populated by
   * {@link HFileContextAttributesBuilderConsumer#CONTEXT_KEY}.
   */
  ...
  /**
   * Conditionally annotate {@code span} with the appropriate attribute when value
   * is non-zero.
   */
  ...
  /**
   * Conditionally annotate {@code attributesBuilder} with appropriate attributes
   * when values are
   * non-zero.
   */
