 ...
 */
package org.waterme7on.hbase.io;
...
import org.waterme7on.hbase.nio.ByteBuff;
import org.waterme7on.hbase.nio.SingleByteBuff;
...
/**
 * ByteBuffAllocator is used for allocating/freeing the ByteBuffers from/to NIO
 * ByteBuffer pool, and
 * it provide high-level interfaces for upstream. when allocating desired memory
 * size, it will
 * return {@link ByteBuff}, if we are sure that those ByteBuffers have reached
 * the end of life
 * cycle, we must do the {@link ByteBuff#release()} to return back the buffers
 * to the pool,
 * otherwise ByteBuffers leak will happen, and the NIO ByteBuffer pool may be
 * exhausted. there's
 * possible that the desired memory size is large than ByteBufferPool has, we'll
 * downgrade to
 * allocate ByteBuffers from heap which meaning the GC pressure may increase
 * again. Of course, an
 * better way is increasing the ByteBufferPool size if we detected this case.
 * <br/>
 * <br/>
 * On the other hand, for better memory utilization, we have set an lower bound
 * named
 * minSizeForReservoirUse in this allocator, and if the desired size is less
 * than
 * minSizeForReservoirUse, the allocator will just allocate the ByteBuffer from
 * heap and let the JVM
 * free its memory, because it's too wasting to allocate a single fixed-size
 * ByteBuffer for some
 * small objects. <br/>
 * <br/>
 * We recommend to use this class to allocate/free {@link ByteBuff} in the RPC
 * layer or the entire
 * read/write path, because it hide the details of memory management and its
 * APIs are more friendly
 * to the upper layer.
 */
...
public class ByteBuffAllocator {
  ...
  // usage, such as
  // scanning snapshot, we won't have an RpcServer to initialize the allocator, so
  // just use the
   ...
   * {@link DeallocateRewriteByteBuffAllocator} to find out prematurely release
   * issues
   */
   ...
   * delta is very small,
   * depends on the size of lastKeyValue. If we set buffer.size=64KB, then each
   * block will be
   * allocated as a MultiByteBuff: one 64KB DirectByteBuffer and delta bytes
   * HeapByteBuffer, the
   * HeapByteBuffer will increase the GC pressure. Ideally, we should let the data
   * block to be
   * allocated as a SingleByteBuff, it has simpler data structure, faster access
   * speed, less heap
   * usage.
   * <p>
   * 2. Since the blocks are MultiByteBuff when using buffer.size=64KB, so we have
   * to calculate the
   * checksum by an temp heap copying (see HBASE-21917), while if it's a
   * SingleByteBuff, we can
   * speed the checksum by calling the hadoop' checksum in native lib, which is
   * more faster.
   * <p>
  ...
  // If the desired size is at least this size, it'll allocated from
  // ByteBufferPool, otherwise it'll
  // allocated from heap for better utilization. We make this to be 1/6th of the
  // pool buffer size.
  private final int minSizeForReservoirUse;

  private final Queue<ByteBuffer> buffers = new ConcurrentLinkedQueue<>();

  // Metrics to track the pool allocation bytes and heap allocation bytes. If heap
  // allocation
  // bytes is increasing so much, then we may need to increase the
  // max.buffer.count .
  ...
  /**
   * Initialize an {@link ByteBuffAllocator} which will try to allocate
   * ByteBuffers from off-heap if
   * reservoir is enabled and the reservoir has enough buffers, otherwise the
   * allocator will just
   ...
   * @param reservoirEnabled indicate whether the reservoir is enabled or
   *                         disabled. NOTICE: if
   *                         reservoir is enabled, then we will use the pool
   *                         allocator to allocate
   *                         off-heap ByteBuffers and use the HEAP allocator to
   *                         allocate heap
   *                         ByteBuffers. Otherwise if reservoir is disabled then
   *                         all allocations
  ...
  public static ByteBuffAllocator create(Configuration conf, boolean reservoirEnabled) {
    ...
    if (reservoirEnabled) {
      // The max number of buffers to be pooled in the ByteBufferPool. The default
      // value been
      // selected based on the #handlers configured. When it is read request, 2 MB is
      // the max size
      // at which we will send back one RPC request. Means max we need 2 MB for
      // creating the
      // response cell block. (Well it might be much lesser than this because in 2 MB
      // size calc, we
      // include the heap size overhead of each cells also.) Considering 2 MB, we will
      // need
      // (2 * 1024 * 1024) / poolBufSize buffers to make the response cell block. Pool
      // buffer size
      // is by default 64 KB.
      // In case of read request, at the end of the handler process, we will make the
      // response
      // cellblock and add the Call to connection's response Q and a single Responder
      // thread takes
      // connections and responses from that one by one and do the socket write. So
      // there is chances
      // that by the time a handler originated response is actually done writing to
      // socket and so
      // released the BBs it used, the handler might have processed one more read req.
      // On an avg 2x
      // we consider and consider that also for the max buffers to pool
      int bufsForTwoMB = (2 * 1024 * 1024) / poolBufSize;
      int maxBuffCount = conf.getInt(MAX_BUFFER_COUNT_KEY, conf.getInt(HConstants.REGION_SERVER_HANDLER_COUNT,
  ...
  /**
   * Initialize an {@link ByteBuffAllocator} which only allocate ByteBuffer from
   * on-heap, it's
  ...
  /**
   * The {@link ConcurrentLinkedQueue#size()} is O(N) complexity and
   * time-consuming, so DO NOT use
  ...
  public static double getHeapAllocationRatio(ByteBuffAllocator... allocators) {
    ...
    // If disabled the pool allocator, then we use the global HEAP allocator.
    // otherwise we use
    // the pool allocator to allocate offheap ByteBuffers and use the HEAP to
    // allocate heap
    // ByteBuffers. So here we use a HashSet to remove the duplicated allocator
    // object in disable
    // case.
    for (ByteBuffAllocator alloc : Sets.newHashSet(allocators)) {
   ...
   * {@link ByteBuff#release()} if no need any more, otherwise the memory leak
   * happen in NIO
   * ByteBuffer pool.
   * 
  ...
  public ByteBuff allocate(int size) {
    ...
    // Allocate from ByteBufferPool until the remaining is less than
    // minSizeForReservoirUse or
    ...
    if (remain > 0) {
      // If the last ByteBuffer is too small or the reservoir can not provide more
      // ByteBuffers, we
  ...
  /**
   * @return One free DirectByteBuffer from the pool. If no free ByteBuffer and we
   *         have not reached
   *         the maximum pool size, it will create a new one and return. In case
   *         of max pool size
   *         also reached, will return null. When pool returned a ByteBuffer, make
   *         sure to return it
  ...
  /**
   * Return back a ByteBuffer after its use. Don't read/write the ByteBuffer after
   * the returning.
   * 
