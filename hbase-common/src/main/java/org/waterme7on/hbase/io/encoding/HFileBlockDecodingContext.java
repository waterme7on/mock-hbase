...
public interface HFileBlockDecodingContext {
   ...
   * process.
   * Decompression needs to be done if {@link HFileContext#getCompression()}
   * returns a valid
   * compression algorithm. numBytes after block and encoding headers numBytes
   * without header
   * required to store the block after decompressing (not decoding) ByteBuffer
   * pointed after the
