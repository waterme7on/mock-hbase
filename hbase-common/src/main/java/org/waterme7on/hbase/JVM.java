 ...
 * will decide to use the sun api or its own implementation depending on the
 * runtime (vendor) used.
 */
...
public class JVM {
    ...
    private static final boolean ibmvendor = System.getProperty("java.vendor") != null
            && System.getProperty("java.vendor").contains("IBM");
    // At least on my systems os.name reports as "linux", not "Linux". Prefer case
    // insensitive tests.
    ...
    private static final boolean amd64 = System.getProperty("os.arch") != null
            && System.getProperty("os.arch").contains("amd64");
    private static final boolean aarch64 = System.getProperty("os.arch") != null
            && System.getProperty("os.arch").contains("aarch64");

    private static final String JVMVersion = System.getProperty("java.version");

    /**
     * The raw String of java specification version. "1.8" for java8, "9","10"...
     * for Java 9, 10...
     */
    ...
    /**
     * Load the implementation of UnixOperatingSystemMXBean for Oracle jvm and runs
     * the desired
     * method.
     * 
     * @param mBeanMethodName : method to run from the interface
    ...
    /**
     * Get the number of opened filed descriptor for the runtime jvm. If Oracle
     * java, it will use the
     * com.sun.management interfaces. Otherwise, this methods implements it (linux
     * only).
     * 
    ...
    /**
     * Return the physical free memory (not the JVM one, as it's not very useful as
     * it depends on the
     * GC), but the one from the OS as it allows a little bit more to guess if the
     * machine is
     * overloaded or not).
     */
    ...
    /**
     * Workaround to get the current number of process running. Approach is the one
     * described here:
    ...
    /**
     * Get the number of the maximum file descriptors the system can use. If Oracle
     * java, it will use
     * the com.sun.management interfaces. Otherwise, this methods implements it
     * (linux only).
     * 
