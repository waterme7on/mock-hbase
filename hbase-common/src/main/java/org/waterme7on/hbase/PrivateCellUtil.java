...
import org.waterme7on.hbase.io.TagCompressionContext;
import org.waterme7on.hbase.io.util.Dictionary;
import org.waterme7on.hbase.io.util.StreamUtils;
...
/**
 * Utility methods helpful slinging {@link Cell} instances. It has more powerful
 * and rich set of
...
public final class PrivateCellUtil {
    ...
    /**
     * This can be used when a Cell has to change with addition/removal of one or
     * more tags. This is
    ...
    public static Optional<Tag> getTag(Cell cell, byte type) {
        ...
        int offset = bufferBacked ? ((ByteBufferExtendedCell) cell).getTagsPosition() : cell.getTagsOffset();
        int pos = offset;
     ...
     * specifically used when we
     * need to optimize the seeks based on the next indexed key. This is an advanced
     * usage API
     ...
     * @param coff            the offset of the column hint if provided, if not
     *                        offset of the
     *                        currentCell's qualifier
     * @param clen            the length of the column hint if provided, if not
     *                        length of the
     *                        currentCell's qualifier
     * @param ts              the timestamp to be seeked
     * @param type            the type to be seeked
     * @return an int based on the given column hint TODO : To be moved out of here
     *         because this is a
     ...
     * id/mvcc of the cell
     * 
     * @return an int greater than 0 if left &gt; than right lesser than 0 if left
     *         &lt; than right
    ...
    /**
     * These cells are used in reseeks/seeks to improve the read performance. They
     * are not real cells
    ...
    /**
     * Writes the qualifier from the given cell to the output stream excluding the
     * common prefix
     * 
     * @param out     The dataoutputstream to which the data has to be written
     * @param cell    The cell whose contents has to be written
     ...
     * 
     * @param out                   the outputstream to which the compression should
     *                              happen
     ...
     * @return greater than 0 if left cell is bigger, less than 0 if right cell is
     *         bigger, 0 if both
     *         cells are equal
     */
    ...
    /**
     * Return a new cell is located following input cell. If both of type and
     * timestamp are minimum,
    ...
    /**
     * Creates the first cell with the row/family/qualifier of this cell and the
     * given timestamp. Uses
     * the "maximum" type that guarantees that the new cell is the lowest possible
     * for this
     * combination of row, family, qualifier, and timestamp. This cell's own
    ...
    /**
     * Create a Delete Family Cell for the specified row and family that would be
     * smaller than all
