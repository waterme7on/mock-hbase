...
import org.waterme7on.hbase.io.HeapSize;
 ...
 * comparing to other keys in
 * the same table. It uses CellComparator which does not work on the -ROOT- and
 * hbase:meta tables.
 * </p>
 * <p>
...
public interface Cell extends HeapSize {
    ...
    /**
     * Contiguous raw bytes that may start at any index in the containing array. Max
     * length is
    ...
    /**
     * Returns Number of qualifier bytes. Must be &lt; qualifierArray.length -
     * offset.
     */
    ...
    /**
     * Return a long value representing time at which this cell was "Put" into the
     * row. Typically
    ...
    /**
     * Return the byte representation of the KeyValue.TYPE of this cell: one of Put,
     * Delete, etc
     * 
    ...
    /**
     * A region-specific unique monotonically increasing sequence ID given to each
     * Cell. It always
     * exists for cells in the memstore but is not retained forever. It will be kept
     * for
     * {@link HConstants#KEEP_SEQID_PERIOD} days, but generally becomes irrelevant
     * after the cell's
     ...
     * @deprecated As of HBase-2.0. Will be removed in HBase-3.0. Tags are are now
     *             internal.
     */
    @Deprecated
    byte[] getTagsArray();
    ...
    /**
     * HBase internally uses 2 bytes to store tags length in Cell. As the tags
     * length is always a
    ...
    /**
     * Returns the type of cell in a human readable format using {@link Type}. Note
     * : This does not
