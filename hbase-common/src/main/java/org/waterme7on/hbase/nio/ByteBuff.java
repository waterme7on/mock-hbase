 ...
 */
package org.waterme7on.hbase.nio;
...
import org.waterme7on.hbase.io.ByteBuffAllocator.Recycler;
 ...
 * either single or
 * multiple. We have this interface because the java's ByteBuffers cannot be
 * sub-classed. This class
 * provides APIs similar to the ones provided in java's nio ByteBuffers and
 * allows you to do
 * positional reads/writes and relative reads and writes on the underlying BB.
 * In addition to it, we
 * have some additional APIs which helps us in the read path. <br/>
 * The ByteBuff implement {@link HBaseReferenceCounted} interface which mean
 * need to maintains a
 * {@link RefCnt} inside, if ensure that the ByteBuff won't be used any more, we
 * must do a
 * {@link ByteBuff#release()} to recycle its NIO ByteBuffers. when considering
 * the
 * {@link ByteBuff#duplicate()} or {@link ByteBuff#slice()}, releasing either
 * the duplicated one or
 * the original one will free its memory, because they share the same NIO
 * ByteBuffers. when you want
 * to retain the NIO ByteBuffers even if the origin one called
 * {@link ByteBuff#release()}, you can
 * do like this:
 *
 * <pre>
 *   ByteBuff original = ...;
...
public abstract class ByteBuff implements HBaseReferenceCounted {
  ...
  /**
   * Returns bytes from current position till length specified, as a single
   * ByteBuffer. When all
   ...
   * limit of this
   * returned ByteBuffer. The position of the returned byte buffer is at the begin
   * of the required
   * bytes. When the required bytes happen to span across multiple ByteBuffers,
   * this API will copy
   * the bytes to a newly created ByteBuffer of required size and return that.
   * 
   * @param length number of bytes required.
   * @return bytes from current position till length specified, as a single
   *         ByteButter.
   */
  public abstract ByteBuffer asSubByteBuffer(int length);

  /**
   * Returns bytes from given offset till length specified, as a single
   * ByteBuffer. When all these
   * bytes happen to be in a single ByteBuffer, which this object wraps, that
   * ByteBuffer item as
   * such will be returned (with offset in this ByteBuffer where the bytes
   * starts). So users are
   * warned not to change the position or limit of this returned ByteBuffer. When
   * the required bytes
   * happen to span across multiple ByteBuffers, this API will copy the bytes to a
   * newly created
   * ByteBuffer of required size and return that.
   * 
   * @param offset the offset in this ByteBuff from where the subBuffer should be
   *               created
   * @param length the length of the subBuffer
   * @param pair   a pair that will have the bytes from the current position till
   *               length specified,
   *               as a single ByteBuffer and offset in that Buffer where the
   *               bytes starts. Since
   *               this API gets called in a loop we are passing a pair to it
   *               which could be created
   *               outside the loop and the method would set the values on the
   *               pair that is passed
   *               in by the caller. Thus it avoids more object creations that
   *               would happen if the
  ...
  /**
   * Similar to {@link ByteBuffer}.reset(), ensures that this ByteBuff is reset
   * back to last marked
   * position.
   * 
  ...
  /**
   * Returns an ByteBuff which is a sliced version of this ByteBuff. The position,
   * limit and mark of
   * the new ByteBuff will be independent than that of the original ByteBuff. The
   * content of the new
   ...
   * The content of the
   * new ByteBuff will start at this ByteBuff's current position The position,
   * limit and mark of the
  ...
  /**
   * Fetches the byte at the given index. Does not change position of the
  ...
  /**
   * Fetches the byte at the given offset from current position. Does not change
   * position of the
   * underlying ByteBuffers.
   * 
  ...
  /**
   * Writes a byte to this ByteBuff at the current position and increments the
   * position
   * 
   * @return this object
   */
   ...
   * 
   * @param sourceOffset the offset in this ByteBuff from where the copy should
   *                     happen
   * @param dst          the byte[] to which the ByteBuff's content is to be
   *                     copied
   * @param offset       within the current array
   * @param length       upto which the bytes to be copied
   */
  ...
  /**
   * Copies the content from this ByteBuff's current position to the byte array
   * and fills it. Also
   ...
   * @param offset the position in the byte array from which the copy should be
   *               done
  ...
  /**
   * Returns the short value at the current position. Also advances the position
   * by the size of
   * short.
   */
   ...
   * underlying
   * ByteBuffers. The caller is sure that the index will be after the current
   * position of this
   * ByteBuff. So even if the current short does not fit in the current item we
   * can safely move to
  ...
  /**
   * Fetches the short value at the given offset from current position. Does not
   * change position of
  ...
  /**
   * Returns the int value at the current position. Also advances the position by
   * the size of int.
   */
  public abstract int getInt();

  /**
   * Writes an int to this ByteBuff at its current position. Also advances the
   * position by size of
   * int.
   */
  ...
  /**
   * Fetches the int at the given index. Does not change position of the
   * underlying ByteBuffers.
   * Even if the current int does not fit in the current item we can safely move
   * to the next item
  ...
  /**
   * Fetches the int value at the given offset from current position. Does not
   * change position of
   * the underlying ByteBuffers.
   */
  ...
  /**
   * Returns the long value at the current position. Also advances the position by
   * the size of long.
   */
  public abstract long getLong();

  /**
   * Writes a long to this ByteBuff at its current position. Also advances the
   * position by size of
   * long.
   */
   ...
   * underlying ByteBuffers.
   * The caller is sure that the index will be after the current position of this
   * ByteBuff. So even
   * if the current long does not fit in the current item we can safely move to
   * the next item and
  ...
  /**
   * Fetches the long value at the given offset from current position. Does not
   * change position of
   * the underlying ByteBuffers.
   * 
   * @return the long value at the given index.
  ...
  /**
   * Copies the content from this ByteBuff to a ByteBuffer Note : This will
   ...
   * @param sourceOffset the offset in the ByteBuff from which the elements has to
   *                     be copied
   * @param length       the length in this ByteBuff upto which the elements has
   *                     to be copied
   */
   ...
   * 
   * @param offset    the position in this ByteBuff to which the copy should
   *                  happen
   * @param src       the src ByteBuff
   * @param srcOffset the offset in the src ByteBuff from where the elements
   *                  should be read
   ...
   * @param o1   the offset in the first ByteBuff from where the compare has to
   *             happen
   * @param len1 the length in the first ByteBuff upto which the compare has to
   *             happen
   * @param buf2 the second ByteBuff
   * @param o2   the offset in the second ByteBuff from where the compare has to
   *             happen
   * @param len2 the length in the second ByteBuff upto which the compare has to
   *             happen
   * @return Positive if buf1 is bigger than buf2, 0 if they are equal, and
   *         negative if buf1 is
   *         smaller than buf2.
   */
  ...
  /**
   * In theory, the upstream should never construct an ByteBuff by passing an
   * given refCnt, so
   * please don't use this public method in other place. Make the method public
   * here because the
   * BucketEntry#wrapAsCacheable in hbase-server module will use its own refCnt
   * and ByteBuffers from
   * IOEngine to composite an HFileBlock's ByteBuff, we didn't find a better way
   * so keep the public
   * way here.
   */
  ...
  /**
   * Calling this method in strategic locations where ByteBuffs are referenced may
   * help diagnose
   * potential buffer leaks. We pass the buffer itself as a default hint, but one
   * can use
  ...
  // Make this private because we don't want to expose the refCnt related wrap
  // method to upstream.
