...
import static org.waterme7on.hbase.io.ByteBuffAllocator.NONE;
...
/**
 * Provides a unified view of all the underlying ByteBuffers and will look as if
 * a bigger sequential
 * buffer. This class provides similar APIs as in {@link ByteBuffer} to put/get
 * int, short, long etc
 * and doing operations like mark, reset, slice etc. This has to be used when
 * data is split across
 * multiple byte buffers and we don't want copy them to single buffer for
 * reading from it.
 */
...
public class MultiByteBuff extends ByteBuff {
  ...
  MultiByteBuff(RefCnt refCnt, ByteBuffer... items) {
    ...
    // See below optimization in getInt(int) where we check whether the given index
    // land in current
    // item. For this we need to check whether the passed index is less than the
    // next item begin
    // offset. To handle this effectively for the last item buffer, we add an extra
    // item into this
    // array.
  ...
  /**
   * @throws UnsupportedOperationException MBB does not support array based
   *                                       operations
   */
  @Override
  public byte[] array() {
  ...
  public byte getByteAfterPosition(int offset) {
    checkRefCount();
    // Mostly the index specified will land within this current item. Short circuit
    // for that
  ...
  /**
   * Fetches the short at the given index. Does not change position of the
   * underlying ByteBuffers
   * 
  ...
  public short getShort(int index) {
    ...
    if (items.length - 1 == itemIndex) {
      // means cur item is the last one and we wont be able to read a int. Throw
      // exception
  ...
  private short getShort(int index, int itemIndex) {
    ...
    if (items.length - 1 == itemIndex) {
      // means cur item is the last one and we wont be able to read a short. Throw
      // exception
      throw new BufferUnderflowException();
    }
    ByteBuffer nextItem = items[itemIndex + 1];
  ...
  private long getLong(int index, int itemIndex) {
    ...
    if (items.length - 1 == itemIndex) {
      // means cur item is the last one and we wont be able to read a long. Throw
      // exception
      throw new BufferUnderflowException();
    }
    long l = 0;
  ...
  /**
   * Similar to {@link ByteBuffer}.reset(), ensures that this MBB is reset back to
   * last marked
   * position.
   * 
   * @return This MBB
  ...
  public MultiByteBuff reset() {
    checkRefCount();
    // when the buffer is moved to the next one.. the reset should happen on the
    // previous marked
    ...
    // All items after the marked position upto the current item should be reset to
    // 0
  ...
  /**
   * Copies the content from this MBB's current position to the byte array and
   * fills it. Also
  ...
  /**
   * Copies the specified number of bytes from this MBB's current position to the
   * byte[]'s offset.
  ...
  /**
   * Returns an MBB which is a sliced version of this MBB. The position, limit and
   * mark of the new
   * MBB will be independent than that of the original MBB. The content of the new
   * MBB will start at
  ...
  /**
   * Returns an MBB which is a duplicate version of this MBB. The position, limit
   * and mark of the
   * new MBB will be independent than that of the original MBB. The content of the
   * new MBB will
   * start at this MBB's current position The position, limit and mark of the new
   * MBB would be
  ...
  /**
   * Writes a byte to this MBB at the given index and won't affect the position of
   * any of the
   * buffers.
   * 
   * @return this object
  ...
  /**
   * Copies from a src BB to this MBB. This will be absolute positional copying
   * and won't affect the
   ...
   * @param srcOffset  the offset in the src MBB from where the elements should be
   *                   read
   ...
   *                                  remaining in src
   *                                  ByteBuff.
   * @throws BufferOverflowException  If there is insufficient available space in
   *                                  this MBB for
   *                                  length bytes.
   */
  @Override
  ...
  /**
   * Writes an int to this MBB at its current position. Also advances the position
   * by size of int
   * 
  ...
  public MultiByteBuff putInt(int val) {
    ...
    // During read, we will read as byte by byte for this case. So just write in Big
    // endian
    put(int3(val));
    put(int2(val));
    put(int1(val));
  ...
  /**
   * Writes a long to this MBB at its current position. Also advances the position
   * by size of long
   * 
   ...
   * 
   * @param offset the offset in this MBB from where the subBuffer should be
   *               created
   ...
   *               bytes starts. The
   *               method would set the values on the pair that is passed in by
   *               the caller
   */
  @Override
   ...
   * 
   * @param out          the ByteBuffer to which the copy has to happen, its
   *                     position will be
   *                     advanced.
   * @param sourceOffset the offset in the MBB from which the elements has to be
   *                     copied
   ...
   * length the position
   * from where the copy should start the length upto which the copy has to be
   * done
   * 
