...
public final class CellUtil {
    ...
    /**
     * Splits a column in {@code family:qualifier} form into separate byte arrays.
     * An empty qualifier
     * (ie, {@code fam:}) is parsed as <code>{ fam, EMPTY_BYTE_ARRAY }</code> while
     * no delimiter (ie,
     ...
     * @param destination       the destination byte[] to which the row has to be
     *                          copied
     ...
     * @param destination       the destination bytebuffer to which the row has to
     *                          be copied
     * @param destinationOffset the offset in the destination byte[]
     ...
     * @param destination       the destination byte[] to which the family has to be
     *                          copied
     * @param destinationOffset the offset in the destination byte[]
     * @return the offset of the byte[] after the copy has happened
     */
     ...
     * 
     * @param cell              the cell whose family has to be copied
     * @param destination       the destination bytebuffer to which the family has
     *                          to be copied
     ...
     * @param destination       the destination byte[] to which the qualifier has to
     *                          be copied
     * @param destinationOffset the offset in the destination byte[]
     * @return the offset of the byte[] after the copy has happened
     */
     ...
     * 
     * @param cell              the cell whose qualifier has to be copied
     * @param destination       the destination bytebuffer to which the qualifier
     *                          has to be copied
     * @param destinationOffset the offset in the destination bytebuffer
     * @return the offset of the bytebuffer after the copy has happened
     */
     ...
     * @param destination       the destination byte[] to which the value has to be
     *                          copied
     * @param destinationOffset the offset in the destination byte[]
     * @return the offset of the byte[] after the copy has happened
     */
     ...
     * 
     * @param cell              the cell whose value has to be copied
     * @param destination       the destination bytebuffer to which the value has to
     *                          be copied
     * @param destinationOffset the offset in the destination bytebuffer
     * @return the offset of the bytebuffer after the copy has happened
     */
    ...
    public static Cell createCell(final byte[] rowArray, final int rowOffset, final int rowLength,
        ...
        // See createCell(final byte [] row, final byte [] value) for why we default
        // Maximum type.
    ...
    /**
     * Marked as audience Private as of 1.2.0. Creating a Cell with a
     * memstoreTS/mvcc is an internal
    ...
    /**
     * Marked as audience Private as of 1.2.0. Creating a Cell with tags and a
     * memstoreTS/mvcc is an
    ...
    /**
     * Marked as audience Private as of 1.2.0. Creating a Cell with tags is an
    ...
    public static Cell createCell(final byte[] row, final byte[] value) {
        // An empty family + empty qualifier + Type.Minimum is used as flag to indicate
        // last on row.
        // See the CellComparator and KeyValue comparator. Search for compareWithoutRow.
        // Lets not make a last-on-row key as default but at same time, if you are
        // making a key
        // without specifying type, etc., flag it as weird by setting type to be
        // Maximum.
    ...
    @Deprecated
    public static Cell createCell(final byte[] row, final byte[] family, final byte[] qualifier) {
        // See above in createCell(final byte [] row, final byte [] value) why we set
        // type to Maximum.
    ...
    /**
     * Note : Now only CPs can create cell with tags using the CP environment Within
     * CP, use
    ...
    /**
     * Now only CPs can create cell with tags using the CP environment Within CP,
     * use
     * {@link RawCell#createCell(Cell, List)} method instead
     * 
     * @return A new cell which is having the extra tags also added to it.
    ...
    /**
     * Returns CellScanner interface over <code>cellIterable</code> or null if
     * <code>cells</code> is
     * null
     */
     ...
     * 
     * @param map Map of Cell Lists; for example, the map of families to Cells that
     *            is used inside
    ...
    /**
     * Return true if the rows in <code>left</code> and <code>right</code> Cells
     * match
     * 
    ...
    /**
     * Finds if the qualifier part of the cell and the KV serialized byte[] are
     * equal.
     * 
    ...
    /**
     * Estimate based on keyvalue's serialization format in the RPC layer. Note that
     * there is an extra
     ...
     * 
     * @return Estimate of the <code>cell</code> size in bytes plus an extra
     *         SIZEOF_INT indicating the
     *         actual cell length.
     * @deprecated As of release 2.0.0, this will be removed in HBase 3.0.0.
     */
    @Deprecated
    public static int estimatedSerializedSizeOf(final Cell cell) {
        ...
        // Use the KeyValue's infrastructure size presuming that another implementation
        // would have
        // same basic cost.
                KeyValue.ROW_LENGTH_SIZE + KeyValue.FAMILY_LENGTH_SIZE +
                // Serialization is probably preceded by a length (it is in the KeyValueCodec at
                // least).
                Bytes.SIZEOF_INT;
    }

    /**
     * Return sum of the lengths of all the elements in a Cell; does not count in
     * any infrastructure
     */
    ...
    /**
     * This is an estimate of the heap space occupied by a cell. When the cell is of
     * type
    ...
    /**
     * Sets the given seqId to the cell. Marked as audience Private as of 1.2.0.
     * Setting a Cell
    ...
    /**
     * Sets the given timestamp to the cell. Note that this method is a
     * LimitedPrivate API and may
    ...
    /**
     * Writes the Cell's key part as it would have serialized in a KeyValue. The
     * format is &lt;2 bytes
     * rk len&gt;&lt;rk&gt;&lt;1 byte cf len&gt;&lt;cf&gt;&lt;qualifier&gt;&lt;8
     * bytes
    ...
    public static void writeFlatKey(Cell cell, DataOutputStream out) throws IOException {
        ...
        byte fLen = cell.getFamilyLength();
        int qLen = cell.getQualifierLength();
        // Using just one if/else loop instead of every time checking before writing
        // every
        // component of cell
        if (cell instanceof ByteBufferExtendedCell) {
    ...
    /**
     * Writes the row from the given cell to the output stream excluding the common
     * prefix
     * 
    ...
    /**
     * This method exists just to encapsulate how we serialize keys. To be replaced
     * by a factory that
     ...
     * 
     * @return The key portion of the Cell serialized in the old-school KeyValue way
     *         or null if passed
    ...
    /**
     * Find length of common prefix in keys of the cells, considering key as byte[]
     * if serialized in
     * {@link KeyValue}. The key format is &lt;2 bytes rk len&gt;&lt;rk&gt;&lt;1
     * byte cf
     * len&gt;&lt;cf&gt;&lt;qualifier&gt;&lt;8 bytes timestamp&gt;&lt;1 byte
     * type&gt;
     * 
     * @param c1                the cell
     * @param c2                the cell
     * @param bypassFamilyCheck when true assume the family bytes same in both
     *                          cells. Pass it as true
     *                          when dealing with Cells in same CF so as to avoid
     *                          some checks
    ...
    public static int findCommonPrefixInFlatKey(Cell c1, Cell c2, boolean bypassFamilyCheck,
        ...
        if (c1 instanceof ByteBufferExtendedCell && c2 instanceof ByteBufferExtendedCell) {
            rkCommonPrefix = ByteBufferUtils.findCommonPrefix(((ByteBufferExtendedCell) c1).getRowByteBuffer(),
            ...
            // There cannot be a case where one cell is BBCell and other is KeyValue. This
            // flow comes
            // either
        ...
        if (bypassFamilyCheck) {
            // This flag will be true when caller is sure that the family will be same for
            // both the cells
            ...
            int fCommonPrefix;
            if (c1 instanceof ByteBufferExtendedCell && c2 instanceof ByteBufferExtendedCell) {
                fCommonPrefix = ByteBufferUtils.findCommonPrefix(((ByteBufferExtendedCell) c1).getFamilyByteBuffer(),
        ...
        int qCommon;
        if (c1 instanceof ByteBufferExtendedCell && c2 instanceof ByteBufferExtendedCell) {
            qCommon = ByteBufferUtils.findCommonPrefix(((ByteBufferExtendedCell) c1).getQualifierByteBuffer(),
     ...
     * @return greater than 0 if left cell's qualifier is bigger than byte[], lesser
     *         than 0 if left
     ...
     * @return an int greater than 0 if left is greater than right lesser than 0 if
     *         left is lesser
     ...
     * @return greater than 0 if left cell's family is bigger than byte[], lesser
     *         than 0 if left
     ...
     * @return greater than 0 if left cell's column is bigger than byte[], lesser
     *         than 0 if left
