...
/**
 * Comparator for comparing cells and has some specialized methods that allows
...
public interface CellComparator extends Comparator<Cell>, Serializable {
    /**
     * A comparator for ordering cells in user-space tables. Useful when writing
     * cells in sorted order
     * as necessary for bulk import (i.e. via MapReduce).
     * <p>
     * CAUTION: This comparator may provide inaccurate ordering for cells from
     * system tables, and
    ...
    /**
     * Lexographically compares two cells. The key part of the cell is taken for
     * comparison which
     ...
     * Compare cells.
     * 
     * @param ignoreSequenceid True if we are to compare the key portion only and
     *                         ignore the
     ...
     * 
     * @param row ByteBuffer that wraps a row; will read from current position and
     *            will reading all
    ...
    /**
     * Lexicographically compares the two cells excluding the row part. It compares
     * family, qualifier,
    ...
    /**
     * Compares cell's timestamps in DESCENDING order. The below older timestamps
     * sorting ahead of
     ...
     * @param leftCell  the left hand side cell
     * @param rightCell the right hand side cell
     * @return 1 if left's timestamp &lt; right's timestamp -1 if left's timestamp
     *         &gt; right's
     ...
     * {@link ByteBufferKeyValue}.
     * Create an instance when you make a new memstore, when you know only BBKVs
     * will be passed. Do
     * not pollute with types other than BBKV if can be helped; the Comparator will
     * slow.
     */
