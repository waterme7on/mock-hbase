 ...
 * {@link #getQualifierArray()},
 * {@link #getTimestamp()}, and {@link #getValueArray()}. These methods allocate
 * new byte arrays and
 ...
 * &lt;timestamp&gt; &lt;keytype&gt;</code> The <code>rowlength</code> maximum
 * is
 * <code>Short.MAX_SIZE</code>, column family length maximum is
 * <code>Byte.MAX_SIZE</code>, and
 * column qualifier + key length must be &lt; <code>Integer.MAX_SIZE</code>. The
 * column does not
...
public class KeyValue implements ExtendedCell, Cloneable {
    ...
    /**
     * Computes the number of bytes that a <code>KeyValue</code> instance with the
     * provided
    ...
    /**
     * Creates a KeyValue from the specified byte array, starting at offset, and for
     * length
     * <code>length</code>.
     * 
     * @param bytes  byte array
    ...
    /**
     * Constructs KeyValue structure filled with specified values. Uses the provided
     * buffer as the
     * data buffer.
     * <p>
     * Column is split into two fields, family and qualifier.
    ...
    /**
     * Create an empty byte[] representing a KeyValue All lengths are preset and can
     * be filled in
     * later.
     * 
     * @param rlength   row length
    ...
    /**
     * Returns the backing array of the entire KeyValue (all KeyValue fields are in
     * a single array)
     */
    @Override
    ...
    /**
     * Creates a new KeyValue that only contains the key portion (the value is set
     * to be null). TODO
     ...
     * @return KeyValue created by deserializing from <code>in</code> OR if we find
     *         a length of zero,
     ...
     * @return Created KeyValue OR if we find a length of zero, we will return null
     *         which can be
    ...
    /**
     * A simple form of KeyValue that creates a keyvalue with only the key part of
     * the byte[] Mainly
